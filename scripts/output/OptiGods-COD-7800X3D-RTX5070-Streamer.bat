...
setlocal
set "SELF=%~f0"
set "TMPPS1=%TEMP%\OptiGods-leaq-stream.ps1"

title Opti Gods by leaq  --  Streamer Optimizer
echo.
echo  ==========================================
echo    OPTI GODS by leaq  --  Streamer Build
echo    7800X3D + RTX 5070  --  Stream-Safe
echo  ==========================================
echo.
...
if not exist "%TMPPS1%" (
  echo.
  echo  [ERROR] Script extraction failed. Please re-download the file.
  echo.
  pause
  exit /b 1
)
...
##PS1_START##
# ============================================
# OPTI GODS by leaq — Streamer Optimizer
# Rig: AMD Ryzen 7 7800X3D | RTX 5070 | 32GB
# Profile: STREAMER-SAFE (OBS-priority build)
# Generated by leaq — optigods.com
# ============================================
...
Write-Host "  7800X3D + RTX 5070 + 32GB Win11" -ForegroundColor Cyan
Write-Host "=====================================" -ForegroundColor Red
Write-Host ""
Write-Host "  Stream-killer tweaks REMOVED:" -ForegroundColor DarkGray
Write-Host "  x CodHighPriority  (starves OBS encoder)" -ForegroundColor DarkGray
Write-Host "  x CodGameMode      (deprioritizes OBS)" -ForegroundColor DarkGray
Write-Host "  x CodDisableHAGS   (RTX 5070 needs HAGS ON)" -ForegroundColor DarkGray
Write-Host ""
...
Write-Host "[DETECT] Streamer mode: game priority NORMAL, OBS priority AboveNormal" -ForegroundColor Cyan
Write-Host ""
...
Write-Host "[NVCP] Low Latency=On, Shader Cache=Unlimited" -ForegroundColor Green
Write-Host ""

Write-Host "--- [Streamer Fixes] 3 tweak(s) ---" -ForegroundColor DarkRed

Write-Host "[>>] RemoveIfeoOverride..." -ForegroundColor DarkYellow
try {
    ...
    If (Test-Path $ifeo) {
        ...
        Write-Host "[STREAM] cod.exe IFEO priority override removed — OBS encoder threads no longer starved by COD." -ForegroundColor Green
    } Else {
        Write-Host "[STREAM] cod.exe IFEO was already clean — no action needed." -ForegroundColor DarkGray
    }
    $appliedTweaks.Add("RemoveIfeoOverride") | Out-Null
} catch {
    $failedTweaks.Add("RemoveIfeoOverride") | Out-Null
    Write-Host "[ERR] RemoveIfeoOverride: $_" -ForegroundColor Red
}

Write-Host "[>>] DisableGameMode..." -ForegroundColor DarkYellow
try {
    Set-ItemProperty -Path 'HKCU:\SOFTWARE\Microsoft\GameBar' -Name 'AutoGameModeEnabled' -Value 0 -Type DWord -Force -EA SilentlyContinue
    $key = 'HKCU:\System\GameConfigStore'
    If (!(Test-Path $key)) { New-Item $key -Force | Out-Null }
    Set-ItemProperty -Path $key -Name 'GameDVR_Enabled'          -Value 0 -Type DWord -Force -EA SilentlyContinue
    Set-ItemProperty -Path $key -Name 'GameDVR_FSEBehaviorMode'  -Value 2 -Type DWord -Force -EA SilentlyContinue
    ...
    Write-Host "[STREAM] Windows Game Mode DISABLED — OBS no longer deprioritized during COD gameplay." -ForegroundColor Green
    $appliedTweaks.Add("DisableGameMode") | Out-Null
} catch {
    $failedTweaks.Add("DisableGameMode") | Out-Null
    Write-Host "[ERR] DisableGameMode: $_" -ForegroundColor Red
}

Write-Host "[>>] ObsStreamPriority..." -ForegroundColor DarkYellow
try {
    $obs = Get-Process -Name 'obs64','obs' -EA SilentlyContinue | Select-Object -First 1
    If ($obs) {
        $obs.PriorityClass = 'AboveNormal'
        Write-Host "[STREAM] OBS set to AboveNormal priority — encoder gets CPU time before game when both compete." -ForegroundColor Green
    } Else {
        Write-Host "[STREAM] OBS not running. After you open OBS: Settings > Advanced > Process Priority = Above Normal." -ForegroundColor DarkYellow
    }
    $appliedTweaks.Add("ObsStreamPriority") | Out-Null
} catch {
    $failedTweaks.Add("ObsStreamPriority") | Out-Null
    Write-Host "[ERR] ObsStreamPriority: $_" -ForegroundColor Red
}
...
try {
    $base = 'HKLM:\SOFTWARE\NVIDIA Corporation\Global\'
    $ansel = Join-Path $base 'Ansel'
    If (Test-Path $ansel) { Set-ItemProperty -Path $ansel -Name 'AnselEnable' -Value 0 -Type DWord -Force -EA SilentlyContinue }
    $nfe = Join-Path $base 'NFE'
    If (!(Test-Path $nfe)) { New-Item $nfe -Force | Out-Null }
    Set-ItemProperty -Path $nfe -Name 'FeatureIds' -Value '' -Type String -Force -EA SilentlyContinue
    Write-Host "[COD] NVIDIA Ansel overlay disabled — removes DirectX hook from every game process." -ForegroundColor Green
...
try {
    ...
    If (Test-Path $key) {
        Set-ItemProperty -Path $key -Name 'NvCplLowLatencyMode' -Value 1 -Type DWord -Force -EA SilentlyContinue
        Write-Host "[COD] NVIDIA Low Latency mode on RTX 5070 — pre-rendered frame queue 3→1, lower input lag in gunfights." -ForegroundColor Green
    } Else { Write-Host "[SKIP] NVIDIA driver key not found." -ForegroundColor DarkGray }
...
try {
    ...
    If (Test-Path $cpPath) {
        Set-ItemProperty -Path $cpPath -Name 'ValueMax'   -Value 0 -Type DWord -Force -EA SilentlyContinue
        Set-ItemProperty -Path $cpPath -Name 'Attributes' -Value 1 -Type DWord -Force -EA SilentlyContinue
    }
    ...
    Write-Host "[COD] All 16 threads unparked on 7800X3D — no core wake latency during BO6 threading bursts." -ForegroundColor Green
...
try {
    ...
    If ($ryzen) {
        ...
        Write-Host "[COD] AMD Ryzen Balanced power plan activated — correct boost clock behavior for 7800X3D." -ForegroundColor Green
    } Else {
        ...
        Write-Host "[COD] CPU min 0% / max 100% applied — 7800X3D sustains full 5GHz boost during BO6." -ForegroundColor Green
    }
...
try {
    @("BattleNet", "Battle.net", "Agent") | ForEach-Object { Get-Process -Name $_ -EA SilentlyContinue | Where-Object { $_.MainWindowHandle -eq 0 } | Stop-Process -Force -EA SilentlyContinue }
    Write-Host "[COD] Battle.net background agents stopped — frees 50-150MB RAM and CPU cycles during gameplay." -ForegroundColor Green
...
try {
    Get-NetAdapter | Where-Object {$_.Status -eq 'Up'} | ForEach-Object { Disable-NetAdapterLso -Name $_.Name -EA SilentlyContinue; Write-Host "[COD] LSO disabled on: $($_.Name)" -ForegroundColor Cyan }
    Write-Host "[COD] Large Send Offload disabled on all active adapters — eliminates 5-30ms latency spikes." -ForegroundColor Green
...
try {
    Set-ItemProperty -Path 'HKLM:\SYSTEM\CurrentControlSet\Services\AFD\Parameters' -Name 'DefaultReceiveWindow' -Value 524288 -Type DWord -Force -EA SilentlyContinue
    Set-ItemProperty -Path 'HKLM:\SYSTEM\CurrentControlSet\Services\AFD\Parameters' -Name 'DefaultSendWindow'   -Value 524288 -Type DWord -Force -EA SilentlyContinue
    Write-Host "[COD] Network socket buffers 512KB — reduces packet loss spikes in Warzone BR server model." -ForegroundColor Green
...
try {
    $paths = @("$env:LOCALAPPDATA\Activision\cod\cache","$env:LOCALAPPDATA\Battle.net\Cache","$env:LOCALAPPDATA\NVIDIA\DXCache","$env:LOCALAPPDATA\D3DSCache")
    $cleaned = 0
    foreach ($p in $paths) { If (Test-Path $p) { Remove-Item -Path "$p\*" -Recurse -Force -EA SilentlyContinue; $cleaned++; Write-Host "[COD] Cleared: $p" -ForegroundColor Cyan } }
    Write-Host "[COD] Shader + GPU driver cache cleared ($cleaned folders). BO6 recompiles on next launch — 2-3 min stutter once, then clean every game." -ForegroundColor Green
...
try {
    ...
    Set-ItemProperty -Path 'HKLM:\SYSTEM\CurrentControlSet\Services\Tcpip\Parameters' -Name 'TcpTimestampOpt' -Value 0 -Type DWord -Force -EA SilentlyContinue
    Write-Host "[COD] TCP: Nagle off, immediate ACKs, timestamps off — tighter COD server tick alignment." -ForegroundColor Green
...
try {
    $cpPath = 'HKLM:\SYSTEM\CurrentControlSet\Control\Power\PowerSettings\54533251-82be-4824-96c1-47b60b740d00\0cc5b647-c1df-4637-891a-dec35c318583'
    Set-ItemProperty -Path $cpPath -Name 'ValueMax'   -Value 0 -Type DWord
    Set-ItemProperty -Path $cpPath -Name 'Attributes' -Value 1 -Type DWord
    powercfg -setacvalueindex SCHEME_CURRENT 54533251-82be-4824-96c1-47b60b740d00 0cc5b647-c1df-4637-891a-dec35c318583 100
    Write-Host "[OK] CPU Core Parking disabled — all cores remain active." -ForegroundColor Green
...
try {
    ...
    If (Test-Path $ptPath) { Set-ItemProperty -Path $ptPath -Name 'Attributes' -Value 1 -Type DWord }
    ...
    Write-Host "[OK] Power Throttling (Advanced) disabled." -ForegroundColor Green
...
try {
    ...
    Stop-Service -Name 'WerSvc' -Force; Set-Service -Name 'WerSvc' -StartupType Disabled
...
try {
    ...
    Write-Host "[NVIDIA] HAGS ENABLED (HwSchMode=2) — RTX 5070 benefits from hardware GPU scheduling. Reboot required." -ForegroundColor Green
...
try {
    ...
    Write-Host "[OK] TCP Auto-Tuning = Normal — dynamic receive window for max throughput." -ForegroundColor Green
...
try {
    ...
    Set-ItemProperty -Path $gamePath -Name 'MaximumPreRenderedFrames'-Value 1      -Type DWord
    Write-Host "[OK] MMCSS Game scheduler: High Category, GPU Priority 8, CPU Priority 6, PreRendered 1." -ForegroundColor Green
...
try {
    ...
    Set-ItemProperty -Path $tcpPath -Name 'EnablePMTUBHDetect'-Value 0 -Type DWord
    Write-Host "[OK] TCP Input Lag: TcpAckFrequency=1, TCPNoDelay=1, EnablePMTUBHDetect=0." -ForegroundColor Green
...
try {
    Get-NetAdapter | Where-Object { $_.Status -eq 'Up' } | ForEach-Object {
        ...
        $pnp = Get-PnpDevice -FriendlyName $_.InterfaceDescription -EA SilentlyContinue | Select-Object -First 1
        if ($pnp) { $pmPath = "HKLM:\SYSTEM\CurrentControlSet\Enum\$($pnp.InstanceId)\Device Parameters"; Set-ItemProperty $pmPath -Name 'PnPCapabilities' -Value 24 -Type DWord -Force -EA SilentlyContinue }
    }
    Write-Host "[Network] NIC power saving (EEE, Flow Control) disabled on all active adapters." -ForegroundColor Green
...
try {
    ...
    Write-Host "[Network] DNS set to Cloudflare (1.1.1.1 / 1.0.0.1) on all active adapters." -ForegroundColor Green
...
try {
    ...
    Write-Host "[Network] QoS bandwidth reservation 0% — full bandwidth available to all apps." -ForegroundColor Green
...
try {
    Get-NetAdapter | Where-Object { $_.Status -eq 'Up' } | ForEach-Object { Set-NetAdapterAdvancedProperty -Name $_.Name -RegistryKeyword '*InterruptModeration' -RegistryValue 0 -EA SilentlyContinue }
    Write-Host "[Network] Interrupt Moderation disabled — each packet triggers immediate CPU interrupt." -ForegroundColor Green
...
try {
    Get-NetAdapter | Where-Object { $_.Status -eq 'Up' } | ForEach-Object {
        $max = (Get-NetAdapterAdvancedProperty -Name $_.Name -RegistryKeyword '*NumRssQueues' -EA SilentlyContinue).NumericParameterMaxValue
        if ($max) { Set-NetAdapterAdvancedProperty -Name $_.Name -RegistryKeyword '*NumRssQueues' -RegistryValue $max -EA SilentlyContinue }
    }
    Write-Host "[Network] RSS queues set to maximum on all active adapters." -ForegroundColor Green
...
try {
    ...
    0,1,2,3 | ForEach-Object {
        ...
        If ((Test-Path $k) -and (Get-ItemProperty $k -Name 'DriverDesc' -EA SilentlyContinue).DriverDesc -match 'NVIDIA') {
            Set-ItemProperty $k -Name 'ForcedMipmapsMinLod' -Value 0  -Type DWord -Force -EA SilentlyContinue
            Set-ItemProperty $k -Name 'AnisotropicDegree'   -Value 16 -Type DWord -Force -EA SilentlyContinue
            Write-Host "[NVIDIA] Anisotropic Filtering 16x forced on $k" -ForegroundColor Green; $found = $true
        }
    }
...
try {
    Get-AppxPackage *XboxGamingOverlay* | Remove-AppxPackage -EA SilentlyContinue
    Stop-Process -Name "nvcontainer" -Force -EA SilentlyContinue
    Set-ItemProperty -Path 'HKCU:\SOFTWARE\NVIDIA Corporation\NVControlPanel2\Client' -Name 'OptInOrOutPreference' -Value 0 -Type DWord -EA SilentlyContinue
    Write-Host "[NVIDIA] Overlay and container process hints suppressed." -ForegroundColor Green
...
try {
    @('NvTelemetryContainer') | ForEach-Object { Stop-Service $_ -Force -EA SilentlyContinue; Set-Service $_ -StartupType Disabled -EA SilentlyContinue }
    reg delete "HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\Run" /v "NvBackend" /f 2>$null
    Write-Host "[NVIDIA] NvTelemetryContainer stopped. Display container kept running (stopping it causes NVIDIA Overlay.exe crash)." -ForegroundColor Green
...
try {
    ...
    Remove-ItemProperty $gdrv 'VerticalSyncOverride'  -EA SilentlyContinue
    Remove-ItemProperty $gdrv 'TripleBufferingOverride' -EA SilentlyContinue
    Set-ItemProperty -Path 'HKCU:\SOFTWARE\NVIDIA Corporation\Global\NVTweak\Policies' -Name 'VSync' -Value 0 -Type DWord -Force -EA SilentlyContinue
    Write-Host "[NVIDIA] VSync override cleared — force VSync Off in NVCP or in-game." -ForegroundColor Green
...
try {
    ...
    Set-ItemProperty $gamesPath 'GPU Priority'             8       -Type DWord  -Force
    Set-ItemProperty $gamesPath 'Priority'                 6       -Type DWord  -Force
    Set-ItemProperty $gamesPath 'Scheduling Category'     'High'   -Type String -Force
    Set-ItemProperty $gamesPath 'SFIO Priority'           'High'   -Type String -Force
    Set-ItemProperty $gamesPath 'MaximumPreRenderedFrames' 1       -Type DWord  -Force
    Set-ItemProperty -Path 'HKLM:\SYSTEM\CurrentControlSet\Control\GraphicsDrivers' -Name 'TdrDelay' -Value 10 -Type DWord -Force -EA SilentlyContinue
    Write-Host "[NVIDIA] Low Latency: GPU priority 8, Scheduling=High, PreRendered=1, TDR extended." -ForegroundColor Green
...
try {
    powercfg -setacvalueindex SCHEME_CURRENT 19caa947-ffffffff-ffffffff-ffffffff-ffffffff 233cfb73-ffffffff-ffffffff-ffffffff-ffffffff 1 2>$null
    Set-ItemProperty -Path 'HKLM:\SYSTEM\CurrentControlSet\Control\GraphicsDrivers' -Name 'PlatformSupportMiracast' -Value 0 -EA SilentlyContinue
    Set-ItemProperty -Path 'HKLM:\SYSTEM\CurrentControlSet\Control\GraphicsDrivers' -Name 'TdrLevel'                -Value 3 -EA SilentlyContinue
    Write-Host "[NVIDIA] Max performance mode hints applied." -ForegroundColor Green
...
try {
    ...
    0,1,2,3 | ForEach-Object {
        ...
        If ((Test-Path $k) -and (Get-ItemProperty $k -Name 'DriverDesc' -EA SilentlyContinue).DriverDesc -match 'NVIDIA') {
            ...
            Set-ItemProperty $k 'PowerMizerEnable'  1      -Type DWord -Force -EA SilentlyContinue
            ...
            Write-Host "[NVIDIA] PowerMizer Max Performance on $k" -ForegroundColor Green; $found = $true
        }
    }
    If (-not $found) { Write-Host "[NVIDIA] PowerMizer: GPU class key not found — apply via NVCP manually." -ForegroundColor Yellow }
...
try {
    $gamesPath = 'HKLM:\SOFTWARE\Microsoft\Windows NT\CurrentVersion\Multimedia\SystemProfile\Tasks\Games'
    If (!(Test-Path $gamesPath)) { New-Item -Path $gamesPath -Force | Out-Null }
    Set-ItemProperty -Path $gamesPath -Name 'MaximumPreRenderedFrames' -Value 1 -Type DWord
    ...
    Write-Host "[NVIDIA] MaximumPreRenderedFrames=1, GPU Priority=8 — input latency minimized." -ForegroundColor Green
...
try {
    $vsrPath = 'HKCU:\SOFTWARE\NVIDIA Corporation\NvControlPanel2\Client'
    If (!(Test-Path $vsrPath)) { New-Item $vsrPath -Force | Out-Null }
    Set-ItemProperty $vsrPath 'OptInOrOutPreference' 0 -Type DWord -Force -EA SilentlyContinue
    $rtxVid = 'HKCU:\SOFTWARE\NVIDIA Corporation\Global\RTXVideoManager'
    If (!(Test-Path $rtxVid)) { New-Item $rtxVid -Force | Out-Null }
    Set-ItemProperty $rtxVid 'RTXVideoSuperRes' 0 -Type DWord -Force -EA SilentlyContinue
    Set-ItemProperty $rtxVid 'RTXVideoHDR'      0 -Type DWord -Force -EA SilentlyContinue
    Write-Host "[GPU Thermal] RTX Video Super Resolution + RTX HDR disabled — frees tensor cores for gaming." -ForegroundColor Green
...
try {
    $reflexPath = 'HKLM:\SOFTWARE\NVIDIA Corporation\Global\Reflex'
    If (!(Test-Path $reflexPath)) { New-Item $reflexPath -Force | Out-Null }
    Set-ItemProperty $reflexPath 'Enable'       1 -Type DWord -Force -EA SilentlyContinue
    Set-ItemProperty $reflexPath 'BoostEnabled' 1 -Type DWord -Force -EA SilentlyContinue
    $gamePath = 'HKLM:\SOFTWARE\Microsoft\Windows NT\CurrentVersion\Multimedia\SystemProfile\Tasks\Games'
    If (!(Test-Path $gamePath)) { New-Item $gamePath -Force | Out-Null }
    Set-ItemProperty $gamePath 'MaximumPreRenderedFrames' 1 -Type DWord -Force -EA SilentlyContinue
    Write-Host "[NVIDIA] Reflex enabled (Enable=1, BoostEnabled=1) — enable Reflex in COD settings for lowest input lag." -ForegroundColor Green
...
try {
    If (!(Test-Path 'HKLM:\SOFTWARE\NVIDIA Corporation\Global\NGXCore')) { New-Item 'HKLM:\SOFTWARE\NVIDIA Corporation\Global\NGXCore' -Force | Out-Null }
    Set-ItemProperty -Path 'HKCU:\SOFTWARE\NVIDIA Corporation\Global\NVTweak' -Name 'Ordinal' -Value 1 -Type DWord -EA SilentlyContinue
    Set-ItemProperty -Path 'HKLM:\SOFTWARE\Microsoft\DirectX' -Name 'ShaderCache' -Value 1 -Type DWord -EA SilentlyContinue
    Write-Host "[NVIDIA] Shader pre-caching enabled." -ForegroundColor Green
...
try {
    ...
    Set-ItemProperty $nvKey 'Threaded_Optimization_Override' 1 -Type DWord -Force -EA SilentlyContinue
    netsh int tcp set global dca=enabled 2>$null
    Set-ItemProperty 'HKLM:\SOFTWARE\Microsoft\DirectX' 'ThreadedOptimization' 1 -Type DWord -Force -EA SilentlyContinue
...
try {
    $gdrv = 'HKLM:\SYSTEM\CurrentControlSet\Control\GraphicsDrivers'
    Remove-ItemProperty $gdrv 'TripleBufferingOverride' -EA SilentlyContinue
    $nvPol = 'HKCU:\SOFTWARE\NVIDIA Corporation\Global\NVTweak\Policies'
    If (!(Test-Path $nvPol)) { New-Item $nvPol -Force | Out-Null }
    Set-ItemProperty $nvPol 'TripleBuffering' 0 -Type DWord -Force -EA SilentlyContinue
    Write-Host "[NVIDIA] Triple Buffering disabled — lower input latency." -ForegroundColor Green
...
try {
    Add-Type -MemberDefinition '[DllImport("psapi.dll")] public static extern int EmptyWorkingSet(IntPtr hProcess);' -Name 'MemTrimRO' -Namespace 'WinAPI' -EA SilentlyContinue
    [WinAPI.MemTrimRO]::EmptyWorkingSet([IntPtr](-1)) | Out-Null
    [System.GC]::Collect()
    Write-Host "[OK] Standby list flushed — physical RAM reclaimed." -ForegroundColor Green
...
try {
    ...
    netsh int tcp set global dca=enabled            2>$null
    Write-Host "[OK] TCP globals: autotune=normal, chimney=off, dca=on." -ForegroundColor Green
...
try {
    ...
    @('GTA5.exe','FiveM.exe','valorant.exe','cs2.exe','FortniteClient-Win64-Shipping.exe','r5apex.exe') | ForEach-Object {
        $k = "$ifeo\$_\PerfOptions"
        If (!(Test-Path $k)) { New-Item $k -Force | Out-Null }
        Set-ItemProperty $k 'GpuPriority' 8 -Type DWord -Force
    }
    Write-Host "[Process] GPU Scheduler Priority 8 set for game executables (cod.exe intentionally excluded — streamer safe)." -ForegroundColor Green
...
try {
    ...
    If (!(Test-Path $mmcss)) { New-Item $mmcss -Force | Out-Null }
    Set-ItemProperty $mmcss -Name 'Scheduling Category' -Value 'High'  -Type String -Force
    Set-ItemProperty $mmcss -Name 'SFIO Priority'       -Value 'High'  -Type String -Force
    Set-ItemProperty $mmcss -Name 'GPU Priority'        -Value 8       -Type DWord  -Force
    Set-ItemProperty $mmcss -Name 'Priority'            -Value 6       -Type DWord  -Force
    Set-ItemProperty $mmcss -Name 'Background Only'     -Value 'False' -Type String -Force
    Write-Host "[Process] MMCSS Gaming: SchedulingCategory=High, GPU Priority=8, CPU Priority=6." -ForegroundColor Green
...
try {
    ...
    Set-ItemProperty -Path 'HKLM:\SYSTEM\CurrentControlSet\Services\Dnscache\Parameters' -Name 'MaxNegativeCacheTtl' -Value 0     -Type DWord -Force
    netsh int tcp set global timestamps=disabled 2>$null
    Write-Host "[OK] DNS: MaxCacheTTL=86400, NegativeCache=0, timestamps disabled." -ForegroundColor Green
...
try {
    ...
    $guid = (powercfg -l | Select-String 'Ultimate Performance').Line.Split(' ')[3]
...
try {
    # 14 instead of 10 — gives OBS and background apps more CPU slice during gaming
    ...
    Write-Host "[OK] SystemResponsiveness=14 — OBS encoder gets fair CPU share alongside the game." -ForegroundColor Green
...
try {
    ...
    New-Item -Path 'HKLM:\SYSTEM\CurrentControlSet\Control\Session Manager\Power' -Force -EA SilentlyContinue | Out-Null
    Set-ItemProperty -Path 'HKLM:\SYSTEM\CurrentControlSet\Control\Session Manager\Power' -Name 'HiberbootEnabled' -Value 0 -Type DWord -Force -EA SilentlyContinue
    Write-Host "[System] Hibernation disabled — reclaims disk space equal to RAM." -ForegroundColor Green
...
try {
    Set-ItemProperty -Path 'HKCU:\Software\Microsoft\Windows\CurrentVersion\Explorer\VisualEffects' -Name 'VisualFXSetting' -Value 2 -Type DWord -Force -EA SilentlyContinue
    $mask = [byte[]](0x90,0x12,0x01,0x80,0x10,0x00,0x00,0x00)
    Set-ItemProperty -Path 'HKCU:\Control Panel\Desktop' -Name 'UserPreferencesMask' -Value $mask -Type Binary -Force -EA SilentlyContinue
    Set-ItemProperty -Path 'HKCU:\Control Panel\Desktop' -Name 'FontSmoothing'       -Value '2'   -Force -EA SilentlyContinue
    New-Item -Path 'HKCU:\Software\Microsoft\Windows\DWM' -Force -EA SilentlyContinue | Out-Null
    Set-ItemProperty -Path 'HKCU:\Software\Microsoft\Windows\DWM' -Name 'EnableAeroPeek' -Value 0 -Type DWord -Force -EA SilentlyContinue
    Write-Host "[System] Visual effects = Best Performance — compositor animations off." -ForegroundColor Green
...
try {
    # 38 (0x26) instead of 26 — balanced foreground/background, better for OBS running alongside game
    Set-ItemProperty -Path 'HKLM:\SYSTEM\CurrentControlSet\Control\PriorityControl' -Name 'Win32PrioritySeparation' -Value 38
    Write-Host "[OK] Win32PrioritySeparation=38 — balanced CPU time between game and OBS encoder." -ForegroundColor Green
...
try {
    Add-Type -MemberDefinition '[DllImport("psapi.dll")] public static extern int EmptyWorkingSet(IntPtr hProcess);' -Name 'MemHelperSL' -Namespace 'WinAPI' -EA SilentlyContinue
    [WinAPI.MemHelperSL]::EmptyWorkingSet([IntPtr](-1)) | Out-Null
    Write-Host "[OK] Standby list cleared — RAM freed for game." -ForegroundColor Green
...
try {
    ...
    Get-AppxPackage *MicrosoftTeams* | Where-Object SignatureKind -eq 'Store' | Remove-AppxPackage
...
try {
    ...
    winget uninstall --id MicrosoftCorporationII.Windows.DevHome 2>$null
...
Write-Host ""
Write-Host "  STREAMER CHECKLIST (do these in COD):" -ForegroundColor Yellow
Write-Host "  1. Graphics > Display Mode = BORDERLESS WINDOWED" -ForegroundColor White
Write-Host "     (fixes alt-tab choppiness on stream)" -ForegroundColor DarkGray
Write-Host "  2. In OBS: Settings > Advanced > Process Priority = Above Normal" -ForegroundColor White
Write-Host "     (if OBS wasn't open when you ran this file)" -ForegroundColor DarkGray
Write-Host "  3. Enable Reflex in COD graphics settings" -ForegroundColor White
Write-Host "     (RTX 5070 supports it — lower input lag)" -ForegroundColor DarkGray
Write-Host ""
