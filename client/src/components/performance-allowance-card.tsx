...
import { applyTweak, createRestorePoint, isNative } from "@/lib/tauri-bridge";
  ...
  const chooseBest = async () => {
    ...
    try {
      ...
      if (!isNative()) {
        ...
        toast({ title: "Best 15 selected", description: "Server-validated safe tweaks are selected for your next script. Nothing was applied yet." });
        await refresh();
        return;
      }
      ...
      for (const id of ids) {
        ...
        let osApplied = false;
        try {
          const result = await applyTweak(id, authBody.ticket, getNativeAuthHeaders()["X-Native-Auth"] || null);
  ...
  return (
    <section className="rounded-xl border border-red-500/20 bg-red-500/5 p-5" data-testid="performance-allowance-card">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h2 className="text-sm font-bold text-white">Free performance allowance</h2>
          <p className="mt-1 text-xs text-zinc-400">Choose up to {status.limit} unique tweaks for this account. Undo never refunds.</p>
          <p className="mt-2 text-xs font-semibold text-red-300">Used {status.used} / {status.limit} · {status.remaining} remaining</p>
        </div>
        <div className="flex gap-2">
          <button type="button" disabled={busy || status.remaining === 0} onClick={() => void chooseBest()} className="rounded-lg bg-red-600 px-3 py-2 text-xs font-bold text-white disabled:opacity-50">
            {busy ? "Selecting…" : "Best 15 for my system"}
          </button>
