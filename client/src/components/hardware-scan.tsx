...
export const PS1_CMD = `$gpu=(Get-WmiObject Win32_VideoController|Where-Object{$_.AdapterRAM -gt 0}|Sort-Object AdapterRAM -Desc|Select-Object -First 1).Name; if(!$gpu){$gpu=(Get-WmiObject Win32_VideoController|Select-Object -First 1).Name}; $cpu=Get-WmiObject Win32_Processor|Select-Object -First 1; $ram=[Math]::Round((Get-WmiObject Win32_ComputerSystem).TotalPhysicalMemory/1GB,0); $os=Get-WmiObject Win32_OperatingSystem; $osBuild=[int]$os.BuildNumber; $osName=$os.Caption.Trim(); $dir=if(Test-Path "$env:USERPROFILE\\Desktop"){"$env:USERPROFILE\\Desktop"}else{"$env:TEMP"}; $base=$dir+"\\OptiGods-HW-Scan"; $path=$base+".json"; $n=2; while(Test-Path $path){$path=$base+"_"+$n+".json";$n++}; @{GPU=$gpu;CPU=$cpu.Name;Cores=$cpu.NumberOfCores;Threads=$cpu.NumberOfLogicalProcessors;RAM_GB=$ram;OsName=$osName;OsBuild=$osBuild}|ConvertTo-Json|Out-File $path -Encoding utf8; Write-Host "Done! File saved to: $path" -ForegroundColor Green`;
