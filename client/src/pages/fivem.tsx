...
export default function Fivem() {
  ...
  const CRASH_FIX_TWEAKS: Tweak[] = [
    { id: "FiveMFixProductId", title: "Fix: CfxState.h Crash — productId != ProductID::INVALID", desc: "Fixes the 'Assertion failure: productId != ProductID::INVALID (CfxState.h:88)' crash. Root cause: IFEO MitigationOptions or a stale Debugger key on Rockstar/FiveM executables corrupts FiveM's hardware product ID read at startup. This fix clears MitigationOptions + Debugger from IFEO for RockstarGamesLauncher.exe, PlayGTAV.exe, SocialClubHelper.exe, GTA5.exe and FiveM.exe, purges the CfxState priv cache, and re-enables Rockstar Service if it was disabled. Reboot and relaunch via Rockstar Games Launcher after applying.", impact: "HIGH", badge: "CFXSTATE FIX" },
