...
const TWEAK_COMMANDS: Record<string, string> = {
  ...
  // Mouse
  MousePointerSpeed611: `Set-ItemProperty -Path 'HKCU:\\Control Panel\\Mouse' -Name 'MouseSensitivity' -Value '10' -Type String -Force; Write-Host "[Mouse] Pointer speed set to 6/11 — neutral baseline, no acceleration curve applied" -ForegroundColor Green`,
  MouseDataQueueSize: `If (!(Test-Path 'HKLM:\\SYSTEM\\CurrentControlSet\\Services\\mouclass\\Parameters')) { New-Item -Path 'HKLM:\\SYSTEM\\CurrentControlSet\\Services\\mouclass\\Parameters' -Force | Out-Null }; Set-ItemProperty -Path 'HKLM:\\SYSTEM\\CurrentControlSet\\Services\\mouclass\\Parameters' -Name 'MouseDataQueueSize' -Value 20 -Type DWord -Force; Write-Host "[Mouse] Input buffer reduced to 20 entries (default 100) — lower click-to-action latency" -ForegroundColor Green`,
  MouseHoverTimeMin: `Set-ItemProperty -Path 'HKCU:\\Control Panel\\Mouse' -Name 'MouseHoverTime' -Value '10' -Type String -Force; Write-Host "[Mouse] Hover time set to 10ms (default 400ms) — snappier UI response" -ForegroundColor Green`,
  ...
  // Keyboard
  KeyboardRepeatRateMax: `Set-ItemProperty -Path 'HKCU:\\Control Panel\\Keyboard' -Name 'KeyboardSpeed' -Value '31' -Type String -Force; Write-Host "[Keyboard] Repeat rate set to maximum (31) — fastest key repeat speed" -ForegroundColor Green`,
  KeyboardRepeatDelayMin: `Set-ItemProperty -Path 'HKCU:\\Control Panel\\Keyboard' -Name 'KeyboardDelay' -Value '0' -Type String -Force; Write-Host "[Keyboard] Repeat delay set to minimum (250ms) — key repeat kicks in immediately" -ForegroundColor Green`,
  KeyboardDisableFilterKeys: `If (!(Test-Path 'HKCU:\\Control Panel\\Accessibility\\Keyboard Response')) { New-Item -Path 'HKCU:\\Control Panel\\Accessibility\\Keyboard Response' -Force | Out-Null }; Set-ItemProperty -Path 'HKCU:\\Control Panel\\Accessibility\\Keyboard Response' -Name 'Flags' -Value '0' -Type String -Force; Set-ItemProperty -Path 'HKCU:\\Control Panel\\Accessibility\\Keyboard Response' -Name 'AutoRepeatDelay' -Value '0' -Type String -Force; Set-ItemProperty -Path 'HKCU:\\Control Panel\\Accessibility\\Keyboard Response' -Name 'AutoRepeatRate' -Value '0' -Type String -Force; Set-ItemProperty -Path 'HKCU:\\Control Panel\\Accessibility\\Keyboard Response' -Name 'BounceTime' -Value '0' -Type String -Force; Set-ItemProperty -Path 'HKCU:\\Control Panel\\Accessibility\\Keyboard Response' -Name 'DelayBeforeAcceptance' -Value '0' -Type String -Force; Write-Host "[Keyboard] Filter Keys disabled — all artificial keystroke delay removed" -ForegroundColor Green`,
  ...
  FiveMDisableVSync: `$cfg = "$env:AppData\\CitizenFX\\gta5_settings.xml"; If (!(Test-Path -LiteralPath $cfg)) { Write-Host "[SKIP] FiveM settings were not detected; VSync is unchanged until FiveM has been launched once." -ForegroundColor DarkGray } Else { try { [xml]$xml = Get-Content -LiteralPath $cfg -Raw -ErrorAction Stop; $node = $xml.SelectSingleNode("//video/VSync"); If (!$node) { Write-Host "[SKIP] FiveM VSync is not exposed in this settings file." -ForegroundColor DarkGray } Else { $node.value = "0"; $xml.Save($cfg); [xml]$verify = Get-Content -LiteralPath $cfg -Raw -ErrorAction Stop; If ($verify.SelectSingleNode("//video/VSync").value -ne "0") { throw "Windows saved the file but VSync verification failed." }; Write-Host "[OK] FiveM VSync disabled and verified." -ForegroundColor Green } } catch { Write-Host "[SKIP] FiveM VSync could not be changed safely: $($_.Exception.Message)" -ForegroundColor DarkGray } }`,
  ...
  DebloatOneDrive: `Get-Process -Name OneDrive -ErrorAction SilentlyContinue | Stop-Process -Force -ErrorAction SilentlyContinue; $setupPaths = @("$env:SystemRoot\\System32\\OneDriveSetup.exe","$env:SystemRoot\\SysWOW64\\OneDriveSetup.exe","$env:LOCALAPPDATA\\Microsoft\\OneDrive\\OneDriveSetup.exe","$env:LOCALAPPDATA\\Microsoft\\OneDrive\\Update\\OneDriveSetup.exe"); $found = $setupPaths | Where-Object { Test-Path -LiteralPath $_ } | Select-Object -First 1; if ($found) { & $found /uninstall 2>$null; Write-Host "[OK] OneDrive uninstaller ran" -ForegroundColor Green } else { Write-Host "[OK] OneDrive is already absent — nothing to uninstall" -ForegroundColor Green }; Get-AppxPackage *Microsoft.OneDrive* -EA SilentlyContinue | Remove-AppxPackage -EA SilentlyContinue; Remove-ItemProperty -Path "HKCU:\\SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\Run" -Name "OneDrive" -EA SilentlyContinue; Write-Host "[OK] OneDrive removal verified" -ForegroundColor Green`,
  ...
  DiscordClearCache: `$cacheDirs = @("$env:APPDATA\\discord\\Cache","$env:APPDATA\\discord\\Code Cache","$env:APPDATA\\discord\\GPUCache","$env:APPDATA\\discord\\blob_storage"); $total = [long]0; ForEach ($dir in $cacheDirs) { If (Test-Path -LiteralPath $dir) { $files = @(Get-ChildItem -LiteralPath $dir -File -Recurse -EA SilentlyContinue); $size = If ($files.Count -gt 0) { [long](($files | Measure-Object -Property Length -Sum).Sum) } Else { [long]0 }; Remove-Item "$dir\\*" -Recurse -Force -EA SilentlyContinue; $total += $size } }; $mb = [Math]::Round($total/1MB, 1); Write-Host "[Discord] Cache cleared — freed $($mb) MB. Fixes lag, texture glitches, and slow load times" -ForegroundColor Green`,
  ...
  DiscordDisableRichPresence: `$settings = "$env:APPDATA\\discord\\settings.json"; If (Test-Path $settings) { $raw = Get-Content $settings -Raw; If ($raw -notmatch '"detectPlatformGames"') { $raw = $raw.TrimEnd().TrimEnd('}') + ', "detectPlatformGames": false, "showCurrentGame": false }'; $raw | Set-Content $settings -Encoding UTF8; Write-Host "[Discord] Game Activity (Rich Presence) disabled — Discord stops scanning running processes every few seconds" -ForegroundColor Green } ElseIf ($raw -match '"detectPlatformGames"\\s*:\\s*true') { $raw = $raw -replace '"detectPlatformGames"\\s*:\\s*true', '"detectPlatformGames": false'; $raw | Set-Content $settings -Encoding UTF8; Write-Host "[Discord] Game Activity scanner disabled" -ForegroundColor Green } Else { Write-Host "[Discord] Game Activity already disabled" -ForegroundColor Yellow } } Else { Write-Host "[Discord] settings.json not found — open Discord first" -ForegroundColor Yellow }`,
  DiscordDisableGifAutoplay: `$settings = "$env:APPDATA\\discord\\settings.json"; If (Test-Path $settings) { $raw = Get-Content $settings -Raw; If ($raw -notmatch '"gifAutoPlay"') { $raw = $raw.TrimEnd().TrimEnd('}') + ', "gifAutoPlay": false, "animatedEmojiAutoplay": false, "showEmojiSuggestions": false }'; $raw | Set-Content $settings -Encoding UTF8; Write-Host "[Discord] GIF autoplay + animated emoji disabled — stops per-frame GPU render calls for animated content while gaming" -ForegroundColor Green } Else { Write-Host "[Discord] GIF autoplay already disabled" -ForegroundColor Yellow } } Else { Write-Host "[Discord] settings.json not found — open Discord first" -ForegroundColor Yellow }`,
  DiscordMinimizeBgLoad: `$settings = "$env:APPDATA\\discord\\settings.json"; If (Test-Path $settings) { $raw = Get-Content $settings -Raw; If ($raw -notmatch '"inlineAttachmentMedia"') { $raw = $raw.TrimEnd().TrimEnd('}') + ', "inlineAttachmentMedia": false, "renderSpoilers": "ON_CLICK", "showMemberListAvatars": false }'; $raw | Set-Content $settings -Encoding UTF8; Write-Host "[Discord] Inline media preload off + spoilers on-click + member avatars off — reduces background network + GPU load while gaming" -ForegroundColor Green } Else { Write-Host "[Discord] Background load already minimized" -ForegroundColor Yellow } } Else { Write-Host "[Discord] settings.json not found — open Discord first" -ForegroundColor Yellow }`,
  DiscordSuppressNotifications: `$settings = "$env:APPDATA\\discord\\settings.json"; If (Test-Path $settings) { $raw = Get-Content $settings -Raw; If ($raw -notmatch '"notifyFriendsOnline"') { $raw = $raw.TrimEnd().TrimEnd('}') + ', "notifyFriendsOnline": false, "notifyTyping": false }'; $raw | Set-Content $settings -Encoding UTF8; Write-Host "[Discord] Friend online + typing notifications disabled — no popup interruptions during gaming" -ForegroundColor Green } Else { Write-Host "[Discord] Notifications already suppressed" -ForegroundColor Yellow } }; $faPath = "HKCU:\\SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\Notifications\\Settings"; If (!(Test-Path $faPath)) { New-Item -Path $faPath -Force | Out-Null }; Set-ItemProperty -Path $faPath -Name "NOC_GLOBAL_SETTING_ALLOW_TOASTS_ABOVE_LOCK" -Value 0 -Type DWord -Force; Write-Host "[Discord] Windows fullscreen toast suppression enabled" -ForegroundColor Green`,
  DiscordDisableSpellcheck: `$settings = "$env:APPDATA\\discord\\settings.json"; If (Test-Path $settings) { $raw = Get-Content $settings -Raw; If ($raw -notmatch '"enableSpellCheck"') { $raw = $raw.TrimEnd().TrimEnd('}') + ', "enableSpellCheck": false }'; $raw | Set-Content $settings -Encoding UTF8; Write-Host "[Discord] Spellcheck worker disabled — no more background spell analysis on every keystroke in chat" -ForegroundColor Green } ElseIf ($raw -match '"enableSpellCheck"\\s*:\\s*true') { $raw = $raw -replace '"enableSpellCheck"\\s*:\\s*true', '"enableSpellCheck": false'; $raw | Set-Content $settings -Encoding UTF8; Write-Host "[Discord] Spellcheck disabled" -ForegroundColor Green } Else { Write-Host "[Discord] Spellcheck already disabled" -ForegroundColor Yellow } } Else { Write-Host "[Discord] settings.json not found — open Discord first" -ForegroundColor Yellow }`,
  // Startup apps
  ...
  su_onedrive: `$_runKey = Get-ItemProperty 'HKCU:\\SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\Run' -EA SilentlyContinue; $_odLnk = "$env:APPDATA\\Microsoft\\Windows\\Start Menu\\Programs\\Startup\\OneDrive.lnk"; $_odFound = ($_runKey.OneDrive -ne $null) -or (Test-Path $_odLnk); if ($_odFound) { reg delete "HKCU\\SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\Run" /v "OneDrive" /f 2>$null; if (Test-Path $_odLnk) { Remove-Item $_odLnk -Force -EA SilentlyContinue }; $saPath = "HKCU:\\SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\Explorer\\StartupApproved\\Run"; if (!(Test-Path $saPath)) { New-Item $saPath -Force | Out-Null }; New-ItemProperty -Path $saPath -Name "OneDrive" -Value ([byte[]](0x03,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00)) -PropertyType Binary -Force -EA SilentlyContinue | Out-Null; Write-Host "[OK] OneDrive removed from startup" -ForegroundColor Green } else { Write-Host "[SKIP] OneDrive not on startup" -ForegroundColor DarkGray }`,
  ...
  su_ea_app: `$eaKeys = @("EADesktop","EA Desktop","EALauncher","Electronic Arts"); foreach ($v in $eaKeys) { reg delete "HKCU\\SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\Run" /v $v /f 2>$null }; $saPath = "HKCU:\\SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\Explorer\\StartupApproved\\Run"; if (!(Test-Path $saPath)) { New-Item $saPath -Force | Out-Null }; foreach ($k in $eaKeys) { New-ItemProperty -Path $saPath -Name $k -Value ([byte[]](0x03,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00)) -PropertyType Binary -Force -EA SilentlyContinue | Out-Null }; Get-ScheduledTask | Where-Object { $_.TaskName -like "*EABackground*" -or $_.TaskName -like "*EA Desktop*" -or $_.TaskName -like "*EALauncher*" } | Disable-ScheduledTask -EA SilentlyContinue; Write-Host "[OK] EA App removed from ALL startup locations — open EA App manually when you want to play EA games" -ForegroundColor Green`,
  su_epic: `$epicKeys = @("EpicGamesLauncher","Epic Games Launcher","EpicLauncher"); foreach ($v in $epicKeys) { reg delete "HKCU\\SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\Run" /v $v /f 2>$null }; $lnks = @("$env:APPDATA\\Microsoft\\Windows\\Start Menu\\Programs\\Startup\\Epic Games Launcher.lnk"); foreach ($lnk in $lnks) { If (Test-Path $lnk) { Remove-Item $lnk -Force } }; $saPath = "HKCU:\\SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\Explorer\\StartupApproved\\Run"; if (!(Test-Path $saPath)) { New-Item $saPath -Force | Out-Null }; New-ItemProperty -Path $saPath -Name "EpicGamesLauncher" -Value ([byte[]](0x03,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00)) -PropertyType Binary -Force -EA SilentlyContinue | Out-Null; Get-ScheduledTask | Where-Object { $_.TaskName -like "*Epic*" -or $_.TaskName -like "*EOS*" } | Disable-ScheduledTask -EA SilentlyContinue; Write-Host "[OK] Epic Games Launcher removed from ALL startup locations" -ForegroundColor Green`,
  su_ubisoft: `$ubiKeys = @("Ubisoft Connect","UbisoftConnect","upc"); foreach ($v in $ubiKeys) { reg delete "HKCU\\SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\Run" /v $v /f 2>$null }; $saPath = "HKCU:\\SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\Explorer\\StartupApproved\\Run"; if (!(Test-Path $saPath)) { New-Item $saPath -Force | Out-Null }; foreach ($k in $ubiKeys) { New-ItemProperty -Path $saPath -Name $k -Value ([byte[]](0x03,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00)) -PropertyType Binary -Force -EA SilentlyContinue | Out-Null }; Get-ScheduledTask | Where-Object { $_.TaskName -like "*Ubisoft*" } | Disable-ScheduledTask -EA SilentlyContinue; Write-Host "[OK] Ubisoft Connect removed from ALL startup locations" -ForegroundColor Green`,
  ...
  DisableWindowsError: `$werPath = 'HKLM:\\SOFTWARE\\Microsoft\\Windows\\Windows Error Reporting'; If (!(Test-Path $werPath)) { New-Item -Path $werPath -Force | Out-Null }; Set-ItemProperty -Path $werPath -Name 'Disabled' -Value 1 -Type DWord -Force; $wer = Get-Service -Name 'WerSvc' -EA SilentlyContinue; If ($wer) { Stop-Service -Name 'WerSvc' -Force -EA SilentlyContinue; Set-Service -Name 'WerSvc' -StartupType Disabled -EA SilentlyContinue; Write-Host "[OK] Windows Error Reporting disabled" -ForegroundColor Green } Else { Write-Host "[OK] Windows Error Reporting service is not installed; registry policy disabled" -ForegroundColor Green }`,
  ...
  ServiceRetailDemo: `$svc = Get-Service -Name 'RetailDemo' -EA SilentlyContinue; If ($svc) { Stop-Service -Name 'RetailDemo' -Force -EA SilentlyContinue; Set-Service -Name 'RetailDemo' -StartupType Disabled -EA SilentlyContinue; Write-Host "[OK] RetailDemo disabled" -ForegroundColor Green } Else { Write-Host "[OK] RetailDemo is not installed; nothing to disable" -ForegroundColor Green }`,
  ServiceTabletInput: `$svc = Get-Service -Name 'TabletInputService' -EA SilentlyContinue; If ($svc) { sc.exe stop 'TabletInputService' 2>$null | Out-Null; sc.exe config 'TabletInputService' start= disabled 2>$null | Out-Null; Write-Host "[OK] Tablet Input Panel service disabled" -ForegroundColor Green } Else { Write-Host "[OK] Tablet Input Panel service is not installed; nothing to disable" -ForegroundColor Green }`,
  ...
  FortniteDisableThrottling: `$_c=(Get-CimInstance Win32_Processor -Property NumberOfCores|Measure-Object NumberOfCores -Sum).Sum; $_pri=if($_c -ge 6){4}else{3}; $key = 'HKLM:\\SOFTWARE\\Microsoft\\Windows NT\\CurrentVersion\\Image File Execution Options\\FortniteClient-Win64-Shipping.exe'; If (!(Test-Path $key)) { New-Item -Path $key -Force }; Set-ItemProperty -Path "$key\\PerfOptions" -Name 'CpuPriorityClass' -Value $_pri`,
  // Call of Duty BO6 / Warzone
  CodHighPriority: `$ifeo = 'HKLM:\\SOFTWARE\\Microsoft\\Windows NT\\CurrentVersion\\Image File Execution Options\\cod.exe\\PerfOptions'; If (!(Test-Path $ifeo)) { New-Item -Path $ifeo -Force | Out-Null }; Set-ItemProperty -Path $ifeo -Name 'CpuPriorityClass' -Value 3 -Type DWord -Force; Set-ItemProperty -Path $ifeo -Name 'IoPriority' -Value 2 -Type DWord -Force; Set-ItemProperty -Path $ifeo -Name 'PagePriority' -Value 5 -Type DWord -Force; Set-ItemProperty -Path $ifeo -Name 'DisableEnergyThrottling' -Value 1 -Type DWord -Force; Set-ItemProperty -Path $ifeo -Name 'EnableBoost' -Value 1 -Type DWord -Force; Set-ItemProperty -Path $ifeo -Name 'ForceForegroundBoost' -Value 1 -Type DWord -Force; Write-Host "[COD] cod.exe priority set to Above Normal so Windows, DWM, and Alt-Tab remain responsive while the game gets a performance boost." -ForegroundColor Green`,
  ...
  CodShaderCacheClear: `Write-Host "[COD] Shader caches were preserved. Clearing them during optimization causes black menus and a long first-launch shader rebuild; use the game's repair tools only when troubleshooting corruption." -ForegroundColor Yellow`,
  CodPagefileOptimize: `$cs = Get-CimInstance -ClassName Win32_ComputerSystem -ErrorAction Stop; $ramMB = [uint32][Math]::Max(1024, [int]($cs.TotalPhysicalMemory / 1MB)); $minMB = [uint32][Math]::Max(4096, [Math]::Min(16384, [int]($ramMB / 2))); $maxMB = [uint32][Math]::Max($minMB, [Math]::Min(32768, [int]$ramMB)); try { if ($cs.AutomaticManagedPagefile) { Set-CimInstance -InputObject $cs -Property @{ AutomaticManagedPagefile = $false } -ErrorAction Stop | Out-Null }; $pf = Get-CimInstance -ClassName Win32_PageFileSetting -ErrorAction SilentlyContinue | Where-Object { $_.Name -like 'C:*' } | Select-Object -First 1; if ($pf) { Set-CimInstance -InputObject $pf -Property @{ InitialSize = $minMB; MaximumSize = $maxMB } -ErrorAction Stop | Out-Null } else { New-CimInstance -Namespace root/cimv2 -ClassName Win32_PageFileSetting -Property @{ Name = 'C:\pagefile.sys'; InitialSize = $minMB; MaximumSize = $maxMB } -ErrorAction Stop | Out-Null }; $verify = Get-CimInstance -ClassName Win32_PageFileSetting -ErrorAction SilentlyContinue | Where-Object { $_.Name -like 'C:*' } | Select-Object -First 1; if ($verify -and ([uint32]$verify.InitialSize -ne $minMB -or [uint32]$verify.MaximumSize -ne $maxMB)) { throw "Windows did not verify the requested pagefile range." }; Write-Host "[COD] Pagefile set to $([int]($minMB/1024))GB-$([int]($maxMB/1024))GB based on installed RAM." -ForegroundColor Green } catch { Write-Host "[COD] Pagefile left Windows-managed because this firmware rejected a fixed range: $($_.Exception.Message)" -ForegroundColor Yellow }`,
  ...
  CodBattlenetOptimize: `Write-Host "[COD] Battle.net services were left running so online menus, inventory, and matchmaking can finish loading without interruption." -ForegroundColor Yellow`,
  ...
  CodGPUPriority: `$exes = @('cod.exe','ModernWarfare.exe','ModernWarfareII.exe','ModernWarfareIII.exe'); foreach ($exe in $exes) { $key = "HKLM:\\SOFTWARE\\Microsoft\\Windows NT\\CurrentVersion\\Image File Execution Options\\$exe\\PerfOptions"; If (!(Test-Path $key)) { New-Item $key -Force | Out-Null }; Remove-ItemProperty $key 'GPUPriority' -EA SilentlyContinue; Set-ItemProperty $key 'CpuPriorityBoost' 1 -Type DWord -Force; Set-ItemProperty $key 'DisableEnergyThrottling' 1 -Type DWord -Force; Set-ItemProperty $key 'ForceForegroundBoost' 1 -Type DWord -Force }; Write-Host "[COD] Removed the undocumented maximum GPU-priority override so DWM, Windows, and Alt-Tab can preempt the game normally." -ForegroundColor Green`,
  CodDefenderExclusion: `$codPaths = @('C:\\Program Files\\Call of Duty','C:\\Program Files (x86)\\Call of Duty','D:\\Call of Duty','E:\\Call of Duty','C:\\Program Files\\Battle.net Apps\\Call of Duty'); foreach ($p in $codPaths) { If (Test-Path -LiteralPath $p) { try{Add-MpPreference -ExclusionPath $p -EA SilentlyContinue}catch{}; Write-Host "[COD] Defender exclusion added: $p" -ForegroundColor Green } }; $steamPaths = @('C:\\Program Files (x86)\\Steam\\steamapps\\common','D:\\SteamLibrary\\steamapps\\common','E:\\SteamLibrary\\steamapps\\common'); foreach ($s in ($steamPaths | Where-Object { Test-Path -LiteralPath $_ })) { $full = Join-Path -Path $s -ChildPath 'Call of Duty Modern Warfare 2'; If (Test-Path -LiteralPath $full) { try{Add-MpPreference -ExclusionPath $full -EA SilentlyContinue}catch{}; Write-Host "[COD] Defender exclusion added: $full" -ForegroundColor Green } }; Write-Host "[COD] Defender exclusion applied — Defender was scanning COD pak files on every load causing 2-8s load time spikes and mid-game disk hitching" -ForegroundColor Cyan`,
  CodDirectXQueue: `$dxKey = 'HKCU:\\SOFTWARE\\Microsoft\\Direct3D'; If (!(Test-Path $dxKey)) { New-Item $dxKey -Force | Out-Null }; Set-ItemProperty $dxKey 'MaxFrameLatency' 2 -Type DWord -Force; $dwm = 'HKLM:\\SOFTWARE\\Microsoft\\Windows\\Dwm'; Remove-ItemProperty $dwm 'OverlayTestMode' -EA SilentlyContinue; $graphics = 'HKLM:\\SYSTEM\\CurrentControlSet\\Control\\GraphicsDrivers'; Remove-ItemProperty $graphics 'PlatformSupportMiracast' -EA SilentlyContinue; Write-Host "[COD] Restored normal DWM overlay and display scheduling; Direct3D frame latency is limited to 2 queued frames without blocking Alt-Tab." -ForegroundColor Green`,
  ...
  RustOcclusionOff: `$cfg = "$env:APPDATA\\Rust\\cfg\\client.cfg"; If (!(Test-Path $cfg)) { New-Item $cfg -ItemType File -Force | Out-Null }; $c = Get-Content $cfg -Raw -EA SilentlyContinue; If ($c -match 'occlusion\.base') { (Get-Content $cfg) -replace 'occlusion\.base\s+\S+','occlusion.base 0' | Set-Content $cfg } Else { Add-Content $cfg "occlusion.base 0" }; Write-Host "[Rust] occlusion.base 0 — reduces CPU occlusion culling overhead during complex scenes." -ForegroundColor Green`,
  RustDisableAniso: `$cfg = "$env:APPDATA\\Rust\\cfg\\client.cfg"; If (!(Test-Path $cfg)) { New-Item $cfg -ItemType File -Force | Out-Null }; $c = Get-Content $cfg -Raw -EA SilentlyContinue; If ($c -match 'graphics\.aniso') { (Get-Content $cfg) -replace 'graphics\.aniso\s+\S+','graphics.aniso 0' | Set-Content $cfg } Else { Add-Content $cfg "graphics.aniso 0" }; Write-Host "[Rust] graphics.aniso 0 — anisotropic filtering off, frees GPU texture sampling budget." -ForegroundColor Green`,
  RustNagleOff: `$tp = 'HKLM:\\SYSTEM\\CurrentControlSet\\Services\\Tcpip\\Parameters'; Set-ItemProperty $tp 'TcpNoDelay' 1 -Type DWord -Force -EA SilentlyContinue; Set-ItemProperty $tp 'TcpAckFrequency' 1 -Type DWord -Force -EA SilentlyContinue; Write-Host "[Rust] Nagle algorithm disabled — immediate TCP sends, reduced ping variance on Rust servers." -ForegroundColor Green`,
  ...
  RobloxNagleOff: `$tp = 'HKLM:\\SYSTEM\\CurrentControlSet\\Services\\Tcpip\\Parameters'; Set-ItemProperty $tp 'TcpNoDelay' 1 -Type DWord -Force -EA SilentlyContinue; Set-ItemProperty $tp 'TcpAckFrequency' 1 -Type DWord -Force -EA SilentlyContinue; Write-Host "[Roblox] Nagle algorithm disabled — immediate TCP sends, reduced ping variance on Roblox servers." -ForegroundColor Green`,
  CodRawInput: `$dir = "$env:USERPROFILE\\Documents\\Call of Duty\\players"; New-Item $dir -ItemType Directory -Force -EA SilentlyContinue | Out-Null; $cfg = "$dir\\adv_options.ini"; If (!(Test-Path $cfg)) { @('[INPUT]','raw_mouse_input = true','mouse_filter = 0','mouse_deflection = 0.0') | Set-Content $cfg -Encoding UTF8 } Else { $c = Get-Content $cfg -Raw; If ($c -notmatch 'raw_mouse_input') { Add-Content $cfg ([Environment]::NewLine + 'raw_mouse_input = true') } }; Write-Host "[COD] Raw mouse input enabled in adv_options.ini — bypasses Windows mouse acceleration stack for 1:1 aim tracking." -ForegroundColor Green`,
  CodFramePacing: `$dxgiU='HKCU:\\SOFTWARE\\Microsoft\\DXGI'; If(!(Test-Path $dxgiU)){New-Item $dxgiU -Force|Out-Null}; Remove-ItemProperty $dxgiU 'WaitableObjectsThreshold' -EA SilentlyContinue; Set-ItemProperty $dxgiU 'MaximumFrameLatency' 2 -Type DWord -Force -EA SilentlyContinue; $dxgiM='HKLM:\\SOFTWARE\\Microsoft\\DXGI'; Remove-ItemProperty $dxgiM 'UseFlipModel' -EA SilentlyContinue; Remove-ItemProperty $dxgiM 'WaitableObject' -EA SilentlyContinue; $d3d='HKCU:\\SOFTWARE\\Microsoft\\Direct3D'; If(!(Test-Path $d3d)){New-Item $d3d -Force|Out-Null}; Set-ItemProperty $d3d 'MaxFrameLatency' 2 -Type DWord -Force -EA SilentlyContinue; Write-Host "[COD Frame Pacing] Removed undocumented queue overrides and restored a 2-frame latency target to avoid blocking menu rendering and Alt-Tab." -ForegroundColor Green`,
  CodMemPriority: `$ifeo='HKLM:\\SOFTWARE\\Microsoft\\Windows NT\\CurrentVersion\\Image File Execution Options'; @('cod.exe','ModernWarfare.exe','ModernWarfareII.exe','ModernWarfareIII.exe','BlackOps6.exe','warzone.exe') | ForEach-Object { $k="$ifeo\\$_\\PerfOptions"; If(!(Test-Path $k)){New-Item $k -Force|Out-Null}; Set-ItemProperty $k 'PagePriority' 5 -Type DWord -Force -EA SilentlyContinue; Set-ItemProperty $k 'WorkingSetPolicy' 1 -Type DWord -Force -EA SilentlyContinue; Write-Host "[COD] Memory priority=High, WorkingSet=Locked: $_" -ForegroundColor Green }; Write-Host "[COD Memory] PagePriority=5 (highest) applied to all COD executables — prevents Windows from paging out COD texture buffers during background bursts. On 32GB RAM rigs this keeps the full texture streaming cache in physical RAM, eliminating the 200-500ms stutter when BO6 re-fetches paged-out map assets." -ForegroundColor Green`,

  // ── Intel 4th–8th Gen (Haswell through Coffee Lake) power optimization ────────
  IntelOldGenPowerOpt: `# Activate High Performance plan (GUID 8c5e7fda = High Perf built-in)
...
Write-Host "[Intel 4th-8th Gen] High Performance plan active. Min/Max=100%, BoostMode=Aggressive, PowerThrottling disabled. All cores run at rated Turbo frequency throughout your gaming session — no ramp-up delay on first frame." -ForegroundColor Green`,
  ...
  // Rust Game Optimizer
  RustHighPriority: `$_c=(Get-CimInstance Win32_Processor -Property NumberOfCores|Measure-Object NumberOfCores -Sum).Sum; $_pri=if($_c -ge 6){4}else{3}; $key = 'HKLM:\\SOFTWARE\\Microsoft\\Windows NT\\CurrentVersion\\Image File Execution Options\\RustClient.exe\\PerfOptions'; If (!(Test-Path $key)) { New-Item -Path $key -Force | Out-Null }; Set-ItemProperty -Path $key -Name 'CpuPriorityClass' -Value $_pri -Type DWord -Force; Set-ItemProperty -Path $key -Name 'IoPriority' -Value 3 -Type DWord -Force; Write-Host "[OK] Rust: CPU=$(if($_pri-eq 4){'High'}else{'AboveNormal'}) + High I/O applied (persists across reboots)" -ForegroundColor Green`,
  ...
  // Roblox Optimizer
  RobloxHighPriority: `$_c=(Get-CimInstance Win32_Processor -Property NumberOfCores|Measure-Object NumberOfCores -Sum).Sum; $_pri=if($_c -ge 6){4}else{3}; $key = 'HKLM:\\SOFTWARE\\Microsoft\\Windows NT\\CurrentVersion\\Image File Execution Options\\RobloxPlayerBeta.exe\\PerfOptions'; If (!(Test-Path $key)) { New-Item -Path $key -Force | Out-Null }; Set-ItemProperty -Path $key -Name 'CpuPriorityClass' -Value $_pri -Type DWord -Force; Set-ItemProperty -Path $key -Name 'IoPriority' -Value 3 -Type DWord -Force; Write-Host "[OK] Roblox: CPU=$(if($_pri-eq 4){'High'}else{'AboveNormal'}) + High I/O applied" -ForegroundColor Green`,
  ...
  game_overwatch: `$paths = @("C:\\Program Files (x86)\\Overwatch\\_retail_\\Overwatch.exe","C:\\Program Files\\Overwatch\\_retail_\\Overwatch.exe"); $found = $paths | Where-Object { Test-Path $_ } | Select-Object -First 1; If ($found) { Write-Host "[DETECTED] Overwatch 2" -ForegroundColor Green; $_c=(Get-CimInstance Win32_Processor -Property NumberOfCores|Measure-Object NumberOfCores -Sum).Sum; $_pri=if($_c -ge 6){4}else{3}; $key = 'HKLM:\\SOFTWARE\\Microsoft\\Windows NT\\CurrentVersion\\Image File Execution Options\\Overwatch.exe\\PerfOptions'; If (!(Test-Path $key)) { New-Item -Path $key -Force | Out-Null }; Set-ItemProperty -Path $key -Name 'CpuPriorityClass' -Value $_pri; Write-Host "[OK] Overwatch 2: CPU=$(if($_pri-eq 4){'High'}else{'AboveNormal'}) priority" -ForegroundColor Green } Else { Write-Host "[SKIP] Overwatch 2 not detected" -ForegroundColor DarkGray }`,
  game_siege: `$paths = @("C:\\Program Files (x86)\\Ubisoft\\Ubisoft Game Launcher\\games\\Tom Clancy's Rainbow Six Siege","C:\\Program Files\\Ubisoft\\Ubisoft Game Launcher\\games\\Tom Clancy's Rainbow Six Siege"); $found = $paths | Where-Object { Test-Path $_ } | Select-Object -First 1; If ($found) { Write-Host "[DETECTED] Rainbow Six Siege" -ForegroundColor Green; $_c=(Get-CimInstance Win32_Processor -Property NumberOfCores|Measure-Object NumberOfCores -Sum).Sum; $_pri=if($_c -ge 6){4}else{3}; $key = 'HKLM:\\SOFTWARE\\Microsoft\\Windows NT\\CurrentVersion\\Image File Execution Options\\RainbowSix.exe\\PerfOptions'; If (!(Test-Path $key)) { New-Item -Path $key -Force | Out-Null }; Set-ItemProperty -Path $key -Name 'CpuPriorityClass' -Value $_pri; Write-Host "[OK] R6 Siege: CPU=$(if($_pri-eq 4){'High'}else{'AboveNormal'}) priority" -ForegroundColor Green } Else { Write-Host "[SKIP] Rainbow Six Siege not detected" -ForegroundColor DarkGray }`,
  ...
  game_pubg: `$paths = @("C:\\Program Files (x86)\\Steam\\steamapps\\common\\PUBG\\TslGame\\Binaries\\Win64\\TslGame.exe","D:\\SteamLibrary\\steamapps\\common\\PUBG\\TslGame\\Binaries\\Win64\\TslGame.exe"); $found = $paths | Where-Object { Test-Path $_ } | Select-Object -First 1; If ($found) { Write-Host "[DETECTED] PUBG" -ForegroundColor Green; $_c=(Get-CimInstance Win32_Processor -Property NumberOfCores|Measure-Object NumberOfCores -Sum).Sum; $_pri=if($_c -ge 6){4}else{3}; $key = 'HKLM:\\SOFTWARE\\Microsoft\\Windows NT\\CurrentVersion\\Image File Execution Options\\TslGame.exe\\PerfOptions'; If (!(Test-Path $key)) { New-Item -Path $key -Force | Out-Null }; Set-ItemProperty -Path $key -Name 'CpuPriorityClass' -Value $_pri; Write-Host "[OK] PUBG: CPU=$(if($_pri-eq 4){'High'}else{'AboveNormal'}) priority" -ForegroundColor Green } Else { Write-Host "[SKIP] PUBG not detected" -ForegroundColor DarkGray }`,
  ...
  game_dota2: `$paths = @("C:\\Program Files (x86)\\Steam\\steamapps\\common\\dota 2 beta","D:\\SteamLibrary\\steamapps\\common\\dota 2 beta"); $found = $paths | Where-Object { Test-Path $_ } | Select-Object -First 1; If ($found) { Write-Host "[DETECTED] Dota 2" -ForegroundColor Green; $_c=(Get-CimInstance Win32_Processor -Property NumberOfCores|Measure-Object NumberOfCores -Sum).Sum; $_pri=if($_c -ge 6){4}else{3}; $key = 'HKLM:\\SOFTWARE\\Microsoft\\Windows NT\\CurrentVersion\\Image File Execution Options\\dota2.exe\\PerfOptions'; If (!(Test-Path $key)) { New-Item -Path $key -Force | Out-Null }; Set-ItemProperty -Path $key -Name 'CpuPriorityClass' -Value $_pri; Write-Host "[OK] Dota 2: CPU=$(if($_pri-eq 4){'High'}else{'AboveNormal'}) priority" -ForegroundColor Green } Else { Write-Host "[SKIP] Dota 2 not detected" -ForegroundColor DarkGray }`,
  // Startup apps (previously missing)
  su_steam: `$steamKeys = @("Steam","steam"); foreach ($v in $steamKeys) { reg delete "HKCU\\SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\Run" /v $v /f 2>$null }; $lnks = @("$env:APPDATA\\Microsoft\\Windows\\Start Menu\\Programs\\Startup\\Steam.lnk","$env:USERPROFILE\\AppData\\Roaming\\Microsoft\\Windows\\Start Menu\\Programs\\Startup\\Steam.lnk"); foreach ($lnk in $lnks) { If (Test-Path $lnk) { Remove-Item $lnk -Force } }; $saPath = "HKCU:\\SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\Explorer\\StartupApproved\\Run"; if (!(Test-Path $saPath)) { New-Item $saPath -Force | Out-Null }; New-ItemProperty -Path $saPath -Name "Steam" -Value ([byte[]](0x03,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00)) -PropertyType Binary -Force -EA SilentlyContinue | Out-Null; Get-ScheduledTask | Where-Object { $_.TaskName -like "*Steam*" } | Disable-ScheduledTask -EA SilentlyContinue; Write-Host "[OK] Steam removed from ALL startup locations (registry, StartupApproved, .lnk, scheduled tasks)" -ForegroundColor Green`,
  ...
  NvidiaMaxPerfMode: `Set-ItemProperty -Path 'HKLM:\\SYSTEM\\CurrentControlSet\\Control\\GraphicsDrivers' -Name 'PlatformSupportMiracast' -Value 0 -Type DWord -Force -EA SilentlyContinue; Set-ItemProperty -Path 'HKLM:\\SYSTEM\\CurrentControlSet\\Control\\GraphicsDrivers' -Name 'TdrLevel' -Value 3 -Type DWord -Force -EA SilentlyContinue; Write-Host "[NVIDIA] Max performance mode hints applied via GraphicsDrivers registry" -ForegroundColor Green`,
  ...
  NvTexHighQuality: `@('HKLM:\\SOFTWARE\\NVIDIA Corporation\\Global\\NVTweak','HKCU:\\SOFTWARE\\NVIDIA Corporation\\Global\\NVTweak') | ForEach-Object { If (!(Test-Path $_)) { New-Item $_ -Force | Out-Null }; Set-ItemProperty -Path $_ -Name 'PS_TexFilterQuality' -Value 2 -Type DWord -Force -EA SilentlyContinue; Set-ItemProperty -Path $_ -Name 'PS_TexFilterAnisoOptOn' -Value 0 -Type DWord -Force -EA SilentlyContinue; Set-ItemProperty -Path $_ -Name 'PS_TexFilterLODBiasAllow' -Value 1 -Type DWord -Force -EA SilentlyContinue; Set-ItemProperty -Path $_ -Name 'PS_TexFilterNoNeg' -Value 0 -Type DWord -Force -EA SilentlyContinue }; $gpuClass = 'HKLM:\\SYSTEM\\CurrentControlSet\\Control\\Class\\{4d36e968-e325-11ce-bfc1-08002be10318}'; $found=$false; 0,1,2,3 | ForEach-Object { $k = "$gpuClass\\000$_"; If ((Test-Path $k) -and (Get-ItemProperty $k -Name 'DriverDesc' -EA SilentlyContinue).DriverDesc -match 'NVIDIA') { Set-ItemProperty $k 'TextureFilterQuality' 2 -Type DWord -Force -EA SilentlyContinue; Set-ItemProperty $k 'ForcedMipmapsMinLod' 0 -Type DWord -Force -EA SilentlyContinue; Write-Host "[NVIDIA] Texture Filtering = High Quality (PS_TexFilterQuality=2) — fixes mushy face/arm textures in FiveM/GTA V" -ForegroundColor Green; $found=$true } }; If (-not $found) { Write-Host "[NVIDIA] GPU class key not found — open NVCP > Manage 3D Settings > Texture filtering quality = High quality" -ForegroundColor Yellow }`,
  AmdTexHighQuality: `$gpuClass = 'HKLM:\\SYSTEM\\CurrentControlSet\\Control\\Class\\{4d36e968-e325-11ce-bfc1-08002be10318}'; $found=$false; Get-ChildItem $gpuClass -EA SilentlyContinue | Where-Object { (Get-ItemProperty $_.PSPath -Name 'DriverDesc' -EA SilentlyContinue).DriverDesc -match 'AMD|Radeon' } | ForEach-Object { Set-ItemProperty $_.PSPath -Name 'TFQ' -Value 2 -Type DWord -Force -EA SilentlyContinue; Set-ItemProperty $_.PSPath -Name 'CatalystAI' -Value 1 -Type DWord -Force -EA SilentlyContinue; Set-ItemProperty $_.PSPath -Name 'TextureOpt' -Value 0 -Type DWord -Force -EA SilentlyContinue; Write-Host "[AMD] Texture Filtering Quality = High Quality (TFQ=2, CatalystAI=1) — fixes mushy face/arm textures in FiveM/GTA V" -ForegroundColor Green; $found=$true }; If (-not $found) { Write-Host "[AMD] AMD GPU class key not found — open Radeon Software > Gaming > Graphics > Texture Filtering Quality = High" -ForegroundColor Yellow }`,
  ...
  AmdCpuCStatePolicy: `$settingPath = 'HKLM:\\SYSTEM\\CurrentControlSet\\Control\\Power\\PowerSettings\\54533251-82be-4824-96c1-47b60b740d00\\40fbefc7-2e9d-4d25-a185-0cfd8574bae6'; if (!(Test-Path -LiteralPath $settingPath)) { Write-Host "[SKIP] AMD CPU performance decrease policy is not exposed by Windows." -ForegroundColor Yellow; return }; $schemeText = (& powercfg.exe /getactivescheme 2>&1 | Out-String); if ($schemeText -notmatch '([0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12})') { Write-Host "[SKIP] Windows did not return an active power scheme." -ForegroundColor Yellow; return }; $g = $matches[1]; & powercfg.exe /setacvalueindex $g 54533251-82be-4824-96c1-47b60b740d00 40fbefc7-2e9d-4d25-a185-0cfd8574bae6 0 2>&1 | Out-Null; if ($LASTEXITCODE -ne 0) { Write-Host "[SKIP] AMD CPU performance decrease policy is not supported by the active power plan." -ForegroundColor Yellow; return }; & powercfg.exe /setactive $g 2>&1 | Out-Null; if ($LASTEXITCODE -ne 0) { Write-Host "[SKIP] Windows could not reactivate the power plan after checking the AMD CPU policy." -ForegroundColor Yellow; return }; Set-ItemProperty -LiteralPath $settingPath -Name 'Attributes' -Value 2 -Type DWord -Force; Write-Host "[AMD CPU] CPU performance decrease policy set to Fastest and verified." -ForegroundColor Green`,
  ...
  // DirectX & PCIe — T003 additions (GPU render queue, DirectX tweaks, interrupt affinity)
  NvidiaD3DOptimize: `$d3dPaths = @('HKLM:\\SOFTWARE\\Microsoft\\Direct3D','HKCU:\\SOFTWARE\\Microsoft\\Direct3D'); foreach ($p in $d3dPaths) { if (!(Test-Path $p)) { New-Item $p -Force | Out-Null }; Set-ItemProperty $p -Name 'ForceDebugRuntime' -Value 0 -Type DWord -Force -EA SilentlyContinue; Set-ItemProperty $p -Name 'LoadDebugRuntime' -Value 0 -Type DWord -Force -EA SilentlyContinue; Set-ItemProperty $p -Name 'EnableAsyncShaderCompilation' -Value 1 -Type DWord -Force -EA SilentlyContinue }; $dxPath = 'HKLM:\\SOFTWARE\\Microsoft\\DirectX'; if (!(Test-Path $dxPath)) { New-Item $dxPath -Force | Out-Null }; Set-ItemProperty $dxPath -Name 'D3D12_ENABLE_UNSAFE_COMMAND_BUFFER_REUSE' -Value 0 -Type DWord -Force -EA SilentlyContinue; Write-Host "[NVIDIA DirectX] D3D debug layers OFF + async shader compile ON — compile-stall stutters eliminated in COD/FiveM/Fortnite on GTX 1650 Super" -ForegroundColor Green`,
  NvidiaPCIeGen3Force: `$gpuClass = 'HKLM:\\SYSTEM\\CurrentControlSet\\Control\\Class\\{4d36e968-e325-11ce-bfc1-08002be10318}'; Get-ChildItem $gpuClass -EA SilentlyContinue | Where-Object { (Get-ItemProperty $_.PSPath -Name 'DriverDesc' -EA SilentlyContinue).DriverDesc -match 'NVIDIA|GeForce' } | ForEach-Object { Set-ItemProperty $_.PSPath -Name 'EnableMsHybrid' -Value 0 -Type DWord -Force -EA SilentlyContinue; Set-ItemProperty $_.PSPath -Name 'PCIELinkSpeedOverride' -Value 2 -Type DWord -Force -EA SilentlyContinue }; Set-ItemProperty -Path 'HKLM:\\SYSTEM\\CurrentControlSet\\Control\\GraphicsDrivers' -Name 'EnablePreemption' -Value 1 -Type DWord -Force -EA SilentlyContinue; Write-Host "[NVIDIA PCIe] PCIe Gen3 link locked + GPU preemption enabled — prevents bandwidth fallback to PCIe 2.0 x8 under power transitions (GTX 1650 Super)" -ForegroundColor Green`,
  NvidiaInterruptAffinity: `$pciRoot = 'HKLM:\\SYSTEM\\CurrentControlSet\\Enum\\PCI'; $gpuKey = Get-ChildItem $pciRoot -EA SilentlyContinue | Get-ChildItem -EA SilentlyContinue | Where-Object { try { (Get-ItemProperty $_.PSPath -EA Stop).Service -match 'nvlddmkm' } catch { $false } } | Select-Object -First 1; if ($gpuKey) { $affPath = "$($gpuKey.PSPath)\\Device Parameters\\Interrupt Management\\Affinity Policy"; if (!(Test-Path $affPath)) { New-Item $affPath -Force | Out-Null }; Set-ItemProperty $affPath -Name 'AssignmentPolicy' -Value 1 -Type DWord -Force -EA SilentlyContinue; Set-ItemProperty $affPath -Name 'TargetedProcessors' -Value 4 -Type DWord -Force -EA SilentlyContinue; Write-Host "[NVIDIA Interrupt] GPU DPC routed to CPU core 2 (mask 0x4) — reduces interrupt contention with Ryzen scheduler on core 0" -ForegroundColor Green } Else { Write-Host "[NVIDIA Interrupt] No nvlddmkm device found — run as admin post-driver install" -ForegroundColor Yellow }`,
  AmdD3DOptimize: `$d3dPaths = @('HKLM:\\SOFTWARE\\Microsoft\\Direct3D','HKCU:\\SOFTWARE\\Microsoft\\Direct3D'); foreach ($p in $d3dPaths) { if (!(Test-Path $p)) { New-Item $p -Force | Out-Null }; Set-ItemProperty $p -Name 'ForceDebugRuntime' -Value 0 -Type DWord -Force -EA SilentlyContinue; Set-ItemProperty $p -Name 'LoadDebugRuntime' -Value 0 -Type DWord -Force -EA SilentlyContinue; Set-ItemProperty $p -Name 'EnableAsyncShaderCompilation' -Value 1 -Type DWord -Force -EA SilentlyContinue }; $dxgiPath = 'HKCU:\\SOFTWARE\\Microsoft\\DXGI'; if (!(Test-Path $dxgiPath)) { New-Item $dxgiPath -Force | Out-Null }; Set-ItemProperty $dxgiPath -Name 'WaitableObjectsThreshold' -Value 1 -Type DWord -Force -EA SilentlyContinue; Write-Host "[AMD DirectX] D3D debug layers OFF + async shader ON + DXGI waitable threshold=1 — frame pacing stabilized on RDNA1/2/Polaris" -ForegroundColor Green`,
  AmdPCIeOptimize: `$amdClass = 'HKLM:\\SYSTEM\\CurrentControlSet\\Control\\Class\\{4d36e968-e325-11ce-bfc1-08002be10318}'; Get-ChildItem $amdClass -EA SilentlyContinue | Where-Object { (Get-ItemProperty $_.PSPath -Name 'DriverDesc' -EA SilentlyContinue).DriverDesc -match 'AMD|Radeon|Advanced Micro' } | ForEach-Object { Set-ItemProperty $_.PSPath -Name 'KMD_EnableAsyncCompute' -Value 1 -Type DWord -Force -EA SilentlyContinue; Set-ItemProperty $_.PSPath -Name 'KMD_FRTCEnabled' -Value 0 -Type DWord -Force -EA SilentlyContinue }; Set-ItemProperty -Path 'HKLM:\\SYSTEM\\CurrentControlSet\\Control\\GraphicsDrivers' -Name 'EnablePreemption' -Value 1 -Type DWord -Force -EA SilentlyContinue; Write-Host "[AMD PCIe/Compute] Async compute ON + FRTC driver flag cleared + GPU preemption enabled — Radeon async compute dispatch optimized" -ForegroundColor Green`,
  ...
  AmdFRTC60:         `$gpuClass = 'HKLM:\\SYSTEM\\CurrentControlSet\\Control\\Class\\{4d36e968-e325-11ce-bfc1-08002be10318}'; Get-ChildItem $gpuClass -EA SilentlyContinue | Where-Object { (Get-ItemProperty $_.PSPath -Name 'DriverDesc' -EA SilentlyContinue).DriverDesc -match 'AMD|Radeon' } | ForEach-Object { Set-ItemProperty $_.PSPath -Name 'KMD_FRTCEnabled' -Value 1 -Type DWord -Force -EA SilentlyContinue; Set-ItemProperty $_.PSPath -Name 'KMD_FRTCMaxFPS' -Value 60 -Type DWord -Force -EA SilentlyContinue }; Write-Host "[AMD] Frame Rate Target Control = 60 FPS (FRTC driver cap enabled)" -ForegroundColor Green`,
  AmdFRTC144:        `$gpuClass = 'HKLM:\\SYSTEM\\CurrentControlSet\\Control\\Class\\{4d36e968-e325-11ce-bfc1-08002be10318}'; Get-ChildItem $gpuClass -EA SilentlyContinue | Where-Object { (Get-ItemProperty $_.PSPath -Name 'DriverDesc' -EA SilentlyContinue).DriverDesc -match 'AMD|Radeon' } | ForEach-Object { Set-ItemProperty $_.PSPath -Name 'KMD_FRTCEnabled' -Value 1 -Type DWord -Force -EA SilentlyContinue; Set-ItemProperty $_.PSPath -Name 'KMD_FRTCMaxFPS' -Value 144 -Type DWord -Force -EA SilentlyContinue }; Write-Host "[AMD] Frame Rate Target Control = 144 FPS (FRTC driver cap enabled)" -ForegroundColor Green`,
  AmdFRTC240:        `$gpuClass = 'HKLM:\\SYSTEM\\CurrentControlSet\\Control\\Class\\{4d36e968-e325-11ce-bfc1-08002be10318}'; Get-ChildItem $gpuClass -EA SilentlyContinue | Where-Object { (Get-ItemProperty $_.PSPath -Name 'DriverDesc' -EA SilentlyContinue).DriverDesc -match 'AMD|Radeon' } | ForEach-Object { Set-ItemProperty $_.PSPath -Name 'KMD_FRTCEnabled' -Value 1 -Type DWord -Force -EA SilentlyContinue; Set-ItemProperty $_.PSPath -Name 'KMD_FRTCMaxFPS' -Value 240 -Type DWord -Force -EA SilentlyContinue }; Write-Host "[AMD] Frame Rate Target Control = 240 FPS (FRTC driver cap enabled)" -ForegroundColor Green`,
  ...
  FiveM1650HAGSOffPack: `Write-Host "[HAGS OFF Pack] Applying DXGI + render pipeline fixes for HAGS-disabled GTX 1650 Super..." -ForegroundColor Cyan; $dxgi = 'HKCU:\\SOFTWARE\\Microsoft\\DXGI'; If (!(Test-Path $dxgi)) { New-Item $dxgi -Force | Out-Null }; Set-ItemProperty $dxgi 'MaxFrameLatency' 1 -Type DWord -Force; Set-ItemProperty $dxgi 'AllowTearing' 1 -Type DWord -Force; $dx = 'HKCU:\\SOFTWARE\\Microsoft\\Direct3D'; If (!(Test-Path $dx)) { New-Item $dx -Force | Out-Null }; Set-ItemProperty $dx 'MaxFrameLatency' 1 -Type DWord -Force; $ifeo = 'HKLM:\\SOFTWARE\\Microsoft\\Windows NT\\CurrentVersion\\Image File Execution Options'; @('FiveM.exe','GTA5.exe','FiveM_b2189_GTAProcess.exe','FiveM_b2545_GTAProcess.exe','FiveM_b2612_GTAProcess.exe','FiveM_b2699_GTAProcess.exe','FiveM_b2802_GTAProcess.exe','FiveM_b2944_GTAProcess.exe','FiveM_b3095_GTAProcess.exe','FiveM_b3258_GTAProcess.exe','FiveM_b3323_GTAProcess.exe','FiveM_b3407_GTAProcess.exe','FiveM_b3441_GTAProcess.exe') | ForEach-Object { $p = "$ifeo\\$_\\PerfOptions"; If (!(Test-Path $p)) { New-Item $p -Force | Out-Null }; Set-ItemProperty $p 'RenderThrottlingOff' 1 -Type DWord -Force; Set-ItemProperty $p 'GpuIdleEnabled' 0 -Type DWord -Force; Set-ItemProperty $p 'PowerSavingVsyncOn' 0 -Type DWord -Force; Write-Host "  [OK] Render throttle off on $_" -ForegroundColor Green }; $games = 'HKLM:\\SOFTWARE\\Microsoft\\Windows NT\\CurrentVersion\\Multimedia\\SystemProfile\\Tasks\\Games'; If (!(Test-Path $games)) { New-Item $games -Force | Out-Null }; Set-ItemProperty $games 'MaximumPreRenderedFrames' 1 -Type DWord -Force; Set-ItemProperty $games 'GPU Priority' 8 -Type DWord -Force; Write-Host "[HAGS OFF Pack] DXGI MaxFrameLatency=1, AllowTearing=1, render throttle off, pre-rendered frames=1. Eliminates the 160->60 FPS drop pattern on GTX 1650 Super with HAGS disabled." -ForegroundColor Green`,

  // ── FiveM: GTX 2060 specific tweaks ──────────────────────────────────────────
  FiveM2060VRAMBudget: `$dir = "$env:USERPROFILE\\Documents\\Rockstar Games\\GTA V"; If (!(Test-Path $dir)) { New-Item $dir -ItemType Directory -Force | Out-Null }; $cmd = "$dir\\commandline.txt"; $existing = If (Test-Path $cmd) { Get-Content $cmd -Raw } Else { "" }; If ($existing -notmatch 'availablevidmem') { $existing = $existing.Trim() + " -availablevidmem 6144" }; If ($existing -notmatch 'percentvidmem') { $existing = $existing.Trim() + " -percentvidmem 100" }; Set-Content $cmd $existing.Trim(); Write-Host "[GTX 2060] commandline.txt patched: -availablevidmem 6144 -percentvidmem 100. GTA V now budgets the full 6GB GDDR6 VRAM — eliminates under-reporting that causes texture pop on high-asset FiveM servers." -ForegroundColor Green`,
  ...
  FiveM3500PerfPlan: `$scheme = '8c5e7fda-e8bf-4a96-9a85-a6e23a8c635c'; & powercfg.exe /setactive $scheme 2>&1 | Out-Null; if ($LASTEXITCODE -ne 0) { $scheme = 'SCHEME_CURRENT' }; $settings = @(@('PROCTHROTTLEMIN','100'),@('PROCTHROTTLEMAX','100'),@('PERFBOOSTMODE','2'),@('PERFBOOSTPOL','100'),@('CPMINCORES','100')); $missing = @(); foreach ($item in $settings) { & powercfg.exe /query $scheme SUB_PROCESSOR $item[0] 2>&1 | Out-Null; if ($LASTEXITCODE -ne 0) { $missing += $item[0] } }; if ($missing.Count -gt 0) { Write-Host "[SKIP] Ryzen 5 3500 power-plan settings are unavailable: $($missing -join ', ')." -ForegroundColor Yellow; return }; $failed = @(); foreach ($item in $settings) { & powercfg.exe /setacvalueindex $scheme SUB_PROCESSOR $item[0] $item[1] 2>&1 | Out-Null; if ($LASTEXITCODE -ne 0) { $failed += $item[0] } }; if ($failed.Count -gt 0) { Write-Host "[SKIP] Ryzen 5 3500 power-plan settings could not be applied: $($failed -join ', ')." -ForegroundColor Yellow; return }; & powercfg.exe /setactive $scheme 2>&1 | Out-Null; if ($LASTEXITCODE -ne 0) { Write-Host "[SKIP] Windows could not activate the selected Ryzen 5 3500 power plan." -ForegroundColor Yellow; return }; Write-Host "[R5 3500] High Performance plan: Min=100%, Max=100%, BoostMode=Aggressive, BoostPolicy=100%. Precision Boost 2 runs at max clocks for all 6 cores for the entire FiveM session." -ForegroundColor Green`,

  // ── FiveM: Intel i5-4XXX (Haswell 4-core, no HT) specific tweaks ─────────────
  FiveMi5CoreAffinity: `$mask = 0xF; @('GTA5','FiveM') | ForEach-Object { $p = Get-Process $_ -EA SilentlyContinue; If ($p) { $p.ProcessorAffinity = $mask; Write-Host "[i5-4XXX] Live affinity set to 0xF (all 4 physical cores) for $_" -ForegroundColor Green } Else { Write-Host "[i5-4XXX] $_ not running — IFEO applies on next launch" -ForegroundColor DarkGray } }; $ifeo = 'HKLM:\\SOFTWARE\\Microsoft\\Windows NT\\CurrentVersion\\Image File Execution Options'; @('GTA5.exe','FiveM.exe') | ForEach-Object { $k = "$ifeo\\$_\\PerfOptions"; If (!(Test-Path $k)) { New-Item $k -Force | Out-Null }; Set-ItemProperty $k 'CpuPriorityClass' 3 -Type DWord -Force; Set-ItemProperty $k 'IoPriority' 3 -Type DWord -Force; Set-ItemProperty $k 'ForceForegroundBoost' 1 -Type DWord -Force; Set-ItemProperty $k 'DisableEnergyThrottling' 1 -Type DWord -Force }; Write-Host "[i5-4XXX] Haswell has 4 physical cores, no HT — 0xF is ALL cores. IFEO: CpuPriorityClass=High, IO=High, FgBoost=On, EnergyThrottle=Off. With only 4 real threads, pinning the game here prevents background tasks from stealing a core mid-gunfight." -ForegroundColor Cyan`,
  FiveMIntel14PcoreAffinity: `$mask = 0xFFF; @('GTA5','FiveM') | ForEach-Object { $p = Get-Process $_ -EA SilentlyContinue; If ($p) { $p.ProcessorAffinity = $mask; Write-Host "[i5-14XXX] Live affinity set to P-cores only (0xFFF, threads 0-11) for $_" -ForegroundColor Green } Else { Write-Host "[i5-14XXX] $_ not running — IFEO applies on next launch" -ForegroundColor DarkGray } }; $ifeo = 'HKLM:\\SOFTWARE\\Microsoft\\Windows NT\\CurrentVersion\\Image File Execution Options'; @('GTA5.exe','FiveM.exe') | ForEach-Object { $k = "$ifeo\\$_\\PerfOptions"; If (!(Test-Path $k)) { New-Item $k -Force | Out-Null }; Set-ItemProperty $k 'CpuPriorityClass' 3 -Type DWord -Force; Set-ItemProperty $k 'IoPriority' 3 -Type DWord -Force; Set-ItemProperty $k 'ForceForegroundBoost' 1 -Type DWord -Force; Set-ItemProperty $k 'DisableEnergyThrottling' 1 -Type DWord -Force }; Write-Host "[i5-14XXX] Intel 14th gen hybrid: mask 0xFFF pins GTA5 + FiveM to P-cores (threads 0-11). E-cores (threads 12-15) are excluded. GTA V render + physics threads run best on P-cores — this eliminates the frametime variance from landing on a slower E-core thread between frames. Persists via IFEO." -ForegroundColor Cyan`,
  FiveMIntel14PowerPlan: `$ultra = 'e9a42b02-d5df-448d-aa00-03f14749eb61'; $high = '8c5e7fda-e8bf-4a96-9a85-a6e23a8c635c'; powercfg /setactive $ultra 2>nul; If ($LASTEXITCODE -ne 0) { powercfg /setactive $high 2>nul; Write-Host "[i5-14XXX] Ultra Performance plan not found — using High Performance" -ForegroundColor Yellow } Else { Write-Host "[i5-14XXX] Ultra Performance plan activated" -ForegroundColor Green }; powercfg /setacvalueindex SCHEME_CURRENT SUB_PROCESSOR PROCTHROTTLEMIN 100 2>nul; powercfg /setacvalueindex SCHEME_CURRENT SUB_PROCESSOR PROCTHROTTLEMAX 100 2>nul; powercfg /setacvalueindex SCHEME_CURRENT SUB_PROCESSOR PERFBOOSTMODE 2 2>nul; powercfg /setacvalueindex SCHEME_CURRENT SUB_PROCESSOR PERFBOOSTPOL 100 2>nul; powercfg /setacvalueindex SCHEME_CURRENT SUB_PROCESSOR CPMINCORES 100 2>nul; powercfg /setacvalueindex SCHEME_CURRENT SUB_PROCESSOR HETEROPOLICY 1 2>nul; powercfg /setactive SCHEME_CURRENT 2>nul; Write-Host "[i5-14XXX] CPU plan: Min=100%, Max=100%, Boost=Aggressive, HeteroPolicy=1 (P-core preferred). Intel Thread Director now aggressively routes foreground threads to P-cores. Prevents the governor from dropping P-core clocks between frames — the main cause of irregular frametimes on hybrid Intel CPUs." -ForegroundColor Cyan`,
  FiveM5060VRAMBudget: `$dir = "$env:USERPROFILE\\Documents\\Rockstar Games\\GTA V"; If (!(Test-Path $dir)) { New-Item $dir -ItemType Directory -Force | Out-Null }; $cmd = "$dir\\commandline.txt"; $existing = If (Test-Path $cmd) { Get-Content $cmd -Raw } Else { "" }; If ($existing -notmatch 'availablevidmem') { $existing = $existing.Trim() + " -availablevidmem 8192" }; If ($existing -notmatch 'percentvidmem') { $existing = $existing.Trim() + " -percentvidmem 100" }; Set-Content $cmd $existing.Trim(); Write-Host "[RTX 5060] commandline.txt: -availablevidmem 8192 -percentvidmem 100. Forces GTA V to budget the full 8GB GDDR7 VRAM — eliminates the VRAM under-reporting that causes texture pop-in and hitching on high-asset FiveM servers. Blackwell VRAM management is more aggressive than Turing/Ampere — setting the budget upfront prevents mid-session texture unloads." -ForegroundColor Green`,
  FiveM5060EnableHAGS: `$hwsched = 'HKLM:\\SYSTEM\\CurrentControlSet\\Control\\GraphicsDrivers'; If (!(Test-Path $hwsched)) { New-Item $hwsched -Force | Out-Null }; Set-ItemProperty $hwsched 'HwSchMode' 2 -Type DWord -Force; Write-Host "[RTX 5060] HAGS enabled (HwSchMode=2). Blackwell architecture is fully HAGS-optimized — the GPU manages its own DMA work queue without CPU intervention, reducing render latency compared to CPU-scheduled DMA on older cards. Reboot required to take effect." -ForegroundColor Green`,
  FiveM5060LowLatency: `$nv = 'HKLM:\\SYSTEM\\CurrentControlSet\\Control\\Class\\{4d36e968-e325-11ce-bfc1-08002be10318}'; $adapters = Get-ChildItem $nv -EA SilentlyContinue | Where-Object { (Get-ItemProperty $_.PSPath 'DriverDesc' -EA SilentlyContinue).DriverDesc -match 'RTX|NVIDIA' }; $adapters | ForEach-Object { Set-ItemProperty $_.PSPath 'RmLowLatencyMode' 2 -Type DWord -Force; Set-ItemProperty $_.PSPath 'FlipQueueSize' 1 -Type DWord -Force; Set-ItemProperty $_.PSPath 'PerfLevelSrc' 0x2222 -Type DWord -Force; Write-Host "[RTX 5060] $((Get-ItemProperty $_.PSPath 'DriverDesc' -EA SilentlyContinue).DriverDesc): RmLowLatencyMode=Ultra(2), FlipQueueSize=1, PerfLevelSrc=MaxPerf" -ForegroundColor Green }; $global = 'HKLM:\\SOFTWARE\\NVIDIA Corporation\\Global\\NVTweak'; If (!(Test-Path $global)) { New-Item $global -Force | Out-Null }; Set-ItemProperty $global 'RmLowLatencyMode' 2 -Type DWord -Force; Set-ItemProperty $global 'FlipQueueSize' 1 -Type DWord -Force; Write-Host "[RTX 5060] Low Latency Ultra + FlipQueueSize=1 applied globally and per-adapter. Survives NVCP resets. RTX 5060 GDDR7: pairing Low Latency Ultra with HAGS ON gives the tightest frame-to-display pipeline — exactly 1 frame pre-rendered, zero queued input lag." -ForegroundColor Cyan`,
  ...
  game_warframe: `$_c=(Get-CimInstance Win32_Processor -Property NumberOfCores|Measure-Object NumberOfCores -Sum).Sum; $_pri=if($_c -ge 6){4}else{3}; $paths = @("C:\\Program Files (x86)\\Steam\\steamapps\\common\\Warframe","D:\\SteamLibrary\\steamapps\\common\\Warframe","$env:LOCALAPPDATA\\Warframe"); $found = $paths | Where-Object { Test-Path $_ } | Select-Object -First 1; If ($found) { Write-Host "[DETECTED] Warframe at $found" -ForegroundColor Green; $key = 'HKLM:\\SOFTWARE\\Microsoft\\Windows NT\\CurrentVersion\\Image File Execution Options\\Warframe.x64.exe\\PerfOptions'; If (!(Test-Path $key)) { New-Item $key -Force | Out-Null }; Set-ItemProperty $key 'CpuPriorityClass' $_pri; Set-ItemProperty $key 'IoPriority' 3; Write-Host "[OK] Warframe: CPU=$(if($_pri-eq 4){'High'}else{'AboveNormal'}) + High I/O" -ForegroundColor Green } Else { Write-Host "[SKIP] Warframe not detected" -ForegroundColor DarkGray }`,
  game_forza: `$_c=(Get-CimInstance Win32_Processor -Property NumberOfCores|Measure-Object NumberOfCores -Sum).Sum; $_pri=if($_c -ge 6){4}else{3}; $paths = @("C:\\Program Files (x86)\\Steam\\steamapps\\common\\ForzaHorizon5","D:\\SteamLibrary\\steamapps\\common\\ForzaHorizon5","$env:ProgramFiles\\WindowsApps"); $found = $paths | Where-Object { Test-Path $_ } | Select-Object -First 1; If ($found) { Write-Host "[DETECTED] Forza Horizon 5" -ForegroundColor Green; $key = 'HKLM:\\SOFTWARE\\Microsoft\\Windows NT\\CurrentVersion\\Image File Execution Options\\ForzaHorizon5.exe\\PerfOptions'; If (!(Test-Path $key)) { New-Item $key -Force | Out-Null }; Set-ItemProperty $key 'CpuPriorityClass' $_pri; Set-ItemProperty $key 'IoPriority' 3; Write-Host "[OK] Forza Horizon 5: CPU=$(if($_pri-eq 4){'High'}else{'AboveNormal'}) + High I/O" -ForegroundColor Green } Else { Write-Host "[SKIP] Forza Horizon 5 not detected" -ForegroundColor DarkGray }`,
  game_readyornot: `$_c=(Get-CimInstance Win32_Processor -Property NumberOfCores|Measure-Object NumberOfCores -Sum).Sum; $_pri=if($_c -ge 6){4}else{3}; $paths = @("C:\\Program Files (x86)\\Steam\\steamapps\\common\\Ready Or Not","D:\\SteamLibrary\\steamapps\\common\\Ready Or Not","E:\\SteamLibrary\\steamapps\\common\\Ready Or Not"); $found = $paths | Where-Object { Test-Path $_ } | Select-Object -First 1; If ($found) { Write-Host "[DETECTED] Ready or Not" -ForegroundColor Green; $key = 'HKLM:\\SOFTWARE\\Microsoft\\Windows NT\\CurrentVersion\\Image File Execution Options\\ReadyOrNot.exe\\PerfOptions'; If (!(Test-Path $key)) { New-Item $key -Force | Out-Null }; Set-ItemProperty $key 'CpuPriorityClass' $_pri; Set-ItemProperty $key 'IoPriority' 3; Write-Host "[OK] Ready or Not: CPU=$(if($_pri-eq 4){'High'}else{'AboveNormal'}) + High I/O" -ForegroundColor Green } Else { Write-Host "[SKIP] Ready or Not not detected" -ForegroundColor DarkGray }`,
  game_phasmo: `$_c=(Get-CimInstance Win32_Processor -Property NumberOfCores|Measure-Object NumberOfCores -Sum).Sum; $_pri=if($_c -ge 6){4}else{3}; $paths = @("C:\\Program Files (x86)\\Steam\\steamapps\\common\\Phasmophobia","D:\\SteamLibrary\\steamapps\\common\\Phasmophobia"); $found = $paths | Where-Object { Test-Path $_ } | Select-Object -First 1; If ($found) { Write-Host "[DETECTED] Phasmophobia" -ForegroundColor Green; $key = 'HKLM:\\SOFTWARE\\Microsoft\\Windows NT\\CurrentVersion\\Image File Execution Options\\Phasmophobia.exe\\PerfOptions'; If (!(Test-Path $key)) { New-Item $key -Force | Out-Null }; Set-ItemProperty $key 'CpuPriorityClass' $_pri; Set-ItemProperty $key 'IoPriority' 3; Write-Host "[OK] Phasmophobia: CPU=$(if($_pri-eq 4){'High'}else{'AboveNormal'}) + High I/O" -ForegroundColor Green } Else { Write-Host "[SKIP] Phasmophobia not detected" -ForegroundColor DarkGray }`,
  game_battlefield: `$_c=(Get-CimInstance Win32_Processor -Property NumberOfCores|Measure-Object NumberOfCores -Sum).Sum; $_pri=if($_c -ge 6){4}else{3}; $paths = @("C:\\Program Files\\EA Games\\Battlefield 2042","C:\\Program Files (x86)\\Origin Games\\Battlefield 2042","C:\\Program Files (x86)\\Steam\\steamapps\\common\\Battlefield 2042","D:\\SteamLibrary\\steamapps\\common\\Battlefield 2042"); $found = $paths | Where-Object { Test-Path $_ } | Select-Object -First 1; If ($found) { Write-Host "[DETECTED] Battlefield 2042" -ForegroundColor Green; $key = 'HKLM:\\SOFTWARE\\Microsoft\\Windows NT\\CurrentVersion\\Image File Execution Options\\BF2042.exe\\PerfOptions'; If (!(Test-Path $key)) { New-Item $key -Force | Out-Null }; Set-ItemProperty $key 'CpuPriorityClass' $_pri; Set-ItemProperty $key 'IoPriority' 3; Write-Host "[OK] Battlefield 2042: CPU=$(if($_pri-eq 4){'High'}else{'AboveNormal'}) + High I/O" -ForegroundColor Green } Else { Write-Host "[SKIP] Battlefield 2042 not detected" -ForegroundColor DarkGray }`,
  game_gta5: `$_c=(Get-CimInstance Win32_Processor -Property NumberOfCores|Measure-Object NumberOfCores -Sum).Sum; $_pri=if($_c -ge 6){4}else{3}; $_steamReg=(Get-ItemProperty 'HKCU:\\Software\\Valve\\Steam' -EA SilentlyContinue)?.SteamPath; $_steamLibs=@(); if($_steamReg){$_sv=$_steamReg.Replace('/','\\'); $_steamLibs+=@($_sv,"$_sv\\steamapps"); $vdf=Join-Path $_sv 'steamapps\\libraryfolders.vdf'; if(Test-Path $vdf){Get-Content $vdf|%{if($_-match '"path"\\s+"([^"]+)"'){$_lp=$Matches[1].Replace('\\\\','\\'); if(Test-Path $_lp){$_steamLibs+=$_lp}}}}}; $_gta5paths=@("C:\\Program Files\\Rockstar Games\\Grand Theft Auto V\\GTA5.exe","D:\\Rockstar Games\\Grand Theft Auto V\\GTA5.exe","E:\\Rockstar Games\\Grand Theft Auto V\\GTA5.exe","F:\\Rockstar Games\\Grand Theft Auto V\\GTA5.exe","C:\\Games\\Grand Theft Auto V\\GTA5.exe","D:\\Games\\Grand Theft Auto V\\GTA5.exe","E:\\Games\\Grand Theft Auto V\\GTA5.exe"); foreach($_l in $_steamLibs){$_gta5paths+="$_l\\steamapps\\common\\Grand Theft Auto V\\GTA5.exe"}; $found=$_gta5paths|Where-Object{Test-Path $_}|Select-Object -First 1; If ($found) { $dir = Split-Path $found -Parent; Write-Host "[DETECTED] GTA V at $dir" -ForegroundColor Green; $key = 'HKLM:\\SOFTWARE\\Microsoft\\Windows NT\\CurrentVersion\\Image File Execution Options\\GTA5.exe\\PerfOptions'; If (!(Test-Path $key)) { New-Item -Path $key -Force | Out-Null }; Set-ItemProperty $key 'CpuPriorityClass' $_pri -Type DWord -Force; Set-ItemProperty $key 'IoPriority' 3 -Type DWord -Force; Set-ItemProperty $key 'GPUPriority' 8 -Type DWord -Force; Set-ItemProperty $key 'CpuPriorityBoost' 1 -Type DWord -Force; Set-ItemProperty $key 'DisableEnergyThrottling' 1 -Type DWord -Force; try{Add-MpPreference -ExclusionPath $dir -EA SilentlyContinue}catch{}; $scKey = 'HKLM:\\SOFTWARE\\Microsoft\\Windows NT\\CurrentVersion\\Image File Execution Options\\SocialClubHelper.exe\\PerfOptions'; If (!(Test-Path $scKey)) { New-Item -Path $scKey -Force | Out-Null }; Set-ItemProperty $scKey 'CpuPriorityClass' 1 -Type DWord -Force; Write-Host "[OK] GTA V: CPU=$(if($_pri-eq 4){'High'}else{'AboveNormal'}) IO+GPU=8, Defender exclusion, SocialClub deprioritized" -ForegroundColor Green } Else { Write-Host "[SKIP] GTA V not detected" -ForegroundColor DarkGray }`,
  game_fivem: `$_c=(Get-CimInstance Win32_Processor -Property NumberOfCores|Measure-Object NumberOfCores -Sum).Sum; $_pri=if($_c -ge 6){4}else{3}; $paths = @("$env:LocalAppData\\FiveM\\FiveM.exe","$env:LocalAppData\\FiveM\\FiveM.app\\FiveM.exe"); $found = $paths | Where-Object { Test-Path $_ } | Select-Object -First 1; If ($found) { $fivemDir = "$env:LocalAppData\\FiveM\\FiveM.app"; Write-Host "[DETECTED] FiveM at $env:LocalAppData\\FiveM" -ForegroundColor Green; $ifeo = 'HKLM:\\SOFTWARE\\Microsoft\\Windows NT\\CurrentVersion\\Image File Execution Options'; $applyPerf = { param($exe) $p = "$ifeo\\$exe\\PerfOptions"; If (!(Test-Path $p)) { New-Item $p -Force | Out-Null }; Set-ItemProperty $p 'CpuPriorityClass' $_pri -Type DWord -Force; Set-ItemProperty $p 'CpuPriorityBoost' 1 -Type DWord -Force; Set-ItemProperty $p 'DisableEnergyThrottling' 1 -Type DWord -Force; Set-ItemProperty $p 'EnableBoost' 1 -Type DWord -Force; Set-ItemProperty $p 'IoPriority' 2 -Type DWord -Force; Set-ItemProperty $p 'PagePriority' 5 -Type DWord -Force; Write-Host "[OK] PerfOptions applied: $exe" -ForegroundColor Green }; @('FiveM.exe','GTA5.exe','FiveM_b2189_GTAProcess.exe','FiveM_b3323_GTAProcess.exe','FiveM_b3258_GTAProcess.exe','FiveM_b2802_GTAProcess.exe') | ForEach-Object { & $applyPerf $_ }; If (Test-Path $fivemDir) { try{Add-MpPreference -ExclusionPath $fivemDir -EA SilentlyContinue}catch{}; Write-Host "[OK] FiveM.app added to Defender exclusions" -ForegroundColor Green }; Set-ItemProperty -Path 'HKLM:\\SYSTEM\\CurrentControlSet\\Services\\AFD\\Parameters' -Name 'DefaultReceiveWindow' -Value 524288 -Type DWord -Force; Set-ItemProperty -Path 'HKLM:\\SYSTEM\\CurrentControlSet\\Services\\AFD\\Parameters' -Name 'DefaultSendWindow' -Value 524288 -Type DWord -Force; Set-ItemProperty -Path 'HKLM:\\SOFTWARE\\Microsoft\\Windows NT\\CurrentVersion\\Multimedia\\SystemProfile' -Name 'SystemResponsiveness' -Value 10 -Type DWord -Force; $adapter = Get-NetAdapter | Where-Object Status -eq 'Up' | Select-Object -First 1; If ($adapter) { Set-DnsClientServerAddress -InterfaceAlias $adapter.Name -ServerAddresses ('1.1.1.1','1.0.0.1') -EA SilentlyContinue; Write-Host "[OK] DNS set to Cloudflare 1.1.1.1" -ForegroundColor Green }; Write-Host "[OK] FiveM: Full PerfOptions + Defender exclusion + 512KB network buffer + SystemResponsiveness=10" -ForegroundColor Green } Else { Write-Host "[SKIP] FiveM not detected" -ForegroundColor DarkGray }`,
  game_rocketleague: `$_c=(Get-CimInstance Win32_Processor -Property NumberOfCores|Measure-Object NumberOfCores -Sum).Sum; $_pri=if($_c -ge 6){4}else{3}; $_steamReg=(Get-ItemProperty 'HKCU:\\Software\\Valve\\Steam' -EA SilentlyContinue)?.SteamPath; $_steamLibs=@(); if($_steamReg){$_sv=$_steamReg.Replace('/','\\'); $_steamLibs+=@($_sv,"$_sv\\steamapps"); $vdf=Join-Path $_sv 'steamapps\\libraryfolders.vdf'; if(Test-Path $vdf){Get-Content $vdf|%{if($_-match '"path"\\s+"([^"]+)"'){$_lp=$Matches[1].Replace('\\\\','\\'); if(Test-Path $_lp){$_steamLibs+=$_lp}}}}}; $_epicDat='C:\\ProgramData\\Epic\\UnrealEngineLauncher\\LauncherInstalled.dat'; $_epicFound=''; if(Test-Path $_epicDat){try{$_edata=Get-Content $_epicDat -Raw|ConvertFrom-Json; $_rl=$_edata.InstallationList|Where-Object{$_.AppName -like '*ocketleague*' -or $_.AppName -eq 'Sugar'}|Select-Object -First 1; if($_rl){$_epicFound=$_rl.InstallLocation}}catch{}}; $_rlPaths=@("C:\\Program Files\\Epic Games\\rocketleague\\Binaries\\Win64\\RocketLeague.exe","D:\\Epic Games\\rocketleague\\Binaries\\Win64\\RocketLeague.exe","E:\\Epic Games\\rocketleague\\Binaries\\Win64\\RocketLeague.exe","F:\\Epic Games\\rocketleague\\Binaries\\Win64\\RocketLeague.exe"); foreach($_l in $_steamLibs){$_rlPaths+="$_l\\steamapps\\common\\rocketleague\\Binaries\\Win64\\RocketLeague.exe"}; if($_epicFound -and (Test-Path "$_epicFound\\Binaries\\Win64\\RocketLeague.exe")){$_rlPaths="$_epicFound\\Binaries\\Win64\\RocketLeague.exe"+$_rlPaths}; $found=$_rlPaths|Where-Object{Test-Path $_}|Select-Object -First 1; If ($found) { Write-Host "[DETECTED] Rocket League at $found" -ForegroundColor Green; $key = 'HKLM:\\SOFTWARE\\Microsoft\\Windows NT\\CurrentVersion\\Image File Execution Options\\RocketLeague.exe\\PerfOptions'; If (!(Test-Path $key)) { New-Item -Path $key -Force | Out-Null }; Set-ItemProperty $key 'CpuPriorityClass' $_pri -Type DWord -Force; Set-ItemProperty $key 'IoPriority' 3 -Type DWord -Force; Set-ItemProperty $key 'GPUPriority' 8 -Type DWord -Force; Set-ItemProperty $key 'CpuPriorityBoost' 1 -Type DWord -Force; Set-ItemProperty $key 'DisableEnergyThrottling' 1 -Type DWord -Force; Set-ItemProperty -Path 'HKLM:\\SYSTEM\\CurrentControlSet\\Services\\AFD\\Parameters' -Name 'DefaultReceiveWindow' -Value 524288 -Type DWord -Force; Set-ItemProperty -Path 'HKLM:\\SYSTEM\\CurrentControlSet\\Services\\AFD\\Parameters' -Name 'DefaultSendWindow' -Value 524288 -Type DWord -Force; Write-Host "[OK] Rocket League: CPU=$(if($_pri-eq 4){'High'}else{'AboveNormal'}) IO+GPU=8, EnergyThrottle=Off, 512KB net buffer" -ForegroundColor Green } Else { Write-Host "[SKIP] Rocket League not detected" -ForegroundColor DarkGray }`,
  ...
  ProcSvc_TabletInput: `$svc = Get-Service -Name 'TabletInputService' -ErrorAction SilentlyContinue; If ($svc) { Stop-Service -InputObject $svc -Force -ErrorAction SilentlyContinue; Set-Service -InputObject $svc -StartupType Manual -ErrorAction SilentlyContinue; Write-Host "[Processes] Tablet Input Service set to Manual" -ForegroundColor Green } Else { Write-Host "[Processes] Tablet Input Service is not installed — already satisfied" -ForegroundColor DarkGray }`,
  ...
  ProcSvc_RetailDemo: `$svc = Get-Service -Name 'RetailDemo' -ErrorAction SilentlyContinue; If ($svc) { Stop-Service -InputObject $svc -Force -ErrorAction SilentlyContinue; Set-Service -InputObject $svc -StartupType Manual -ErrorAction SilentlyContinue; Write-Host "[Processes] Retail Demo Service set to Manual" -ForegroundColor Green } Else { Write-Host "[Processes] Retail Demo Service is not installed — already satisfied" -ForegroundColor DarkGray }`,
  ...
  // ── CPU Tweaks (dedicated CPU tab) ──────────────────────────────────────────
  CpuBoostModeAggressive: `powercfg -setacvalueindex SCHEME_CURRENT 54533251-82be-4824-96c1-47b60b740d00 be337238-0d82-4146-a960-4f3749d470c7 2 2>$null; powercfg -setactive SCHEME_CURRENT 2>$null; Write-Host "[CPU] Processor Performance Boost Mode = Aggressive — CPU ramps to max frequency instantly on any load spike" -ForegroundColor Green`,
  CpuIdleMin100: `powercfg -setacvalueindex SCHEME_CURRENT 54533251-82be-4824-96c1-47b60b740d00 893dee8e-2bef-41e0-89c6-b55d0929964c 100 2>$null; powercfg -setactive SCHEME_CURRENT 2>$null; Write-Host "[CPU] Minimum Processor State = 100% — no idle frequency scaling, CPU locked to full speed" -ForegroundColor Green`,
  CpuFortniteIFEO: `$_c=(Get-CimInstance Win32_Processor -Property NumberOfCores|Measure-Object NumberOfCores -Sum).Sum; $_pri=3; If($_c -ge 6){$_pri=4}; $ifeo = 'HKLM:\SOFTWARE\Microsoft\Windows NT\CurrentVersion\Image File Execution Options'; @('FortniteClient-Win64-Shipping.exe','FortniteLauncher.exe','EpicGamesLauncher.exe') | ForEach-Object { $exe = $_; $k = Join-Path (Join-Path $ifeo $exe) 'PerfOptions'; If (!(Test-Path -LiteralPath $k)) { New-Item -Path $k -Force | Out-Null }; Set-ItemProperty -LiteralPath $k -Name 'CpuPriorityClass' -Value $_pri -Type DWord -Force; Set-ItemProperty -LiteralPath $k -Name 'CpuPriorityBoost' -Value 1 -Type DWord -Force; Set-ItemProperty -LiteralPath $k -Name 'DisableEnergyThrottling' -Value 1 -Type DWord -Force; Set-ItemProperty -LiteralPath $k -Name 'ForceForegroundBoost' -Value 1 -Type DWord -Force; Set-ItemProperty -LiteralPath $k -Name 'IoPriority' -Value 2 -Type DWord -Force; Set-ItemProperty -LiteralPath $k -Name 'PagePriority' -Value 5 -Type DWord -Force; Set-ItemProperty -LiteralPath $k -Name 'GpuPriority' -Value 8 -Type DWord -Force; Write-Host "[Fortnite] IFEO applied to $exe — CPU=$(if($_pri-eq 4){'High'}else{'AboveNormal'}), boost ON, IO=2, GPU=8" -ForegroundColor Green }`,
  CpuCodIFEO: `$_c=(Get-CimInstance Win32_Processor -Property NumberOfCores|Measure-Object NumberOfCores -Sum).Sum; $_pri=3; If($_c -ge 6){$_pri=4}; $ifeo = 'HKLM:\SOFTWARE\Microsoft\Windows NT\CurrentVersion\Image File Execution Options'; @('cod.exe','BlackOpsColdWar.exe','ModernWarfare.exe','ModernWarfare2.exe','ModernWarfare3.exe','Warzone.exe','MWIII.exe') | ForEach-Object { $exe = $_; $k = Join-Path (Join-Path $ifeo $exe) 'PerfOptions'; If (!(Test-Path -LiteralPath $k)) { New-Item -Path $k -Force | Out-Null }; Set-ItemProperty -LiteralPath $k -Name 'CpuPriorityClass' -Value $_pri -Type DWord -Force; Set-ItemProperty -LiteralPath $k -Name 'CpuPriorityBoost' -Value 1 -Type DWord -Force; Set-ItemProperty -LiteralPath $k -Name 'DisableEnergyThrottling' -Value 1 -Type DWord -Force; Set-ItemProperty -LiteralPath $k -Name 'ForceForegroundBoost' -Value 1 -Type DWord -Force; Set-ItemProperty -LiteralPath $k -Name 'IoPriority' -Value 2 -Type DWord -Force; Set-ItemProperty -LiteralPath $k -Name 'PagePriority' -Value 5 -Type DWord -Force; Set-ItemProperty -LiteralPath $k -Name 'GpuPriority' -Value 8 -Type DWord -Force; Write-Host "[COD] IFEO applied to $exe — CPU=$(if($_pri-eq 4){'High'}else{'AboveNormal'}), boost ON, IO=2, GPU=8" -ForegroundColor Green }`,
  CpuGenericGameIFEO: `$_c=(Get-CimInstance Win32_Processor -Property NumberOfCores|Measure-Object NumberOfCores -Sum).Sum; $_pri=3; If($_c -ge 6){$_pri=4}; $ifeo = 'HKLM:\SOFTWARE\Microsoft\Windows NT\CurrentVersion\Image File Execution Options'; @('cs2.exe','csgo.exe','valorant.exe','VALORANT-Win64-Shipping.exe','r5apex.exe','RainbowSix.exe','RainbowSix_BE.exe','EscapeFromTarkov.exe','DayZ_BE.exe','pubg.exe','RustClient.exe','Overwatch.exe','ARC-Win64-Shipping.exe','MarvelRivals-Win64-Shipping.exe','ReadyOrNot.exe','BF2042.exe','RocketLeague.exe') | ForEach-Object { $exe = $_; $k = Join-Path (Join-Path $ifeo $exe) 'PerfOptions'; If (!(Test-Path -LiteralPath $k)) { New-Item -Path $k -Force | Out-Null }; Set-ItemProperty -LiteralPath $k -Name 'CpuPriorityClass' -Value $_pri -Type DWord -Force; Set-ItemProperty -LiteralPath $k -Name 'CpuPriorityBoost' -Value 1 -Type DWord -Force; Set-ItemProperty -LiteralPath $k -Name 'DisableEnergyThrottling' -Value 1 -Type DWord -Force; Set-ItemProperty -LiteralPath $k -Name 'ForceForegroundBoost' -Value 1 -Type DWord -Force; Set-ItemProperty -LiteralPath $k -Name 'IoPriority' -Value 2 -Type DWord -Force; Set-ItemProperty -LiteralPath $k -Name 'PagePriority' -Value 5 -Type DWord -Force; Write-Host "[Games] IFEO CPU=$(if($_pri-eq 4){'High'}else{'AboveNormal'})+boost: $exe" -ForegroundColor Green }; Write-Host "[Games] Priority stack complete — CS2, Valorant, Apex, ARC Raiders, Marvel Rivals, R6, Rust and more boosted" -ForegroundColor Cyan`,
  CpuDisableSpectreMitigation: `Write-Host "⚠ WARNING: Disabling Spectre/Meltdown mitigations removes protection against kernel memory disclosure attacks. Only safe on offline dedicated gaming rigs." -ForegroundColor Red; $mm = 'HKLM:\SYSTEM\CurrentControlSet\Control\Session Manager\Memory Management'; Set-ItemProperty $mm 'FeatureSettingsOverride' 3 -Type DWord -Force; Set-ItemProperty $mm 'FeatureSettingsOverrideMask' 3 -Type DWord -Force; bcdedit /set nx OptOut 2>$null | Out-Null; Write-Host "[EXPERT] Spectre/Meltdown mitigations disabled — reboot required. Up to 8% latency improvement in kernel-intensive workloads." -ForegroundColor Red`,
  // ── End CPU Tweaks ───────────────────────────────────────────────────────────
...
// Silent Hill: Townfall is also registered here so the unified Game Profiles
// screen can apply the pack even though its command was added after the main
// catalog block.
...
function normalizeWindowsPowerShellCommand(id: string, command: string): string {
  ...
  if (id === "AmdCpuCStatePolicy") {
    return `$ErrorActionPreference = 'Stop'; $settingPath = 'HKLM:\\SYSTEM\\CurrentControlSet\\Control\\Power\\PowerSettings\\54533251-82be-4824-96c1-47b60b740d00\\40fbefc7-2e9d-4d25-a185-0cfd8574bae6'; if (-not (Test-Path -LiteralPath $settingPath)) { Write-Host "[SKIP] Windows did not expose the AMD CPU performance decrease policy." -ForegroundColor Yellow; return }; $schemeText = (& powercfg.exe /getactivescheme 2>&1 | Out-String); if ($schemeText -notmatch '([0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12})') { Write-Host "[SKIP] Windows did not return an active power scheme." -ForegroundColor Yellow; return }; $scheme = $matches[1]; & powercfg.exe /attributes 54533251-82be-4824-96c1-47b60b740d00 40fbefc7-2e9d-4d25-a185-0cfd8574bae6 -ATTRIB_HIDE 2>&1 | Out-Null; & powercfg.exe /setacvalueindex $scheme 54533251-82be-4824-96c1-47b60b740d00 40fbefc7-2e9d-4d25-a185-0cfd8574bae6 0 2>&1 | Out-Null; if ($LASTEXITCODE -ne 0) { Write-Host "[SKIP] The active power scheme does not support the AMD CPU performance decrease policy." -ForegroundColor Yellow; return }; & powercfg.exe /setactive $scheme 2>&1 | Out-Null; if ($LASTEXITCODE -ne 0) { Write-Host "[SKIP] Windows could not activate the current power scheme after checking the AMD CPU policy." -ForegroundColor Yellow; return }; Set-ItemProperty -Path $settingPath -Name 'Attributes' -Value 2 -Type DWord -Force; Write-Host "[AMD CPU] CPU performance decrease policy set to Fastest and verified." -ForegroundColor Green`;
  }
  if (id === "IntelOldGenPowerOpt" || id === "FiveM3500PerfPlan") {
    const label = id === "IntelOldGenPowerOpt" ? "Intel 4th-8th Gen" : "Ryzen 5 3500";
    const settings = id === "IntelOldGenPowerOpt"
      ? ["PROCTHROTTLEMIN", "PROCTHROTTLEMAX", "PERFBOOSTMODE", "PERFBOOSTPOL"]
      : ["PROCTHROTTLEMIN", "PROCTHROTTLEMAX", "PERFBOOSTMODE", "PERFBOOSTPOL", "CPMINCORES"];
    const values = id === "IntelOldGenPowerOpt"
      ? ["100", "100", "2", "100"]
      : ["100", "100", "2", "100", "100"];
    const pairs = settings.map((setting, index) => `if (-not (Set-PowerValue $scheme '${setting}' '${values[index]}')) { $errors += '${setting}' }`).join(" ");
    const checks = settings.map(setting => `if (-not (Test-PowerValue $scheme '${setting}')) { $missing += '${setting}' }`).join(" ");
    return `$ErrorActionPreference = 'Stop'; function Test-PowerValue([string]$scheme, [string]$setting) { & powercfg.exe /query $scheme SUB_PROCESSOR $setting 2>&1 | Out-Null; return ($LASTEXITCODE -eq 0) }; function Set-PowerValue([string]$scheme, [string]$setting, [string]$value) { & powercfg.exe /setacvalueindex $scheme SUB_PROCESSOR $setting $value 2>&1 | Out-Null; return ($LASTEXITCODE -eq 0) }; $scheme = '8c5e7fda-e8bf-4a96-9a85-a6e23a8c635c'; & powercfg.exe /setactive $scheme 2>&1 | Out-Null; if ($LASTEXITCODE -ne 0) { $scheme = 'SCHEME_CURRENT' }; $missing = @(); ${checks} if ($missing.Count -gt 0) { Write-Host "[SKIP] ${label} power settings unavailable: $($missing -join ', ')." -ForegroundColor Yellow; return }; $errors = @(); ${pairs} if ($errors.Count -gt 0) { Write-Host "[SKIP] ${label} power settings could not be applied: $($errors -join ', ')." -ForegroundColor Yellow; return }; & powercfg.exe /setactive $scheme 2>&1 | Out-Null; if ($LASTEXITCODE -ne 0) { Write-Host "[SKIP] Windows could not activate the ${label} power scheme." -ForegroundColor Yellow; return }; Write-Host "[${label}] power plan settings applied and verified." -ForegroundColor Green`;
  }
  if (id === "CpuDisableSpectreMitigation") {
    return `$ErrorActionPreference = 'Stop'; $mm = 'HKLM:\\SYSTEM\\CurrentControlSet\\Control\\Session Manager\\Memory Management'; if (-not (Test-Path -LiteralPath $mm)) { New-Item -Path $mm -Force | Out-Null }; Set-ItemProperty -Path $mm -Name 'FeatureSettingsOverride' -Value 3 -Type DWord -Force; Set-ItemProperty -Path $mm -Name 'FeatureSettingsOverrideMask' -Value 3 -Type DWord -Force; $values = Get-ItemProperty -Path $mm -ErrorAction Stop; if ($values.FeatureSettingsOverride -ne 3 -or $values.FeatureSettingsOverrideMask -ne 3) { throw "Windows did not verify Spectre/Meltdown mitigation values." }; & bcdedit.exe /set nx OptOut 2>&1 | Out-Null; if ($LASTEXITCODE -ne 0) { throw "Windows could not update the NX boot policy." }; Write-Host "[EXPERT] Spectre/Meltdown mitigations disabled and verified. Reboot required." -ForegroundColor Red`;
  }
  // Startup removal is an idempotent desired-state operation. Missing
  // registry values make reg.exe print an error even though the machine is
  // already in the requested state; suppress that expected condition so a
  // clean PC is not reported as a failed tweak.
  if (id.startsWith("su_")) {
    return `$ErrorActionPreference = 'SilentlyContinue'; ${command}`;
  }
  // IFEO paths must be created through the registry provider with a literal
  // path. Join-Path against an HKLM provider path was rejected by some
  // Windows PowerShell 5.1 builds even when the same key was writable.
  if (["CpuFortniteIFEO", "CpuCodIFEO", "CpuGenericGameIFEO"].includes(id)) {
    const executables = id === "CpuCodIFEO"
      ? ["cod.exe", "BlackOpsColdWar.exe", "ModernWarfare.exe", "ModernWarfare2.exe", "ModernWarfare3.exe", "Warzone.exe", "MWIII.exe"]
      : id === "CpuFortniteIFEO"
        ? ["FortniteClient-Win64-Shipping.exe", "FortniteLauncher.exe", "EpicGamesLauncher.exe"]
        : ["cs2.exe", "csgo.exe", "valorant.exe", "VALORANT-Win64-Shipping.exe", "r5apex.exe", "RainbowSix.exe", "RainbowSix_BE.exe", "EscapeFromTarkov.exe", "DayZ_BE.exe", "pubg.exe", "RustClient.exe", "Overwatch.exe", "ARC-Win64-Shipping.exe", "MarvelRivals-Win64-Shipping.exe", "ReadyOrNot.exe", "BF2042.exe", "RocketLeague.exe"];
    const list = executables.map(exe => `'${exe}'`).join(",");
    const label = id === "CpuCodIFEO" ? "COD" : id === "CpuFortniteIFEO" ? "Fortnite" : "generic game";
    return `$ErrorActionPreference = 'Stop'; $cpu = @(Get-CimInstance Win32_Processor -Property NumberOfCores | Measure-Object NumberOfCores -Sum).Sum; $priority = if ($cpu -ge 6) { 4 } else { 3 }; $ifeo = 'Registry::HKEY_LOCAL_MACHINE\\SOFTWARE\\Microsoft\\Windows NT\\CurrentVersion\\Image File Execution Options'; $executables = @(${list}); foreach ($exe in $executables) { $key = "$ifeo\\$exe\\PerfOptions"; if (!(Test-Path -Path $key)) { New-Item -Path $key -Force -ErrorAction Stop | Out-Null }; Set-ItemProperty -Path $key -Name 'CpuPriorityClass' -Value $priority -Type DWord -Force -ErrorAction Stop; Set-ItemProperty -Path $key -Name 'CpuPriorityBoost' -Value 1 -Type DWord -Force; Set-ItemProperty -Path $key -Name 'DisableEnergyThrottling' -Value 1 -Type DWord -Force; Set-ItemProperty -Path $key -Name 'ForceForegroundBoost' -Value 1 -Type DWord -Force; Set-ItemProperty -Path $key -Name 'IoPriority' -Value 2 -Type DWord -Force; Set-ItemProperty -Path $key -Name 'PagePriority' -Value 5 -Type DWord -Force; Set-ItemProperty -Path $key -Name 'GpuPriority' -Value 8 -Type DWord -Force }; Write-Host "[${label}] IFEO CPU priority and foreground boost applied." -ForegroundColor Green`;
  }
  if (!["game_gta5", "game_rocketleague", "game_marvelrivals"].includes(id)) return command;
  // Use ordinary member access here. `$null.Property` evaluates to `$null`
  // in Windows PowerShell 5.1, while `?.` is a PowerShell 7-only parser
  // feature and makes the entire action fail before detection can run.
  ...
  app.post("/api/performance-allowance/authorize", async (req, res) => {
    ...
    if (req.body?.mode === "best") {
      ...
      const selectedGameIds = new Set(
        hasGameSelection
          ? req.body.gameIds.filter((id: unknown): id is string => typeof id === "string" && GAME_DETECT_PACK_IDS.includes(id))
          : GAME_DETECT_PACK_IDS,
      );
      const selectedCore = compatibleCore.filter(id => !id.startsWith("game_") || selectedGameIds.has(id));
      if (isPro) {
        ...
        const instantCandidates = selectedCore.filter(id => freeEligibleId(id) && NATIVE_EXECUTABLE_ALLOWLIST.has(id));
...
`.trim();
    ...
    const bat = [
      ...
      ``,
      `:: ── Self-copy to Desktop so user can re-run anytime ─────────────────────`,
...
# Discover Steam library roots — registry first, then known paths, then VDF
$steamRoots = @()

# 1. Use Steam registry key (most reliable — works regardless of install drive)
$steamReg = Get-ItemProperty 'HKCU:\\Software\\Valve\\Steam' -EA SilentlyContinue
if ($steamReg -and $steamReg.SteamPath) {
    $regSteamPath = $steamReg.SteamPath.Replace('/', '\\')
    if ((Test-Path $regSteamPath) -and $regSteamPath -notin $steamRoots) { $steamRoots += $regSteamPath }
    # Parse libraryfolders.vdf for additional library folders on other drives
    $vdf = Join-Path $regSteamPath 'steamapps\\libraryfolders.vdf'
    if (Test-Path $vdf) {
        Get-Content $vdf | ForEach-Object {
            if ($_ -match '"path"\\s+"([^"]+)"') {
                $libPath = $Matches[1].Replace('\\\\', '\\')
                if ((Test-Path $libPath) -and $libPath -notin $steamRoots) { $steamRoots += $libPath }
            }
        }
    }
}

# 2. Fallback — common install locations across drives C-H
$knownSteamPaths = @(
    'C:\\Program Files (x86)\\Steam','C:\\Program Files\\Steam',
    'D:\\Steam','D:\\SteamLibrary','D:\\Program Files (x86)\\Steam',
    'E:\\Steam','E:\\SteamLibrary','E:\\Program Files (x86)\\Steam',
    'F:\\Steam','F:\\SteamLibrary','F:\\Program Files (x86)\\Steam',
    'G:\\Steam','G:\\SteamLibrary','H:\\Steam','H:\\SteamLibrary'
)
foreach ($s in $knownSteamPaths) { if ((Test-Path $s) -and $s -notin $steamRoots) { $steamRoots += $s } }

# Discover Epic Games installations via LauncherInstalled.dat
$epicInstalls = @{}
$epicDat = 'C:\\ProgramData\\Epic\\UnrealEngineLauncher\\LauncherInstalled.dat'
if (Test-Path $epicDat) {
    try {
        $epicData = Get-Content $epicDat -Raw | ConvertFrom-Json
        foreach ($inst in $epicData.InstallationList) {
            if ($inst.InstallLocation -and (Test-Path $inst.InstallLocation)) {
                $epicInstalls[$inst.AppName] = $inst.InstallLocation
            }
        }
    } catch {}
}

function Find-Game { param([string[]]$Paths, [string[]]$EpicAppNames = @())
    # Check Epic app names first via LauncherInstalled.dat
    foreach ($appName in $EpicAppNames) {
        if ($epicInstalls.ContainsKey($appName)) { return $true }
        # Fuzzy match — some app names contain the game name
        foreach ($key in $epicInstalls.Keys) {
            if ($key -like "*$appName*" -or $appName -like "*$key*") { return $true }
        }
    }
    foreach ($p in $Paths) {
...
$games = @(
    ...
    @{ id = "game_gta5";          epicAppNames = @(); paths = @("steamapps\\common\\Grand Theft Auto V\\GTA5.exe","C:\\Program Files\\Rockstar Games\\Grand Theft Auto V\\GTA5.exe","D:\\Rockstar Games\\Grand Theft Auto V\\GTA5.exe","E:\\Rockstar Games\\Grand Theft Auto V\\GTA5.exe","F:\\Rockstar Games\\Grand Theft Auto V\\GTA5.exe","D:\\Games\\Grand Theft Auto V\\GTA5.exe","E:\\Games\\Grand Theft Auto V\\GTA5.exe","C:\\Games\\GTA V\\GTA5.exe","D:\\Games\\GTA V\\GTA5.exe") },
    @{ id = "game_fivem";         epicAppNames = @(); paths = @("%LocalAppData%\\FiveM\\FiveM.exe","%LocalAppData%\\FiveM\\FiveM.app\\FiveM.exe") },
    @{ id = "game_rocketleague";  epicAppNames = @("Sugar","rocketleague","Rocket League"); paths = @("steamapps\\common\\rocketleague\\Binaries\\Win64\\RocketLeague.exe","C:\\Program Files\\Epic Games\\rocketleague\\Binaries\\Win64\\RocketLeague.exe","D:\\Epic Games\\rocketleague\\Binaries\\Win64\\RocketLeague.exe","E:\\Epic Games\\rocketleague\\Binaries\\Win64\\RocketLeague.exe","F:\\Epic Games\\rocketleague\\Binaries\\Win64\\RocketLeague.exe") },
    @{ id = "game_arcraiders";    epicAppNames = @(); paths = @("steamapps\\common\\ARC Raiders","steamapps\\common\\Arc Raiders","C:\\Games\\ARC Raiders","D:\\Games\\ARC Raiders","E:\\Games\\ARC Raiders") },
    @{ id = "game_marvelrivals";  epicAppNames = @(); paths = @("steamapps\\common\\Marvel Rivals","steamapps\\common\\MarvelRivals","C:\\Games\\Marvel Rivals","D:\\Games\\Marvel Rivals") },
    @{ id = "game_007firstlight"; epicAppNames = @(); paths = @("steamapps\\common\\007 First Light","steamapps\\common\\007FirstLight","C:\\Program Files\\IO Interactive\\007 First Light","D:\\Games\\007 First Light","C:\\Program Files\\EA Games\\007 First Light","steamapps\\common\\Project 007") },
    @{ id = "game_fortnite";      epicAppNames = @("Fortnite"); paths = @("C:\\Program Files\\Epic Games\\Fortnite\\FortniteGame\\Binaries\\Win64\\FortniteClient-Win64-Shipping.exe","D:\\Epic Games\\Fortnite\\FortniteGame\\Binaries\\Win64\\FortniteClient-Win64-Shipping.exe","E:\\Epic Games\\Fortnite\\FortniteGame\\Binaries\\Win64\\FortniteClient-Win64-Shipping.exe","F:\\Epic Games\\Fortnite\\FortniteGame\\Binaries\\Win64\\FortniteClient-Win64-Shipping.exe","G:\\Epic Games\\Fortnite\\FortniteGame\\Binaries\\Win64\\FortniteClient-Win64-Shipping.exe","C:\\Program Files (x86)\\Epic Games\\Fortnite\\FortniteGame\\Binaries\\Win64\\FortniteClient-Win64-Shipping.exe") }
)
...
foreach ($g in $games) {
    $epicNames = if ($g.epicAppNames) { $g.epicAppNames } else { @() }
    $found = Find-Game $g.paths $epicNames
...
`;
  ...
  app.get('/api/movies-tv-fix-script', (req, res) => {
    const script = [
      `$ErrorActionPreference = 'SilentlyContinue'`,
      `Write-Host ""`,
    ...
    res.setHeader('Content-Type', 'application/octet-stream');
    res.setHeader('Content-Disposition', 'attachment; filename="OptiGods-MoviesTVFix.bat"');
    res.end(Buffer.from(wrapInBat(script, { title: 'Movies & TV Clip Fix v2', tmpName: 'OptiGods-MoviesTVFix', marker: 'MOVIES_TV_FIX_PS1_START' }), 'utf8'));
  });
  ...
  app.get('/api/fivem-crash-fix-script', (req, res) => {
    const ps1Lines = [
      ...
      ``,
      `# ── FIX 9: commandline.txt streaming pool (mushy face/hands texture fix) ──────────`,
      `Write-Host "[FIX 9] Adding streaming pool flag to FiveM commandline.txt (mushy texture fix)..." -ForegroundColor Cyan`,
      `\$cmdlinePath = "\$env:LocalAppData\\FiveM\\FiveM Application Data\\commandline.txt"`,
      `\$streamFlag  = "+set streaming_useLargerPool 1"`,
      `\$cmdlineDir  = Split-Path \$cmdlinePath`,
      `If (-not (Test-Path \$cmdlineDir)) { New-Item -ItemType Directory -Path \$cmdlineDir -Force | Out-Null }`,
      `If (-not (Test-Path \$cmdlinePath)) { "" | Set-Content \$cmdlinePath -Encoding UTF8 }`,
      `\$cmdlineContent = Get-Content \$cmdlinePath -Raw -ErrorAction SilentlyContinue`,
      `If (\$cmdlineContent -notmatch [regex]::Escape(\$streamFlag)) {`,
      `  Add-Content \$cmdlinePath "\`n\$streamFlag" -Encoding UTF8`,
      `  Write-Host "  [OK] Added '\$streamFlag' to commandline.txt" -ForegroundColor Green`,
      `  Write-Host "  [OK] Mushy/low-res face + hand textures in FiveM will be fixed after relaunch" -ForegroundColor Green`,
      `} Else {`,
      `  Write-Host "  [OK] '\$streamFlag' already present in commandline.txt — no change needed" -ForegroundColor DarkGray`,
      `}`,
      ``,
      `Write-Host ""`,
      `Write-Host "  =====================================" -ForegroundColor DarkRed`,
      `Write-Host "  DONE — All FiveM crash + texture fixes applied!" -ForegroundColor Green`,
      `Write-Host ""`,
      ...
      `Write-Host "  - commandline.txt streaming pool expanded (+set streaming_useLargerPool 1) — mushy face/hands texture fix" -ForegroundColor Gray`,
      `Write-Host ""`,
  ...
  app.get('/api/mushy-face-fix-script', (req, res) => {
    const ps1Lines = [
      ...
      ``,
      `Write-Host ""`,
      `Write-Host "  Opti Gods -- Fix Mushy / Blurry Face & Arm Textures" -ForegroundColor Red`,
      `Write-Host "  =====================================================" -ForegroundColor DarkRed`,
      `Write-Host "  Auto-detects NVIDIA or AMD and applies High Quality texture filtering" -ForegroundColor Yellow`,
      `Write-Host ""`,
      ``,
      ...
      `}`,
      ``,
      `# ── FIX 1: Driver-level High Quality texture filtering ───────────────────────`,
      `If ($isNvidia) {`,
      `  Write-Host "[FIX 1] NVIDIA detected — applying High Quality texture filtering..." -ForegroundColor Cyan`,
      ...
      `      Write-Host "  [OK] High Quality filtering applied on GPU class $k" -ForegroundColor Green`,
      `      $found = $true`,
      `    }`,
      `  }`,
      `  If (-not $found) {`,
      ...
      `    Write-Host "  [OK] High Quality filtering applied — TFQ=2, CatalystAI=1" -ForegroundColor Green`,
      `    $found = $true`,
      `  }`,
      `  If (-not $found) {`,
      `    Write-Host "  [WARN] AMD GPU class key not found. Open Radeon Software > Gaming > Graphics > Texture Filtering Quality = High" -ForegroundColor Yellow`,
      `  }`,
      `} Else {`,
      `  Write-Host "[WARN] No NVIDIA or AMD GPU detected in driver class registry." -ForegroundColor Yellow`,
      `  Write-Host "       For NVIDIA: NVCP > Manage 3D Settings > Texture filtering quality = High quality" -ForegroundColor Yellow`,
      `  Write-Host "       For AMD: Radeon Software > Gaming > Graphics > Texture Filtering Quality = High" -ForegroundColor Yellow`,
      `}`,
      ``,
      `# ── FIX 2: Expand FiveM texture streaming pool + extra quality flags ────────`,
      `Write-Host ""`,
      `Write-Host "[FIX 2] Patching FiveM commandline.txt..." -ForegroundColor Cyan`,
      `$cmdlinePath = "$env:LocalAppData\\FiveM\\FiveM Application Data\\commandline.txt"`,
      `$cmdlineDir  = Split-Path $cmdlinePath`,
      `If (-not (Test-Path $cmdlineDir)) { New-Item -ItemType Directory -Path $cmdlineDir -Force | Out-Null }`,
      `If (-not (Test-Path $cmdlinePath)) { "" | Set-Content $cmdlinePath -Encoding UTF8 }`,
      ...
      `}`,
      `If ($added.Count -gt 0) {`,
      `  Write-Host "  [OK] Added flags: $($added -join ', ')" -ForegroundColor Green`,
      `} Else {`,
      `  Write-Host "  [OK] All commandline flags already present — no change needed" -ForegroundColor DarkGray`,
      `}`,
      ``,
      `# ── FIX 3: Wipe FiveM texture/shader cache ───────────────────────────────────`,
      `Write-Host ""`,
      `Write-Host "[FIX 3] Wiping FiveM texture and shader cache..." -ForegroundColor Cyan`,
      `$fivemApp = "$env:LocalAppData\\FiveM\\FiveM.app"`,
      `$cacheFolders = @("cache\\game\\cache", "cache\\priv", "citizen\\common\\data\\ui")`,
      `$cacheWiped = 0`,
      `@(`,
      `  "$fivemApp\\cache\\game\\cache",`,
      `  "$fivemApp\\cache\\b2189",`,
      `  "$fivemApp\\cache\\b2802",`,
      `  "$fivemApp\\cache\\b2944",`,
      `  "$fivemApp\\cache\\b3095"`,
      `) | ForEach-Object {`,
      `  If (Test-Path $_) {`,
      `    Remove-Item $_ -Recurse -Force -EA SilentlyContinue`,
      `    Write-Host "  [WIPED] $_" -ForegroundColor DarkYellow`,
      `    $cacheWiped++`,
      `  }`,
      `}`,
      `If ($cacheWiped -eq 0) {`,
      `  Write-Host "  [OK] Cache already clean or FiveM not installed at default path" -ForegroundColor DarkGray`,
      `} Else {`,
      `  Write-Host "  [OK] $cacheWiped cache folder(s) wiped — FiveM will rebuild on next launch" -ForegroundColor Green`,
      `}`,
      ``,
      `# ── FIX 4: Detect conflicting ped/skin texture mods ─────────────────────────`,
      `Write-Host ""`,
      `Write-Host "[FIX 4] Scanning for conflicting ped texture mods..." -ForegroundColor Cyan`,
      `$modPaths = @(`,
      `  "$env:LocalAppData\\FiveM\\FiveM.app\\citizen\\common\\data",`,
      `  "$env:LocalAppData\\FiveM\\FiveM.app\\plugins"`,
      `)`,
      `$suspectKeywords = @("ped","skin","body","face","player","char","mp_m","mp_f")`,
      `$foundMods = @()`,
      `ForEach ($mp in $modPaths) {`,
      `  If (Test-Path $mp) {`,
      `    Get-ChildItem $mp -Recurse -Include "*.ytd","*.rpf","*.yft" -EA SilentlyContinue | ForEach-Object {`,
      `      $name = $_.Name.ToLower()`,
      `      ForEach ($kw in $suspectKeywords) {`,
      `        If ($name -match $kw) { $foundMods += $_.FullName; Break }`,
      `      }`,
      `    }`,
      `  }`,
      `}`,
      `If ($foundMods.Count -gt 0) {`,
      `  Write-Host "  [!] Found $($foundMods.Count) possible ped/skin texture file(s) — these may override face quality:" -ForegroundColor Yellow`,
      `  $foundMods | Select-Object -First 10 | ForEach-Object { Write-Host "      $_" -ForegroundColor DarkYellow }`,
      `  If ($foundMods.Count -gt 10) { Write-Host "      ... and $($foundMods.Count - 10) more" -ForegroundColor DarkYellow }`,
      `  Write-Host "  Rename or remove these files and re-test before reporting blurry textures." -ForegroundColor White`,
      `} Else {`,
      `  Write-Host "  [OK] No conflicting ped texture mods detected" -ForegroundColor Green`,
      `}`,
      ``,
      `# ── FIX 5: Auto-patch GTA V settings.xml — raise ONLY TextureQuality ────────`,
      `Write-Host ""`,
      `Write-Host "[FIX 5] Patching GTA V settings.xml (texture quality only — all other settings unchanged)..." -ForegroundColor Cyan`,
      `$settingsXml = "$env:USERPROFILE\\Documents\\Rockstar Games\\GTA V\\settings.xml"`,
      `If (Test-Path $settingsXml) {`,
      `  $xml = [xml](Get-Content $settingsXml -Encoding UTF8 -Raw)`,
      `  $texNode = $xml.SelectSingleNode("//TextureQuality")`,
      `  If ($texNode -ne $null) {`,
      `    $oldVal = $texNode.GetAttribute("value")`,
      `    # 0=Normal 1=High 2=Very High — only upgrade, never downgrade`,
      `    If ([int]$oldVal -lt 2) {`,
      `      $texNode.SetAttribute("value", "2")`,
      `      $xml.Save($settingsXml)`,
      `      Write-Host "  [OK] TextureQuality raised from $oldVal to 2 (Very High) — all other settings untouched" -ForegroundColor Green`,
      `      Write-Host "  NOTE: Texture Quality is VRAM-only, zero CPU cost — your FPS will not drop" -ForegroundColor Yellow`,
      `    } Else {`,
      `      Write-Host "  [OK] TextureQuality already at $oldVal (High or Very High) — no change needed" -ForegroundColor DarkGray`,
      `    }`,
      `  } Else {`,
      `    Write-Host "  [WARN] TextureQuality node not found in settings.xml — open GTA once to generate settings" -ForegroundColor Yellow`,
      `  }`,
      `} Else {`,
      `  Write-Host "  [WARN] GTA V settings.xml not found at default path — open GTA V once to generate it" -ForegroundColor Yellow`,
      `  Write-Host "         ($settingsXml)" -ForegroundColor DarkGray`,
      `}`,
      ``,
      `Write-Host ""`,
      `Write-Host "  =====================================================" -ForegroundColor DarkRed`,
      `Write-Host "  DONE — All 5 fixes applied!" -ForegroundColor Green`,
      `Write-Host ""`,
      `Write-Host "  What was fixed:" -ForegroundColor White`,
      `Write-Host "  [1] GPU driver texture filtering forced to High Quality (sharper skin samples)" -ForegroundColor Gray`,
      `Write-Host "  [2] FiveM commandline: larger streaming pool + ped/veh shader quality flags" -ForegroundColor Gray`,
      `Write-Host "  [3] FiveM texture/shader cache wiped (forces clean rebuild on next launch)" -ForegroundColor Gray`,
      `Write-Host "  [4] Scanned for conflicting ped texture mod files" -ForegroundColor Gray`,
      `Write-Host "  [5] GTA V settings.xml patched — TextureQuality set to Very High (VRAM only, no FPS loss)" -ForegroundColor Gray`,
      `Write-Host ""`,
      `Write-Host "  NEXT STEP: Launch FiveM — it will rebuild its cache (~1 min longer first launch)" -ForegroundColor Yellow`,
      `Write-Host "  Your gameplay FPS settings are untouched. Only texture quality was raised." -ForegroundColor Green`,
      `Write-Host ""`,
      `Write-Host "  NOTE: If you applied 'High Performance texture filter' in Opti Gods NVIDIA/AMD tab," -ForegroundColor Yellow`,
      `Write-Host "        this fix overrides it — required for correct face quality." -ForegroundColor Yellow`,
      `Write-Host ""`,
      `Write-Host "  Opti Gods by leaq -- discord.gg/optigods" -ForegroundColor DarkRed`,
      `Write-Host ""`,
      `Read-Host "Press Enter to close"`,
    ...
    const batLines = [
      ...
      `setlocal`,
      `set "SELF=%~f0"`,
      `set "TMPPS1=%TEMP%\\OptiGods-MushyFaceFix.ps1"`,
      ``,
      `title Opti Gods by leaq  --  Fix Mushy Face / Arm Textures`,
      `echo.`,
      `echo  =====================================================`,
      `echo    OPTI GODS by leaq  --  Fix Mushy Face / Arm Textures`,
      `echo  =====================================================`,
      `echo.`,
      `echo  [1/2] Extracting fix script...`,
      `PowerShell -NoProfile -ExecutionPolicy Bypass -Command "$c=[IO.File]::ReadAllText($env:SELF,[Text.Encoding]::UTF8);$m='##PS1'+'_START##';$i=$c.IndexOf($m);if($i -ge 0){[IO.File]::WriteAllText($env:TMPPS1,$c.Substring($i+$m.Length),[Text.Encoding]::UTF8)}"`,
    ...
    res.setHeader('Content-Type', 'application/octet-stream');
    res.setHeader('Content-Disposition', 'attachment; filename="OptiGods_MushyFace_Fix.bat"');
    res.end(Buffer.from(script, 'utf8'));
  });
