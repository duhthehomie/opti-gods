...
function buildScript(enabledTweaks: string[], nvidiaPreset?: string): string {
  const scriptLines: string[] = [
    ...
    `    # Use $PSCommandPath — always populated when running a .ps1 file (unlike $MyInvocation.MyCommand.Path which is empty on right-click Run)`,
    `    if (\$PSCommandPath) {`,
    `        Start-Process PowerShell -Verb RunAs -ArgumentList "-NoProfile -ExecutionPolicy Bypass -File \`"\$PSCommandPath\`"" ; exit`,
    `    } else {`,
    `        Write-Host "[UAC] Could not detect script path. Right-click the .ps1 file and choose 'Run with PowerShell'." -ForegroundColor Red`,
