...
const TWEAK_COMMANDS: Record<string, string> = {
  ...
  FiveMMenuFpsUncap: `$cfgDir = "$env:APPDATA\\CitizenFX"; If (!(Test-Path $cfgDir)) { New-Item $cfgDir -ItemType Directory -Force | Out-Null }; $autoexec = "$cfgDir\\autoexec.cfg"; $content = If (Test-Path $autoexec) { Get-Content $autoexec -Raw -Encoding UTF8 } Else { "" }; If ($content -match 'fps_max') { $content = $content -replace 'fps_max\s+\S+', 'fps_max 0' } Else { $content = $content.TrimEnd() + [System.Environment]::NewLine + "fps_max 0" }; Set-Content $autoexec $content.Trim() -Encoding UTF8; Write-Host "[FiveM] autoexec.cfg: fps_max 0 written — menu and in-game FPS uncapped on every launch." -ForegroundColor Green; $gpuClass = 'HKLM:\\SYSTEM\\CurrentControlSet\\Control\\Class\\{4d36e968-e325-11ce-bfc1-08002be10318}'; $found = $false; 0,1,2,3 | ForEach-Object { $k = "$gpuClass\\000$_"; If ((Test-Path $k) -and (Get-ItemProperty $k -Name 'DriverDesc' -EA SilentlyContinue).DriverDesc -match 'NVIDIA') { Set-ItemProperty $k 'OpenGLCompatibilityMode' 0 -Type DWord -Force -EA SilentlyContinue; Write-Host "[NVIDIA] OpenGL GDI Compatibility = Prefer Performance (secondary layer for older driver behaviour)" -ForegroundColor DarkGray; $found = $true } }; If (-not $found) { Write-Host "[NVIDIA] GPU class key not found — NVIDIA secondary layer skipped (non-NVIDIA system)" -ForegroundColor DarkGray }`,
