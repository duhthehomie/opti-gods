...
const TWEAK_COMMANDS: Record<string, string> = {
  ...
  DebloatXboxApp: `$pkg = Get-AppxPackage -AllUsers *XboxApp* -EA SilentlyContinue; if ($pkg) { $pkg | Remove-AppxPackage -AllUsers -EA SilentlyContinue; Write-Host "[OK] Xbox App removed" -ForegroundColor Green } else { Write-Host "[SKIP] Xbox App not installed" -ForegroundColor DarkGray }`,
  DebloatXboxGameBar: `$pkg = Get-AppxPackage -AllUsers *Microsoft.XboxGamingOverlay* -EA SilentlyContinue; if ($pkg) { $pkg | Remove-AppxPackage -AllUsers -EA SilentlyContinue; Write-Host "[OK] Xbox Game Bar removed" -ForegroundColor Green } else { Write-Host "[SKIP] Xbox Game Bar not installed" -ForegroundColor DarkGray }; Set-ItemProperty -Path 'HKLM:\\SOFTWARE\\Policies\\Microsoft\\Windows\\GameDVR' -Name 'AllowGameDVR' -Value 0 -Type DWord -EA SilentlyContinue`,
  DebloatXboxIdentity: `$pkg = Get-AppxPackage -AllUsers *XboxIdentityProvider* -EA SilentlyContinue; if ($pkg) { $pkg | Remove-AppxPackage -AllUsers -EA SilentlyContinue; Write-Host "[OK] Xbox Identity Provider removed" -ForegroundColor Green } else { Write-Host "[SKIP] Xbox Identity Provider not installed" -ForegroundColor DarkGray }`,
  ...
  DebloatWeather: `$pkg = Get-AppxPackage -AllUsers *BingWeather* -EA SilentlyContinue; if ($pkg) { $pkg | Remove-AppxPackage -AllUsers -EA SilentlyContinue; Write-Host "[OK] MSN Weather removed" -ForegroundColor Green } else { Write-Host "[SKIP] Weather app not installed" -ForegroundColor DarkGray }`,
  DebloatNews: `$pkg = Get-AppxPackage -AllUsers *BingNews* -EA SilentlyContinue; if ($pkg) { $pkg | Remove-AppxPackage -AllUsers -EA SilentlyContinue; Write-Host "[OK] MSN News removed" -ForegroundColor Green } else { Write-Host "[SKIP] News app not installed" -ForegroundColor DarkGray }`,
  DebloatMaps: `$pkg = Get-AppxPackage -AllUsers *WindowsMaps* -EA SilentlyContinue; if ($pkg) { $pkg | Remove-AppxPackage -AllUsers -EA SilentlyContinue; Write-Host "[OK] Windows Maps removed" -ForegroundColor Green } else { Write-Host "[SKIP] Maps not installed" -ForegroundColor DarkGray }`,
  DebloatSolitaire: `$pkg = Get-AppxPackage -AllUsers *MicrosoftSolitaireCollection* -EA SilentlyContinue; if ($pkg) { $pkg | Remove-AppxPackage -AllUsers -EA SilentlyContinue; Write-Host "[OK] Microsoft Solitaire Collection removed" -ForegroundColor Green } else { Write-Host "[SKIP] Solitaire not installed" -ForegroundColor DarkGray }`,
  DebloatMixedReality: `$pkg = Get-AppxPackage -AllUsers *MixedReality* -EA SilentlyContinue; if ($pkg) { $pkg | Remove-AppxPackage -AllUsers -EA SilentlyContinue; Write-Host "[OK] Mixed Reality Portal removed" -ForegroundColor Green } else { Write-Host "[SKIP] Mixed Reality not installed" -ForegroundColor DarkGray }`,
  DebloatSkype: `$pkg = Get-AppxPackage -AllUsers *SkypeApp* -EA SilentlyContinue; if ($pkg) { $pkg | Remove-AppxPackage -AllUsers -EA SilentlyContinue; Write-Host "[OK] Skype app removed" -ForegroundColor Green } else { Write-Host "[SKIP] Skype app not installed" -ForegroundColor DarkGray }`,
  DebloatZune: `$zm = Get-AppxPackage -AllUsers *ZuneMusic* -EA SilentlyContinue; if ($zm) { $zm | Remove-AppxPackage -AllUsers -EA SilentlyContinue }; $zv = Get-AppxPackage -AllUsers *ZuneVideo* -EA SilentlyContinue; if ($zv) { $zv | Remove-AppxPackage -AllUsers -EA SilentlyContinue }; Write-Host "[OK] Groove Music / Movies & TV removed" -ForegroundColor Green`,
  DebloatOfficeHub: `$pkg = Get-AppxPackage -AllUsers *MicrosoftOfficeHub* -EA SilentlyContinue; if ($pkg) { $pkg | Remove-AppxPackage -AllUsers -EA SilentlyContinue; Write-Host "[OK] Office Hub removed" -ForegroundColor Green } else { Write-Host "[SKIP] Office Hub not installed" -ForegroundColor DarkGray }`,
  DebloatFeedback: `$pkg = Get-AppxPackage -AllUsers *WindowsFeedbackHub* -EA SilentlyContinue; if ($pkg) { $pkg | Remove-AppxPackage -AllUsers -EA SilentlyContinue; Write-Host "[OK] Windows Feedback Hub removed" -ForegroundColor Green } else { Write-Host "[SKIP] Feedback Hub not installed" -ForegroundColor DarkGray }`,
  DebloatGetHelp: `$pkg = Get-AppxPackage -AllUsers *GetHelp* -EA SilentlyContinue; if ($pkg) { $pkg | Remove-AppxPackage -AllUsers -EA SilentlyContinue; Write-Host "[OK] Get Help removed" -ForegroundColor Green } else { Write-Host "[SKIP] Get Help not installed" -ForegroundColor DarkGray }`,
  DebloatGrooveMusic: `$pkg = Get-AppxPackage -AllUsers *ZuneMusic* -EA SilentlyContinue; if ($pkg) { $pkg | Remove-AppxPackage -AllUsers -EA SilentlyContinue; Write-Host "[OK] Groove Music removed" -ForegroundColor Green } else { Write-Host "[SKIP] Groove Music not installed" -ForegroundColor DarkGray }`,
  DebloatMSPaint3D: `$pkg = Get-AppxPackage -AllUsers *Microsoft.MSPaint* -EA SilentlyContinue; if ($pkg) { $pkg | Remove-AppxPackage -AllUsers -EA SilentlyContinue; Write-Host "[OK] Paint 3D removed" -ForegroundColor Green } else { Write-Host "[SKIP] Paint 3D not installed" -ForegroundColor DarkGray }`,
  DebloatWindowsCamera: `$pkg = Get-AppxPackage -AllUsers *WindowsCamera* -EA SilentlyContinue; if ($pkg) { $pkg | Remove-AppxPackage -AllUsers -EA SilentlyContinue; Write-Host "[OK] Windows Camera removed" -ForegroundColor Green } else { Write-Host "[SKIP] Windows Camera not installed" -ForegroundColor DarkGray }`,
  DebloatYourPhone: `$pkg = Get-AppxPackage -AllUsers *YourPhone* -EA SilentlyContinue; $pkg2 = Get-AppxPackage -AllUsers *PhoneLink* -EA SilentlyContinue; if ($pkg) { $pkg | Remove-AppxPackage -AllUsers -EA SilentlyContinue }; if ($pkg2) { $pkg2 | Remove-AppxPackage -AllUsers -EA SilentlyContinue }; Write-Host "[OK] Your Phone / Phone Link removed" -ForegroundColor Green`,
...
"@; Add-Type $code; [MemoryHelper]::EmptyWorkingSet([IntPtr](-1))`,
  ...
  // New Debloat
  DebloatClipchamp: `$pkg = Get-AppxPackage -AllUsers *Clipchamp* -EA SilentlyContinue; if ($pkg) { $pkg | Remove-AppxPackage -AllUsers -EA SilentlyContinue; Write-Host "[OK] Clipchamp removed" -ForegroundColor Green } else { Write-Host "[SKIP] Clipchamp not installed" -ForegroundColor DarkGray }`,
  DebloatPowerAutomate: `$pkg = Get-AppxPackage -AllUsers *PowerAutomate* -EA SilentlyContinue; if ($pkg) { $pkg | Remove-AppxPackage -AllUsers -EA SilentlyContinue; Write-Host "[OK] Power Automate removed" -ForegroundColor Green } else { Write-Host "[SKIP] Power Automate not installed" -ForegroundColor DarkGray }`,
  DebloatQuickAssist: `$pkg = Get-AppxPackage -AllUsers *QuickAssist* -EA SilentlyContinue; if ($pkg) { $pkg | Remove-AppxPackage -AllUsers -EA SilentlyContinue; Write-Host "[OK] Quick Assist removed" -ForegroundColor Green } else { Write-Host "[SKIP] Quick Assist not installed" -ForegroundColor DarkGray }`,
  ...
  DebloatAlarmsAndClock: `$pkg = Get-AppxPackage -AllUsers *WindowsAlarms* -EA SilentlyContinue; if ($pkg) { $pkg | Remove-AppxPackage -AllUsers -EA SilentlyContinue; Write-Host "[OK] Alarms & Clock removed" -ForegroundColor Green } else { Write-Host "[SKIP] Alarms & Clock not installed" -ForegroundColor DarkGray }`,
  // New Services
...
function buildScript(enabledTweaks: string[], nvidiaPreset?: string): string {
  const scriptLines: string[] = [
    ...
    `    Write-Host "   OPTI GODS — Needs Administrator" -ForegroundColor Red`,
    `    Write-Host "  =============================================" -ForegroundColor Red`,
    `    Write-Host "" `,
    `    Write-Host "  1. Close this window" -ForegroundColor Yellow`,
    `    Write-Host "  2. Right-click the .ps1 file" -ForegroundColor Yellow`,
    `    Write-Host "  3. Select 'Run with PowerShell' (it will ask for admin)" -ForegroundColor Yellow`,
    `    Write-Host "     -- OR --" -ForegroundColor DarkGray`,
    `    Write-Host "  Use the .BAT version instead (auto-elevates on double-click)" -ForegroundColor Cyan`,
    `    Write-Host "" `,
