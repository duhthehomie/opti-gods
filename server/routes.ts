...
const TWEAK_COMMANDS: Record<string, string> = {
  ...
  SecDisableMitigationsForGames: `@('FortniteClient-Win64-Shipping.exe','cs2.exe','VALORANT-Win64-Shipping.exe','r5apex.exe','GTA5.exe','FiveM.exe','Overwatch.exe') | ForEach-Object { \$p = "HKLM:\\SOFTWARE\\Microsoft\\Windows NT\\CurrentVersion\\Image File Execution Options\\\$_"; If (!(Test-Path \$p)) { New-Item \$p -Force | Out-Null }; Set-ItemProperty \$p 'MitigationOptions' ([byte[]](0x22,0x02,0,0,0,0,0,0)) -Type Binary -Force }; Write-Host "[OK] Exploit mitigations disabled for known game exes" -ForegroundColor Green`,
...
const RESTORE_BLOCKS: Record<string, { label: string; commands: string[] }> = {
  ...
  nvidia: {
    ...
    commands: [
      ...
      // EnableMSIMode_Safe rollback — matches the apply path exactly:
      // walks Enum\<InstanceId>\Device Parameters\Interrupt Management\Affinity Policy
      // for every display device, wipes the V1-BSOD trigger values, and
      // (for the rollback) ALSO sets MSISupported=0 to revert MSI mode.
      `Write-Host "[Fixes] Rolling back EnableMSIMode_Safe on all display adapters..." -ForegroundColor Cyan; Get-PnpDevice -Class Display -EA SilentlyContinue | ForEach-Object { $msiPath = "HKLM:\\SYSTEM\\CurrentControlSet\\Enum\\$($_.InstanceId)\\Device Parameters\\Interrupt Management\\MessageSignaledInterruptProperties"; $affPath = "HKLM:\\SYSTEM\\CurrentControlSet\\Enum\\$($_.InstanceId)\\Device Parameters\\Interrupt Management\\Affinity Policy"; If (Test-Path $msiPath) { Set-ItemProperty -Path $msiPath -Name 'MSISupported' -Value 0 -Type DWord -Force -EA SilentlyContinue }; If (Test-Path $affPath) { Remove-ItemProperty -Path $affPath -Name 'DevicePolicy' -EA SilentlyContinue; Remove-ItemProperty -Path $affPath -Name 'DevicePriority' -EA SilentlyContinue; Remove-ItemProperty -Path $affPath -Name 'AssignmentSetOverride' -EA SilentlyContinue }; Write-Host "[Fixes] $($_.FriendlyName) — MSI disabled, Affinity Policy wiped" -ForegroundColor Green }; Write-Host "[OK] EnableMSIMode_Safe rollback complete (NVIDIA Fixes block)" -ForegroundColor Green`,
    ],
  },
  amd: {
    label: "AMD",
    commands: [
      ...
      // walks Enum\<InstanceId>\Device Parameters\Interrupt Management for
      // every Display adapter, matching the apply command's exact path.
