...
Write-Host "[Intel 4th-8th Gen] High Performance plan active. Min/Max=100%, BoostMode=Aggressive, PowerThrottling disabled. All cores run at rated Turbo frequency throughout your gaming session — no ramp-up delay on first frame." -ForegroundColor Green`,
  ...
  CpuFortniteIFEO: `$ifeo = 'HKLM:\SOFTWARE\Microsoft\Windows NT\CurrentVersion\Image File Execution Options'; @('FortniteClient-Win64-Shipping.exe','FortniteLauncher.exe','EpicGamesLauncher.exe') | ForEach-Object { $k = "$ifeo\$_\PerfOptions"; If (!(Test-Path $k)) { New-Item $k -Force | Out-Null }; Set-ItemProperty $k 'CpuPriorityClass' 3 -Type DWord -Force; Set-ItemProperty $k 'CpuPriorityBoost' 1 -Type DWord -Force; Set-ItemProperty $k 'DisableEnergyThrottling' 1 -Type DWord -Force; Set-ItemProperty $k 'ForceForegroundBoost' 1 -Type DWord -Force; Set-ItemProperty $k 'IoPriority' 2 -Type DWord -Force; Set-ItemProperty $k 'PagePriority' 5 -Type DWord -Force; Set-ItemProperty $k 'GpuPriority' 8 -Type DWord -Force; Write-Host "[Fortnite] IFEO PerfOptions applied to $_ — AboveNormal CPU, boost ON, IO High, GPU Priority 8" -ForegroundColor Green }`,
  CpuCodIFEO: `$ifeo = 'HKLM:\SOFTWARE\Microsoft\Windows NT\CurrentVersion\Image File Execution Options'; @('cod.exe','BlackOpsColdWar.exe','ModernWarfare.exe','ModernWarfare2.exe','ModernWarfare3.exe','Warzone.exe','MWIII.exe') | ForEach-Object { $k = "$ifeo\$_\PerfOptions"; If (!(Test-Path $k)) { New-Item $k -Force | Out-Null }; Set-ItemProperty $k 'CpuPriorityClass' 3 -Type DWord -Force; Set-ItemProperty $k 'CpuPriorityBoost' 1 -Type DWord -Force; Set-ItemProperty $k 'DisableEnergyThrottling' 1 -Type DWord -Force; Set-ItemProperty $k 'ForceForegroundBoost' 1 -Type DWord -Force; Set-ItemProperty $k 'IoPriority' 2 -Type DWord -Force; Set-ItemProperty $k 'PagePriority' 5 -Type DWord -Force; Set-ItemProperty $k 'GpuPriority' 8 -Type DWord -Force; Write-Host "[COD] IFEO PerfOptions applied to $_ — AboveNormal CPU, boost ON, IO High" -ForegroundColor Green }`,
  CpuGenericGameIFEO: `$ifeo = 'HKLM:\SOFTWARE\Microsoft\Windows NT\CurrentVersion\Image File Execution Options'; @('cs2.exe','csgo.exe','valorant.exe','VALORANT-Win64-Shipping.exe','r5apex.exe','RainbowSix.exe','RainbowSix_BE.exe','EscapeFromTarkov.exe','DayZ_BE.exe','pubg.exe','RustClient.exe','Overwatch.exe') | ForEach-Object { $k = "$ifeo\$_\PerfOptions"; If (!(Test-Path $k)) { New-Item $k -Force | Out-Null }; Set-ItemProperty $k 'CpuPriorityClass' 3 -Type DWord -Force; Set-ItemProperty $k 'CpuPriorityBoost' 1 -Type DWord -Force; Set-ItemProperty $k 'DisableEnergyThrottling' 1 -Type DWord -Force; Set-ItemProperty $k 'ForceForegroundBoost' 1 -Type DWord -Force; Set-ItemProperty $k 'IoPriority' 2 -Type DWord -Force; Set-ItemProperty $k 'PagePriority' 5 -Type DWord -Force; Write-Host "[Games] IFEO AboveNormal + boost applied to $_ " -ForegroundColor Green }; Write-Host "[Games] Priority stack complete — CS2, Valorant, Apex, Tarkov, PUBG, R6, Rust, OW all boosted" -ForegroundColor Cyan`,
