...
Write-Host "[Intel 4th-8th Gen] High Performance plan active. Min/Max=100%, BoostMode=Aggressive, PowerThrottling disabled. All cores run at rated Turbo frequency throughout your gaming session — no ramp-up delay on first frame." -ForegroundColor Green`,
  ...
  NvFrameLimit30:     `@('HKLM:\\SOFTWARE\\NVIDIA Corporation\\Global\\NVTweak','HKCU:\\SOFTWARE\\NVIDIA Corporation\\Global\\NVTweak') | ForEach-Object { If (!(Test-Path $_)) { New-Item $_ -Force | Out-Null }; Set-ItemProperty -Path $_ -Name 'FrameRateLimitEnable' -Value 1 -Type DWord -Force -EA SilentlyContinue; Set-ItemProperty -Path $_ -Name 'FrameRateLimit' -Value 30 -Type DWord -Force -EA SilentlyContinue }; Write-Host "[NVIDIA] Driver-level frame rate cap = 30 FPS" -ForegroundColor Green`,
  NvFrameLimit60:     `@('HKLM:\\SOFTWARE\\NVIDIA Corporation\\Global\\NVTweak','HKCU:\\SOFTWARE\\NVIDIA Corporation\\Global\\NVTweak') | ForEach-Object { If (!(Test-Path $_)) { New-Item $_ -Force | Out-Null }; Set-ItemProperty -Path $_ -Name 'FrameRateLimitEnable' -Value 1 -Type DWord -Force -EA SilentlyContinue; Set-ItemProperty -Path $_ -Name 'FrameRateLimit' -Value 60 -Type DWord -Force -EA SilentlyContinue }; Write-Host "[NVIDIA] Driver-level frame rate cap = 60 FPS" -ForegroundColor Green`,
  NvFrameLimit120:    `@('HKLM:\\SOFTWARE\\NVIDIA Corporation\\Global\\NVTweak','HKCU:\\SOFTWARE\\NVIDIA Corporation\\Global\\NVTweak') | ForEach-Object { If (!(Test-Path $_)) { New-Item $_ -Force | Out-Null }; Set-ItemProperty -Path $_ -Name 'FrameRateLimitEnable' -Value 1 -Type DWord -Force -EA SilentlyContinue; Set-ItemProperty -Path $_ -Name 'FrameRateLimit' -Value 120 -Type DWord -Force -EA SilentlyContinue }; Write-Host "[NVIDIA] Driver-level frame rate cap = 120 FPS" -ForegroundColor Green`,
  NvFrameLimit144:    `@('HKLM:\\SOFTWARE\\NVIDIA Corporation\\Global\\NVTweak','HKCU:\\SOFTWARE\\NVIDIA Corporation\\Global\\NVTweak') | ForEach-Object { If (!(Test-Path $_)) { New-Item $_ -Force | Out-Null }; Set-ItemProperty -Path $_ -Name 'FrameRateLimitEnable' -Value 1 -Type DWord -Force -EA SilentlyContinue; Set-ItemProperty -Path $_ -Name 'FrameRateLimit' -Value 144 -Type DWord -Force -EA SilentlyContinue }; Write-Host "[NVIDIA] Driver-level frame rate cap = 144 FPS" -ForegroundColor Green`,
  NvFrameLimit240:    `@('HKLM:\\SOFTWARE\\NVIDIA Corporation\\Global\\NVTweak','HKCU:\\SOFTWARE\\NVIDIA Corporation\\Global\\NVTweak') | ForEach-Object { If (!(Test-Path $_)) { New-Item $_ -Force | Out-Null }; Set-ItemProperty -Path $_ -Name 'FrameRateLimitEnable' -Value 1 -Type DWord -Force -EA SilentlyContinue; Set-ItemProperty -Path $_ -Name 'FrameRateLimit' -Value 240 -Type DWord -Force -EA SilentlyContinue }; Write-Host "[NVIDIA] Driver-level frame rate cap = 240 FPS" -ForegroundColor Green`,
  NvFrameLimitCustom: `$_fps = Read-Host "Enter frame rate limit (10-1000)"; if ($_fps -match '^\\d+$' -and [int]$_fps -ge 10 -and [int]$_fps -le 1000) { @('HKLM:\\SOFTWARE\\NVIDIA Corporation\\Global\\NVTweak','HKCU:\\SOFTWARE\\NVIDIA Corporation\\Global\\NVTweak') | ForEach-Object { If (!(Test-Path $_)) { New-Item $_ -Force | Out-Null }; Set-ItemProperty -Path $_ -Name 'FrameRateLimitEnable' -Value 1 -Type DWord -Force -EA SilentlyContinue; Set-ItemProperty -Path $_ -Name 'FrameRateLimit' -Value ([int]$_fps) -Type DWord -Force -EA SilentlyContinue }; Write-Host "[NVIDIA] Driver-level frame rate cap = $_fps FPS" -ForegroundColor Green } else { Write-Host "[ERROR] Invalid value — enter a number between 10 and 1000." -ForegroundColor Red }`,
  ...
  CpuFortniteIFEO: `$_c=(Get-CimInstance Win32_Processor -Property NumberOfCores|Measure-Object NumberOfCores -Sum).Sum; $_pri=if($_c -ge 6){4}else{3}; $ifeo = 'HKLM:\SOFTWARE\Microsoft\Windows NT\CurrentVersion\Image File Execution Options'; @('FortniteClient-Win64-Shipping.exe','FortniteLauncher.exe','EpicGamesLauncher.exe') | ForEach-Object { $k = "$ifeo\$_\PerfOptions"; If (!(Test-Path $k)) { New-Item $k -Force | Out-Null }; Set-ItemProperty $k 'CpuPriorityClass' $_pri -Type DWord -Force; Set-ItemProperty $k 'CpuPriorityBoost' 1 -Type DWord -Force; Set-ItemProperty $k 'DisableEnergyThrottling' 1 -Type DWord -Force; Set-ItemProperty $k 'ForceForegroundBoost' 1 -Type DWord -Force; Set-ItemProperty $k 'IoPriority' 2 -Type DWord -Force; Set-ItemProperty $k 'PagePriority' 5 -Type DWord -Force; Set-ItemProperty $k 'GpuPriority' 8 -Type DWord -Force; Write-Host "[Fortnite] IFEO applied to $_ — CPU=$(if($_pri-eq 4){'High'}else{'AboveNormal'}), boost ON, IO=2, GPU=8" -ForegroundColor Green }`,
  CpuCodIFEO: `$_c=(Get-CimInstance Win32_Processor -Property NumberOfCores|Measure-Object NumberOfCores -Sum).Sum; $_pri=if($_c -ge 6){4}else{3}; $ifeo = 'HKLM:\SOFTWARE\Microsoft\Windows NT\CurrentVersion\Image File Execution Options'; @('cod.exe','BlackOpsColdWar.exe','ModernWarfare.exe','ModernWarfare2.exe','ModernWarfare3.exe','Warzone.exe','MWIII.exe') | ForEach-Object { $k = "$ifeo\$_\PerfOptions"; If (!(Test-Path $k)) { New-Item $k -Force | Out-Null }; Set-ItemProperty $k 'CpuPriorityClass' $_pri -Type DWord -Force; Set-ItemProperty $k 'CpuPriorityBoost' 1 -Type DWord -Force; Set-ItemProperty $k 'DisableEnergyThrottling' 1 -Type DWord -Force; Set-ItemProperty $k 'ForceForegroundBoost' 1 -Type DWord -Force; Set-ItemProperty $k 'IoPriority' 2 -Type DWord -Force; Set-ItemProperty $k 'PagePriority' 5 -Type DWord -Force; Set-ItemProperty $k 'GpuPriority' 8 -Type DWord -Force; Write-Host "[COD] IFEO applied to $_ — CPU=$(if($_pri-eq 4){'High'}else{'AboveNormal'}), boost ON, IO=2, GPU=8" -ForegroundColor Green }`,
  CpuGenericGameIFEO: `$_c=(Get-CimInstance Win32_Processor -Property NumberOfCores|Measure-Object NumberOfCores -Sum).Sum; $_pri=if($_c -ge 6){4}else{3}; $ifeo = 'HKLM:\SOFTWARE\Microsoft\Windows NT\CurrentVersion\Image File Execution Options'; @('cs2.exe','csgo.exe','valorant.exe','VALORANT-Win64-Shipping.exe','r5apex.exe','RainbowSix.exe','RainbowSix_BE.exe','EscapeFromTarkov.exe','DayZ_BE.exe','pubg.exe','RustClient.exe','Overwatch.exe','ARC-Win64-Shipping.exe','MarvelRivals-Win64-Shipping.exe','ReadyOrNot.exe','BF2042.exe','RocketLeague.exe') | ForEach-Object { $k = "$ifeo\$_\PerfOptions"; If (!(Test-Path $k)) { New-Item $k -Force | Out-Null }; Set-ItemProperty $k 'CpuPriorityClass' $_pri -Type DWord -Force; Set-ItemProperty $k 'CpuPriorityBoost' 1 -Type DWord -Force; Set-ItemProperty $k 'DisableEnergyThrottling' 1 -Type DWord -Force; Set-ItemProperty $k 'ForceForegroundBoost' 1 -Type DWord -Force; Set-ItemProperty $k 'IoPriority' 2 -Type DWord -Force; Set-ItemProperty $k 'PagePriority' 5 -Type DWord -Force; Write-Host "[Games] IFEO CPU=$(if($_pri-eq 4){'High'}else{'AboveNormal'})+boost: $_" -ForegroundColor Green }; Write-Host "[Games] Priority stack complete — CS2, Valorant, Apex, ARC Raiders, Marvel Rivals, R6, Rust and more boosted" -ForegroundColor Cyan`,
...
`;
  ...
  app.get('/api/movies-tv-fix-script', (req, res) => {
    ...
    res.end(Buffer.from(wrapInBat(script, { title: 'Movies & TV Fix (Error 0x8007060e)', tmpName: 'OptiGods-MoviesTVFix', marker: 'MOVIES_TV_FIX_PS1_START' }), 'utf8'));
  });
