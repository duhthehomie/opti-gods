...
const TWEAK_COMMANDS: Record<string, string> = {
  ...
  FiveM1060AnselDisable: `@('NVDisplay.ContainerLocalSystem') | ForEach-Object { Stop-Service $_ -Force -EA SilentlyContinue; Set-Service $_ -StartupType Disabled -EA SilentlyContinue }; Set-ItemProperty -Path 'HKCU:\\SOFTWARE\\NVIDIA Corporation\\Ansel' -Name 'AnselEnable' -Value 0 -Type DWord -Force -EA SilentlyContinue; Write-Host "[GTX 1060] NVIDIA Ansel hook disabled — NVContainerLocalSystem stopped. Less per-frame GPU overhead in GTA V." -ForegroundColor Green`,
