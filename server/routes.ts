...
`;
  ...
  app.get('/api/mushy-face-fix-script', (req, res) => {
    const ps1Lines = [
      ...
      `Write-Host ""`,
      `Write-Host "  =====================================================" -ForegroundColor DarkRed`,
      `Write-Host "  DONE — All 4 fixes applied!" -ForegroundColor Green`,
      `Write-Host ""`,
      `Write-Host "  What was fixed:" -ForegroundColor White`,
      ...
      `Write-Host ""`,
      `Write-Host "  NEXT STEP: Launch FiveM — it will rebuild its cache (may take ~1 min longer)" -ForegroundColor Yellow`,
      `Write-Host "  IMPORTANT: In GTA settings set Texture Quality to VERY HIGH before joining a server" -ForegroundColor Red`,
      `Write-Host ""`,
      `Write-Host "  NOTE: If you applied 'High Performance texture filter' in Opti Gods NVIDIA/AMD tab," -ForegroundColor Yellow`,
      `Write-Host "        this fix overrides it — small FPS trade for correct face quality." -ForegroundColor Yellow`,
      `Write-Host ""`,
      `Write-Host "  Opti Gods by leaq -- discord.gg/optigods" -ForegroundColor DarkRed`,
      `Write-Host ""`,
      `Read-Host "Press Enter to close"`,
