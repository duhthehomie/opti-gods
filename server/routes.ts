...
`;
  ...
  app.get('/api/rocket-league-fix-script', (req, res) => {
    const lines = [
      `# ============================================================`,
      `#  Opti Gods — Rocket League Advanced Recovery Fix  (by leaq)`,
      `#  Fixes: Game won't start, shows "Running" then hangs/closes`,
      `#  Method: Nuclear reset + validation (resets all configs)`,
      `# ============================================================`,
      ``,
      `# ── Auto-elevate to Administrator (UAC prompt will appear) ──`,
      `If (-not ([Security.Principal.WindowsPrincipal][Security.Principal.WindowsIdentity]::GetCurrent()).IsInRole([Security.Principal.WindowsBuiltInRole]'Administrator')) {`,
      ...
      `}`,
      ``,
      `# ── Unblock this script file (remove Zone.Identifier) ──`,
      `Unblock-File -Path "\$MyInvocation.MyCommand.Definition" -Confirm:\$false -EA SilentlyContinue`,
      ``,
      `\$ErrorActionPreference = 'SilentlyContinue'`,
      ``,
      `Write-Host ""`,
      ...
      `} Else {`,
      `    Write-Host "  Rocket League executable not found." -ForegroundColor Red`,
      `    Write-Host "  Install location: Program Files\\Epic Games\\Rocket League" -ForegroundColor Yellow`,
      `}`,
      ``,
      `# -- STEP 4: Verify/install DirectX ----`,
      `Write-Host "[STEP 4] Checking DirectX..." -ForegroundColor Yellow`,
      `\$dxdiag = & dxdiag /t "\$env:Temp\\dxdiag_out.txt" /e 2>&1`,
      `Start-Sleep -Seconds 3`,
      `Write-Host "  DirectX diagnostic run. (DirectX should be 12 or later)" -ForegroundColor Cyan`,
      ``,
      ...
      `Remove-Item "\$env:AppData\\Local\\Temp\\RL*" -Recurse -Force -EA SilentlyContinue`,
      `Write-Host "  Temp files cleared." -ForegroundColor Green`,
      ``,
      `# -- STEP 7: Run system file checker ----`,
      `Write-Host "[STEP 7] Checking system files (this may take 5-10 minutes)..." -ForegroundColor Yellow`,
      `sfc /scannow`,
      ...
      `Write-Host "  -------------------------------------------------" -ForegroundColor DarkGray`,
      `Write-Host ""`,
      `Write-Host "  IMPORTANT: You MUST restart your PC now!" -ForegroundColor Red`,
      `Write-Host ""`,
      `Write-Host "  After restart:" -ForegroundColor White`,
      `Write-Host "  1. Launch Rocket League from Epic Games" -ForegroundColor Yellow`,
      `Write-Host "  2. Wait for it to re-download/validate shaders (~2-5 min)" -ForegroundColor Yellow`,
      `Write-Host ""`,
      `Write-Host "  If STILL not working:" -ForegroundColor Yellow`,
      `Write-Host "  • Uninstall Rocket League completely via Epic Games" -ForegroundColor DarkGray`,
      `Write-Host "  • Delete the folder: \$env:ProgramFiles\\Epic Games\\Rocket League" -ForegroundColor DarkGray`,
      `Write-Host "  • Reinstall from Epic Games (fresh install)" -ForegroundColor DarkGray`,
