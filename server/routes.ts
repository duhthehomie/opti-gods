...
const TWEAK_COMMANDS: Record<string, string> = {
  ...
  FiveMGPUPriorityStack: `$ifeo = 'HKLM:\\SOFTWARE\\Microsoft\\Windows NT\\CurrentVersion\\Image File Execution Options'; $applyGPU = { param($exe) $k = "$ifeo\\$exe\\PerfOptions"; If (!(Test-Path $k)) { New-Item $k -Force | Out-Null }; Set-ItemProperty $k 'GPU Priority' 8 -Type DWord -Force; Set-ItemProperty $k 'GpuPriorityClass' 8 -Type DWord -Force; Set-ItemProperty $k 'GpuMax' 256 -Type DWord -Force; Set-ItemProperty $k 'GpuMaxPerformance' 256 -Type DWord -Force; Set-ItemProperty $k 'GpuRenderingPriority' 3 -Type DWord -Force; Set-ItemProperty $k 'GpuThrottling' 0 -Type DWord -Force; Write-Host "[FiveM] GPU priority stack applied to $exe" -ForegroundColor Green }; @('FiveM.exe','GTA5.exe','FiveM_b2189_GTAProcess.exe','FiveM_b2545_GTAProcess.exe','FiveM_b2612_GTAProcess.exe','FiveM_b2699_GTAProcess.exe','FiveM_b2802_GTAProcess.exe','FiveM_b2944_GTAProcess.exe','FiveM_b3095_GTAProcess.exe','FiveM_b3258_GTAProcess.exe','FiveM_b3323_GTAProcess.exe','FiveM_b3407_GTAProcess.exe','FiveM_b3441_GTAProcess.exe') | ForEach-Object { & $applyGPU $_ }; $gamesPath = 'HKLM:\\SOFTWARE\\Microsoft\\Windows NT\\CurrentVersion\\Multimedia\\SystemProfile\\Tasks\\Games'; If (!(Test-Path $gamesPath)) { New-Item $gamesPath -Force | Out-Null }; Set-ItemProperty $gamesPath 'GPU Priority' 8 -Type DWord -Force; Set-ItemProperty $gamesPath 'MaximumPreRenderedFrames' 1 -Type DWord -Force; Write-Host "[FiveM] GPU Priority=8 applied to all 13 FiveM/GTA5 processes + MMCSS Games profile" -ForegroundColor Green`,
  ...
  RegistryDPCLatency: `Write-Host "⚠  WARNING: NOT IDEAL FOR ALL SYSTEMS — Disabling GPU preemption (EnablePreemption=0) can cause GPU hangs or a black screen on some driver/hardware combos, especially under heavy multi-GPU or compute workloads. Also: disabledynamictick increases idle CPU usage slightly. If your PC becomes unstable after this tweak, run: bcdedit /set disabledynamictick no and bcdedit /set useplatformclock true, then reboot." -ForegroundColor Yellow; $storahci = 'HKLM:\\SYSTEM\\CurrentControlSet\\Services\\storahci\\Parameters\\Device'; If (!(Test-Path $storahci)) { New-Item $storahci -Force | Out-Null }; Set-ItemProperty $storahci -Name 'TreatAsInternalPort' -Value ([byte[]](0,0,0,0,0,0,0,0)) -Type Binary -Force -EA SilentlyContinue; bcdedit /set useplatformclock false 2>nul; bcdedit /set tscsyncpolicy enhanced 2>nul; bcdedit /set disabledynamictick yes 2>nul; Set-ItemProperty -Path 'HKLM:\\SYSTEM\\CurrentControlSet\\Control\\GraphicsDrivers\\Scheduler' -Name 'EnablePreemption' -Value 0 -Type DWord -Force -EA SilentlyContinue; Write-Host "[DPC] AHCI link power off, TSC clock, dynamic tick off, GPU preemption off — DPC latency reduced." -ForegroundColor Green`,
...
const RESTORE_BLOCKS: Record<string, { label: string; commands: string[] }> = {
  ...
  fivem: {
    ...
    commands: [
      ...
      `# FIX: LargeSystemCache=1 was causing FiveM_GTAProcess.exe to crash with "memory at 0xDEED could not be written"`,
      `# LargeSystemCache=1 is a Windows Server setting that aggressively trims game process working sets,`,
      `# causing write-to-dead-memory (0xDEED is Windows heap sentinel for trimmed pages). Set back to 0 for gaming.`,
      `$memPath = 'HKLM:\\SYSTEM\\CurrentControlSet\\Control\\Session Manager\\Memory Management'`,
      `$oldLSC = (Get-ItemProperty $memPath -Name LargeSystemCache -EA SilentlyContinue).LargeSystemCache`,
      `Set-ItemProperty -Path $memPath -Name 'LargeSystemCache' -Value 0 -Type DWord -Force -EA SilentlyContinue`,
      `Write-Host "[OK] LargeSystemCache: $oldLSC → 0  (gaming mode — fixes GTA process memory write crash)" -ForegroundColor Green`,
      `# FIX: Memory Compression disabled was causing FiveM_ChromeBrowser to crash with exception 0xe0000008`,
      `# Chromium's PartitionAlloc heap allocator fails fatally under memory pressure when the OS cannot compress`,
      `# pages as a safety valve. Re-enabling lets Windows manage memory pressure safely for CEF.`,
      `Enable-MMAgent -MemoryCompression -EA SilentlyContinue`,
      `Write-Host "[OK] Windows Memory Compression re-enabled (fixes FiveM_ChromeBrowser 0xe0000008 CEF crash)" -ForegroundColor Green`,
...
`;
  ...
  // Public — Rocket League crash/won't start fix
  app.get('/api/rocket-league-fix-script', (req, res) => {
    const lines = [
      `# ============================================================`,
      `#  Opti Gods — Rocket League Won't Start Fix  (by leaq)`,
      `#  Fixes: Game shows "Running" but won't launch, no task manager entry`,
      `# ============================================================`,
      ``,
      `# ── Auto-elevate to Administrator (UAC prompt will appear) ──`,
      `If (-not ([Security.Principal.WindowsPrincipal][Security.Principal.WindowsIdentity]::GetCurrent()).IsInRole([Security.Principal.WindowsBuiltInRole]'Administrator')) {`,
      ...
      ``,
      `Write-Host ""`,
      `Write-Host "  Opti Gods - Rocket League Fix" -ForegroundColor Red`,
      `Write-Host "  ==============================" -ForegroundColor DarkRed`,
      `Write-Host ""`,
      ``,
      `# -- FIX 1: Remove fullscreen optimizations (causes alt-tab crashes) ----`,
      `Write-Host "[FIX 1] Checking Rocket League executable..." -ForegroundColor Yellow`,
      `\$rlPaths = @(`,
      `    "\$env:ProgramFiles\\Epic Games\\Rocket League\\Binaries\\Win64\\RocketLeague.exe",`,
      `    "\$env:ProgramFiles(x86)\\Epic Games\\Rocket League\\Binaries\\Win64\\RocketLeague.exe"`,
      `)`,
      `\$rlExe = \$rlPaths | Where-Object { Test-Path \$_ } | Select-Object -First 1`,
      `If (\$rlExe) {`,
      `    Write-Host "  Found: \$rlExe" -ForegroundColor Cyan`,
      `    \$regPath = "HKCU:\\Software\\Microsoft\\Windows NT\\CurrentVersion\\AppCompatFlags\\Layers"`,
      `    \$regValue = (Get-ItemProperty \$regPath -Name \$rlExe -EA SilentlyContinue).\$rlExe`,
      `    If (\$regValue) {`,
      `        Write-Host "  Removing compatibility flags..." -ForegroundColor Yellow`,
      `        Remove-ItemProperty -Path \$regPath -Name \$rlExe -EA SilentlyContinue`,
      `        Write-Host "  Cleared: \$regValue" -ForegroundColor Green`,
      `    } Else {`,
      `        Write-Host "  No compatibility issues found." -ForegroundColor Green`,
      `    }`,
      `} Else {`,
      `    Write-Host "  Rocket League not found at standard location." -ForegroundColor Yellow`,
      `    Write-Host "  Please verify it's installed in Program Files." -ForegroundColor Yellow`,
      `}`,
      ``,
      `# -- FIX 2: Enable Game Mode (helps with stability) ----`,
      `Write-Host "[FIX 2] Checking Game Mode..." -ForegroundColor Yellow`,
      `\$gameModeKey = "HKCU:\\SOFTWARE\\Microsoft\\GameBarExplorer"`,
      `\$useGameMode = (Get-ItemProperty \$gameModeKey -Name UseGameMode -EA SilentlyContinue).UseGameMode`,
      `If (\$useGameMode -eq 0) {`,
      `    Write-Host "  Game Mode is OFF - enabling..." -ForegroundColor Yellow`,
      `    Set-ItemProperty -Path \$gameModeKey -Name 'UseGameMode' -Value 1 -Type DWord -Force`,
      `    Write-Host "  Game Mode enabled." -ForegroundColor Green`,
      `} Else {`,
      `    Write-Host "  Game Mode is already ON." -ForegroundColor Green`,
      `}`,
      ``,
      `# -- FIX 3: Verify GPU drivers are accessible ----`,
      `Write-Host "[FIX 3] Checking GPU access..." -ForegroundColor Yellow`,
      `\$gpuTest = Get-WmiObject Win32_VideoController`,
      `If (\$gpuTest) {`,
      `    \$gpuName = \$gpuTest[0].Name`,
      `    Write-Host "  Detected GPU: \$gpuName" -ForegroundColor Green`,
      `} Else {`,
      `    Write-Host "  No GPU detected - this could be the issue." -ForegroundColor Red`,
      `    Write-Host "  Update your graphics drivers from NVIDIA/AMD." -ForegroundColor Yellow`,
      `}`,
      ``,
      `# -- FIX 4: Clear Rocket League cache (old settings might be corrupted) ----`,
      `Write-Host "[FIX 4] Clearing game cache..." -ForegroundColor Yellow`,
      `\$cacheDir = "\$env:LocalAppData\\Rocket League\\Saved"`,
      `If (Test-Path \$cacheDir) {`,
      `    try {`,
      `        Remove-Item "\$cacheDir\\Cache" -Recurse -Force -EA SilentlyContinue`,
      `        Write-Host "  Cache cleared." -ForegroundColor Green`,
      `    } catch {`,
      `        Write-Host "  Could not clear cache (game may be open). Try closing Rocket League." -ForegroundColor Yellow`,
      `    }`,
      `} Else {`,
      ...
      `Write-Host "  -------------------------------------------------" -ForegroundColor DarkGray`,
      `Write-Host "  1. Restart your PC (important!)" -ForegroundColor Yellow`,
      `Write-Host "  2. Launch Rocket League from Epic Games" -ForegroundColor Yellow`,
      `Write-Host "  3. If still crashing:" -ForegroundColor Yellow`,
      `Write-Host "     - Update GPU drivers (NVIDIA/AMD/Intel)" -ForegroundColor DarkGray`,
      `Write-Host "     - Disable any overlay software (Discord, OBS, etc.)" -ForegroundColor DarkGray`,
      `Write-Host ""`,
      `Write-Host "  Opti Gods by leaq" -ForegroundColor DarkRed`,
      `Write-Host ""`,
      `pause`,
    ...
    const script = lines.join('\r\n');
    res.setHeader('Content-Type', 'application/octet-stream');
    res.setHeader('Content-Disposition', 'attachment; filename="OptiGods-RocketLeagueFix-by-leaq.ps1"');
    res.end(Buffer.concat([Buffer.from('\ufeff', 'utf8'), Buffer.from(script, 'utf8')]));
  });
