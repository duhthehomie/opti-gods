...
function normalizeWindowsPowerShellCommand(id: string, command: string): string {
  ...
  const unsafe = "$_steamReg=(Get-ItemProperty 'HKCU:\\Software\\Valve\\Steam' -EA SilentlyContinue)?.SteamPath;";
  const safe = "$_steamProps=Get-ItemProperty 'HKCU:\\Software\\Valve\\Steam' -EA SilentlyContinue; $_steamReg=$null; if($_steamProps){$_steamReg=$_steamProps.SteamPath};";
  return command.replace(unsafe, safe);
}
