...
Write-Host "[Intel 4th-8th Gen] High Performance plan active. Min/Max=100%, BoostMode=Aggressive, PowerThrottling disabled. All cores run at rated Turbo frequency throughout your gaming session — no ramp-up delay on first frame." -ForegroundColor Green`,
  ...
  AmdCpuCStatePolicy: `$settingPath = 'HKLM:\\SYSTEM\\CurrentControlSet\\Control\\Power\\PowerSettings\\54533251-82be-4824-96c1-47b60b740d00\\40fbefc7-2e9d-4d25-a185-0cfd8574bae6'; $scheme = (powercfg /getactivescheme 2>$null); $applied = $false; if ($scheme -match '([0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12})') { $g = $matches[1]; powercfg /setacvalueindex $g 54533251-82be-4824-96c1-47b60b740d00 40fbefc7-2e9d-4d25-a185-0cfd8574bae6 0 2>$null; $applied = ($LASTEXITCODE -eq 0); powercfg /setactive $g 2>$null }; if (Test-Path -LiteralPath $settingPath) { Set-ItemProperty -LiteralPath $settingPath -Name 'Attributes' -Value 2 -Type DWord -Force -EA SilentlyContinue; $applied = $true }; if ($applied) { Write-Host "[AMD CPU] CPU performance decrease policy set to Fastest — clock drops between frames eliminated" -ForegroundColor Green } else { Write-Host "[SKIP] AMD CPU C-State policy is not exposed by this Windows power plan" -ForegroundColor Yellow }`,
...
function normalizeWindowsPowerShellCommand(id: string, command: string): string {
  if (id === "CodDefenderExclusion") {
    return `$ErrorActionPreference = 'Stop'; $candidatePaths = @('C:\\Program Files\\Call of Duty','C:\\Program Files (x86)\\Call of Duty','C:\\Program Files\\Battle.net Apps\\Call of Duty','C:\\Program Files (x86)\\Steam\\steamapps\\common\\Call of Duty Modern Warfare 2','D:\\Call of Duty','D:\\SteamLibrary\\steamapps\\common\\Call of Duty Modern Warfare 2','E:\\Call of Duty','E:\\SteamLibrary\\steamapps\\common\\Call of Duty Modern Warfare 2'); $found = @($candidatePaths | Where-Object { Test-Path -LiteralPath $_ }); if (-not $found.Count) { throw "Not for this system: Call of Duty installation folder was not found on the available drives." }; foreach ($path in $found) { Add-MpPreference -ExclusionPath $path -ErrorAction Stop; Write-Host "[COD] Defender exclusion added: $path" -ForegroundColor Green }; Write-Host "[COD] Defender exclusions applied to the detected Call of Duty folders." -ForegroundColor Cyan`;
  }
  if (id === "AmdCpuCStatePolicy") {
    return `$ErrorActionPreference = 'Stop'; $settingPath = 'HKLM:\\SYSTEM\\CurrentControlSet\\Control\\Power\\PowerSettings\\54533251-82be-4824-96c1-47b60b740d00\\40fbefc7-2e9d-4d25-a185-0cfd8574bae6'; if (-not (Test-Path -LiteralPath $settingPath)) { throw "Not for this system: Windows did not expose the AMD CPU performance decrease policy." }; $schemeText = (& powercfg.exe /getactivescheme 2>&1 | Out-String); if ($schemeText -notmatch '([0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12})') { throw "Not for this system: Windows did not return an active power scheme." }; $scheme = $matches[1]; & powercfg.exe /attributes 54533251-82be-4824-96c1-47b60b740d00 40fbefc7-2e9d-4d25-a185-0cfd8574bae6 -ATTRIB_HIDE 2>&1 | Out-Null; & powercfg.exe /setacvalueindex $scheme 54533251-82be-4824-96c1-47b60b740d00 40fbefc7-2e9d-4d25-a185-0cfd8574bae6 0 2>&1 | Out-Null; if ($LASTEXITCODE -ne 0) { throw "Not for this system: the active power scheme does not support the AMD CPU performance decrease policy." }; & powercfg.exe /setactive $scheme 2>&1 | Out-Null; if ($LASTEXITCODE -ne 0) { throw "Windows could not activate the current power scheme after applying the AMD CPU policy." }; Set-ItemProperty -Path $settingPath -Name 'Attributes' -Value 2 -Type DWord -Force; Write-Host "[AMD CPU] CPU performance decrease policy set to Fastest and verified." -ForegroundColor Green`;
  }
  if (id === "IntelOldGenPowerOpt" || id === "FiveM3500PerfPlan") {
    ...
    return `$ErrorActionPreference = 'Stop'; function Set-PowerValue([string]$scheme, [string]$setting, [string]$value) { & powercfg.exe /setacvalueindex $scheme SUB_PROCESSOR $setting $value 2>&1 | Out-Null; return ($LASTEXITCODE -eq 0) }; $scheme = '8c5e7fda-e8bf-4a96-9a85-a6e23a8c635c'; & powercfg.exe /setactive $scheme 2>&1 | Out-Null; if ($LASTEXITCODE -ne 0) { $scheme = 'SCHEME_CURRENT' }; $errors = @(); ${pairs} if ($errors.Count -gt 0) { throw "Not for this system: ${label} power settings unavailable ($($errors -join ', '))." }; & powercfg.exe /setactive $scheme 2>&1 | Out-Null; if ($LASTEXITCODE -ne 0) { throw "Windows could not activate the ${label} power scheme." }; Write-Host "[${label}] power plan settings applied and verified." -ForegroundColor Green`;
  }
