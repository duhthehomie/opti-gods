...
function normalizeWindowsPowerShellCommand(id: string, command: string): string {
  ...
  if (["CpuFortniteIFEO", "CpuCodIFEO", "CpuGenericGameIFEO"].includes(id)) {
    ...
    return `$ErrorActionPreference = 'Stop'; $cpu = @(Get-CimInstance Win32_Processor -Property NumberOfCores | Measure-Object NumberOfCores -Sum).Sum; $priority = if ($cpu -ge 6) { 4 } else { 3 }; $ifeo = 'Registry::HKEY_LOCAL_MACHINE\\SOFTWARE\\Microsoft\\Windows NT\\CurrentVersion\\Image File Execution Options'; @(${list}) | ForEach-Object { $key = "$ifeo\\$_\\PerfOptions"; if (!(Test-Path -LiteralPath $key)) { New-Item -LiteralPath $key -Force -ErrorAction Stop | Out-Null }; Set-ItemProperty -LiteralPath $key -Name 'CpuPriorityClass' -Value $priority -Type DWord -Force -ErrorAction Stop; Set-ItemProperty -LiteralPath $key -Name 'CpuPriorityBoost' -Value 1 -Type DWord -Force; Set-ItemProperty -LiteralPath $key -Name 'DisableEnergyThrottling' -Value 1 -Type DWord -Force; Set-ItemProperty -LiteralPath $key -Name 'ForceForegroundBoost' -Value 1 -Type DWord -Force; Set-ItemProperty -LiteralPath $key -Name 'IoPriority' -Value 2 -Type DWord -Force; Set-ItemProperty -LiteralPath $key -Name 'PagePriority' -Value 5 -Type DWord -Force; Set-ItemProperty -LiteralPath $key -Name 'GpuPriority' -Value 8 -Type DWord -Force }; Write-Host "[${label}] IFEO CPU priority and foreground boost applied." -ForegroundColor Green`;
  }
