...
`;
  ...
  app.get('/api/rocket-league-fix-script', (req, res) => {
    ...
    const ps1Lines = [
      ...
      `} Else {`,
      `    Write-Host "  Rocket League exe not found (Epic/Steam paths checked)." -ForegroundColor DarkGray`,
      `}`,
      ``,
      ...
      `Write-Host "  2. Wait for shaders to re-download (~2-5 min)" -ForegroundColor Yellow`,
      `Write-Host ""`,
      `Write-Host "  If STILL not working:" -ForegroundColor Yellow`,
      `Write-Host "  - Uninstall Rocket League via Epic Games" -ForegroundColor DarkGray`,
      `Write-Host "  - Delete: $env:ProgramFiles\Epic Games\Rocket League" -ForegroundColor DarkGray`,
      `Write-Host "  - Reinstall fresh from Epic Games" -ForegroundColor DarkGray`,
