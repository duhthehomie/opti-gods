...
const TWEAK_COMMANDS: Record<string, string> = {
  ...
  CodPagefileOptimize: `$cs = Get-CimInstance Win32_ComputerSystem; $ramMB = [int]($cs.TotalPhysicalMemory / 1MB); $minMB = [Math]::Max(4096, [Math]::Min(16384, [int]($ramMB / 2))); $maxMB = [Math]::Max($minMB, [Math]::Min(32768, $ramMB)); if ($cs.AutomaticManagedPagefile) { Set-CimInstance -InputObject $cs -Property @{ AutomaticManagedPagefile = $false } | Out-Null }; $pf = Get-CimInstance Win32_PageFileSetting -EA SilentlyContinue | Where-Object { $_.Name -like 'C:*' } | Select-Object -First 1; if ($pf) { Set-CimInstance -InputObject $pf -Property @{ InitialSize = $minMB; MaximumSize = $maxMB } | Out-Null } else { New-CimInstance -ClassName Win32_PageFileSetting -Property @{ Name = 'C:\pagefile.sys'; InitialSize = $minMB; MaximumSize = $maxMB } | Out-Null }; Write-Host "[COD] Pagefile set to $([int]($minMB/1024))GB-$([int]($maxMB/1024))GB based on installed RAM." -ForegroundColor Green`,
