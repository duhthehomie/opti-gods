...
const RESTORE_BLOCKS: Record<string, { label: string; commands: string[] }> = {
  ...
  nvidia: {
    ...
    commands: [
      ...
      // EnableMSIMode_Safe rollback (parity with Fixes tab) — wipes any
      // Affinity Policy residue on every display device, matching what V2.1
      // already did for the V1 EnableMSIMode BSOD victims.
      `$gpuClass = 'HKLM:\\SYSTEM\\CurrentControlSet\\Control\\Class\\{4d36e968-e325-11ce-bfc1-08002be10318}'; Get-ChildItem $gpuClass -EA SilentlyContinue | ForEach-Object { $base = $_.PSPath; $affPath = Join-Path $base 'AffinityPolicy'; If (Test-Path $affPath) { Remove-ItemProperty -Path $affPath -Name 'DevicePolicy' -EA SilentlyContinue; Remove-ItemProperty -Path $affPath -Name 'DevicePriority' -EA SilentlyContinue; Remove-ItemProperty -Path $affPath -Name 'AssignmentSetOverride' -EA SilentlyContinue } }; Write-Host "[OK] EnableMSIMode_Safe rollback — Affinity Policy residue cleared on all display devices" -ForegroundColor Green`,
    ],
  },
  amd: {
    label: "AMD",
