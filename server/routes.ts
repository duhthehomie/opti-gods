...
const RESTORE_BLOCKS: Record<string, { label: string; commands: string[] }> = {
  ...
  nvidia: {
    ...
    commands: [
      ...
      // EnableMSIMode_Safe rollback — mirrors apply scope EXACTLY:
      // Display + filtered Net (excludes Virtual/Loopback/Bluetooth/WAN/Tunnel/Hyper-V)
      // + filtered SCSIAdapter (NVMe only). For every targeted device, sets
      // MSISupported=0 and wipes DevicePolicy/DevicePriority/AssignmentSetOverride
      // (the V1-BSOD trigger).
  ...
  amd: {
    ...
    commands: [
      ...
      // mirrors apply scope EXACTLY: Display + filtered Net + NVMe SCSIAdapter.
