...
`;
  ...
  app.get('/api/mushy-face-fix-script', (req, res) => {
    const ps1Lines = [
      ...
      `}`,
      ``,
      `# ── FIX 2: Expand FiveM texture streaming pool ───────────────────────────────`,
      `Write-Host ""`,
      `Write-Host "[FIX 2] Expanding FiveM texture streaming pool..." -ForegroundColor Cyan`,
      `$cmdlinePath = "$env:LocalAppData\\FiveM\\FiveM Application Data\\commandline.txt"`,
      `$streamFlag  = "+set streaming_useLargerPool 1"`,
      `$cmdlineDir  = Split-Path $cmdlinePath`,
      `If (-not (Test-Path $cmdlineDir)) { New-Item -ItemType Directory -Path $cmdlineDir -Force | Out-Null }`,
      ...
      `If ($cmdlineContent -notmatch [regex]::Escape($streamFlag)) {`,
      `  Add-Content $cmdlinePath "\`n$streamFlag" -Encoding UTF8`,
      `  Write-Host "  [OK] Added '$streamFlag' to FiveM commandline.txt" -ForegroundColor Green`,
      `} Else {`,
      `  Write-Host "  [OK] Streaming pool flag already present — no change needed" -ForegroundColor DarkGray`,
      `}`,
      ``,
      `Write-Host ""`,
      `Write-Host "  =====================================================" -ForegroundColor DarkRed`,
      `Write-Host "  DONE — Mushy face / arm texture fix applied!" -ForegroundColor Green`,
      `Write-Host ""`,
      `Write-Host "  What was fixed:" -ForegroundColor White`,
      `Write-Host "  - GPU driver texture filtering set to High Quality (sharper skin samples)" -ForegroundColor Gray`,
      `Write-Host "  - FiveM streaming pool expanded (loads higher-res face LODs)" -ForegroundColor Gray`,
      `Write-Host ""`,
      `Write-Host "  IMPORTANT: Restart FiveM / GTA V for changes to take effect." -ForegroundColor Red`,
      `Write-Host "  NOTE: If you applied the 'High Performance texture filter' tweak in Opti Gods" -ForegroundColor Yellow`,
      `Write-Host "        NVIDIA or AMD tab, this fix reverses it (small FPS trade for face quality)." -ForegroundColor Yellow`,
      `Write-Host ""`,
      `Write-Host "  Opti Gods by leaq -- discord.gg/optigods" -ForegroundColor DarkRed`,
      `Write-Host ""`,
      `Read-Host "Press Enter to close"`,
