...
const TWEAK_COMMANDS: Record<string, string> = {
  ...
  FiveMMenuFpsUncap: `$wsh = New-Object -ComObject WScript.Shell; $paths = @("$env:APPDATA\\Microsoft\\Windows\\Start Menu\\Programs\\FiveM.lnk","$env:PUBLIC\\Desktop\\FiveM.lnk","$env:USERPROFILE\\Desktop\\FiveM.lnk","$env:OneDrive\\Desktop\\FiveM.lnk" -EA SilentlyContinue); $updated = 0; foreach ($p in $paths) { If (Test-Path $p) { try { $sc = $wsh.CreateShortcut($p); If ($sc.Arguments -notmatch 'fps_max') { $sc.Arguments = ($sc.Arguments + ' +set fps_max 0').Trim(); $sc.Save(); Write-Host "[FiveM] Patched shortcut: $p — added +set fps_max 0" -ForegroundColor Green } Else { Write-Host "[FiveM] Shortcut already has fps_max: $p" -ForegroundColor DarkGray }; $updated++ } catch { Write-Host "[FiveM] Could not patch $p" -ForegroundColor Yellow } } }; If ($updated -eq 0) { Write-Host "[FiveM] No FiveM shortcuts found automatically." -ForegroundColor Yellow; Write-Host "[FiveM] Manual fix: right-click your FiveM shortcut -> Properties -> Target -> append: +set fps_max 0" -ForegroundColor Cyan }; $gpuClass = 'HKLM:\\SYSTEM\\CurrentControlSet\\Control\\Class\\{4d36e968-e325-11ce-bfc1-08002be10318}'; 0,1,2,3 | ForEach-Object { $k = "$gpuClass\\000$_"; If ((Test-Path $k) -and (Get-ItemProperty $k -Name 'DriverDesc' -EA SilentlyContinue).DriverDesc -match 'NVIDIA') { Set-ItemProperty $k 'OpenGLCompatibilityMode' 0 -Type DWord -Force -EA SilentlyContinue; Write-Host "[NVIDIA] OpenGL GDI Compatibility = Prefer Performance" -ForegroundColor Green } }`,
