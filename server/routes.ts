...
"@; Add-Type $code; [MemoryHelper]::EmptyWorkingSet([IntPtr](-1))`,
  ...
  FiveMCommandLineTweaks: `$dir = "$env:USERPROFILE\\Documents\\Rockstar Games\\GTA V"; If (!(Test-Path $dir)) { New-Item $dir -ItemType Directory -Force | Out-Null }; Set-Content "$dir\\commandline.txt" "-dx11 -nomemrestrict -norestrictions -noBlockOnLostFocus -novblank -noprecisefp"; Write-Host "[GTA V] commandline.txt written: -dx11 -nomemrestrict -norestrictions -noBlockOnLostFocus -novblank -noprecisefp" -ForegroundColor Green; Write-Host "[GTA V] DX11 is most stable on GTX 1650; nomemrestrict removes VRAM ceiling; novblank removes VSync frame lock" -ForegroundColor Cyan`,
  // Win11 Debloat
